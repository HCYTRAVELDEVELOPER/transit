...
class exportExcelV2 {
    ...
    public static function excelToArray($p) {
        ...
        require_once dirname(__FILE__) . '/includes/PhpSpreadsheet/vendor/autoload.php';
                    ...
                    foreach ($campos[$iz] as $key => $value) {
                        if (in_array($key, $p["cols"])) {
                            if (!in_array($key, $p["noExportColumns"])) {
                                $colName = $campos[$iz][$key];
                                if (is_array($colName)) {
                                    $colName = utf8_decode($colName["id"]);
                                } else {
                                    $colName = utf8_decode($colName);
                                }
                                $insert[strip_tags(NWUtils::cleanCols($p["labels"][$key]))] = $colName;
                            }
                        }
                    }
                    if (count($insert) > 0) {
                    ...
                    if (in_array($key, $p["cols"])) {
                        if (!in_array($key, $p["noExportColumns"])) {
                            ...
                            if (is_array($colName)) {
                                ...
                                if (!mb_detect_encoding($colName, 'UTF-8', true)) {
                                    $colName = mb_convert_encoding(utf8_encode($colName), "UTF-8", "auto");
                                }
                            }
                            $colName = strip_tags($colName);
                            $insert[$key] = preg_replace('/[\x00-\x1F\x7F-\xA0\xAD]/u', '', $colName);
                        }
                    }
                }
                if (count($insert) > 0) {
    ...
    public static function exportXLSLight($p, $sd = false, $noExport = Array(), $export = Array(), $rowHeight = false, $sorted = '', $special = true) {
        ...
        $filename = $_SERVER["DOCUMENT_ROOT"] . "/tmp/" . (string) str_replace(":", "_", $part) . "_" . (string) str_replace(":", "_", $si["usuario"]) . "_" . (string) $si["empresa"] . ".xlsx";
            ...
            foreach ($row->getElementsByTagName("td") as $col) {
                ...
                if ($imgInside->length != 0) {
                    ...
                    if ($havePath === true && $imgEncPath != "") {
                        ...
                        $objDrawing->setPath($imgEncPath, true);
                            ...
                            for ($ixx = 0; $ixx < count($exploded); $ixx++) {
                                ...
                                switch (trim($style[0])) {
                                    case "text-align":
                                        if (isset($style[1])) {
                                            if ($style[1] == "center") {
                                                $objPHPExcel->getActiveSheet()->getStyle($letters[$counterEnc] . $counterCols)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
                                            } else if ($style[1] == "left") {
                                                $objPHPExcel->getActiveSheet()->getStyle($letters[$counterEnc] . $counterCols)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT);
                                            } else if ($style[1] == "right") {
                                                $objPHPExcel->getActiveSheet()->getStyle($letters[$counterEnc] . $counterCols)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);
                                            }
                                        }
                                        break;
                                    case "font-weight":
...
//        ini_set('display_startup_errors', TRUE);
        ...
        if ($name == '') {
            $filename = "/tmp/" . (string) str_replace(":", "_", $part) . "_" . (string) str_replace(":", "_", $si["usuario"]) . "_" . (string) $si["empresa"] . ".xlsx";
        } else {
        ...
        //$cacheMethod = \PhpOffice\PhpSpreadsheet\Collection\CellsFactory::cache_to_memcache;
...
//        }
        ...
        for ($izzz = 0; $izzz < count($pa); $izzz++) {
                ...
                for ($ia = 0; $ia < $maxCounter; $ia++) {
                    ...
                    if ($type == "image" || $type == "uploader") {
                        ...
                        if (!file_exists($img_path)) {
                            ...
                            if (!file_exists($img_path)) {
                                ...
                                if (!file_exists($img_path)) {
                                    ...
                                    $width_image = self::checkImageSize($img_path);
                                    if (!file_exists($img_path)) {
                                        $img_path = "http://" . $_SERVER["HTTP_HOST"] . "/" . $val;
                                        $width_image = self::checkImageSize($img_path);
                                        if (!file_exists($img_path)) {
                                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($letters[$columnCount] . $startNumber, $val);
                                        } else {
                                            ...
                                            $objDrawing->setPath($img_path, true);
                        ...
                        if ($special === true && $izzz == 0) {
                            ...
                            if ($type == "string") {
                                $objPHPExcel->getActiveSheet()->setCellValueExplicit($letters[$columnCount] . $startNumber, $val, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
                            } else if ($type == "money") {
                                $objPHPExcel->getActiveSheet()->getStyle($letters[$columnCount] . $startNumber)->getNumberFormat()->setFormatCode("$#,##0.00");
                            } else if ($type == "percent") {
                                $objPHPExcel->getActiveSheet()->getStyle($letters[$columnCount] . $startNumber)->getNumberFormat()->setFormatCode("0.00");
                            } else if (($type == "date" || $type == "dateField" || $type == "dateTimeField") && $val != "" && $val != null) {
                                $internal = explode(" ", trim($val));
                                $format = "Y-m-d";
                                $formatExcel = "yyyy-mm-dd";
                                ...
                                if (count($internal) > 1) {
                                    $format = $format . " H:i:s";
                                    $formatExcel = $formatExcel . " hh:mm:ss";
                                }
                                if ($haveDiff === false) {
                                    if (self::verifyDate($val, $format)) {
                                ...
                                $val = \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($unixTimestamp);
                                $objPHPExcel->getActiveSheet()->getStyle($letters[$columnCount] . $startNumber)->getNumberFormat()->setFormatCode($formatExcel);
                            } else if ($type == "numeric") {
                                $objPHPExcel->getActiveSheet()->getStyle($letters[$columnCount] . $startNumber)->getNumberFormat()->setFormatCode("#,##0.00");
                            } else if ($type == "textArea") {
                                $objPHPExcel->getActiveSheet()->getStyle($letters[$columnCount] . $startNumber)->getAlignment()->setWrapText(true);
                            ...
                            if (is_array($val)) {
                                ...
                                if ($type != "string") {
                                    if ($type == "html") {
                                        ...
                                        if ($val === "" || $val === "f" || $val === "false" || $val === false) {
                                            $val = "NO";
                                        } else {
                                            $val = "SI";
                                        }
            ...
            if ($plot_img != false && $plot_img != '' && $izzz == 0) {
...
//        $id = master::getNextSequence("nw_downloads" . "_id_seq", $db);
    ...
    public static function verifyDate($date, $format, $strict = false) {
    ...
    public static function exportPHPExcelSimple($p) {
        ...
        $d = exportExcelV2::exportXLS2007($p["records"], $part, $noExport, $export, $rowHeight, 0, true, $plot_img, $boolEnc, $maxRowsEnc, $name, $startNumbers, $tittle, $dimensions);
        $exportId = $d["id"];
        $exportKey = $d["key"];
        return array(
            "id" => $exportId,
