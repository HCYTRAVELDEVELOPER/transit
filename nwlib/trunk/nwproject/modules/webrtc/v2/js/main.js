...
//    var val = "13123 hola 5 465mundo 3";
//    var regex = /(\d+)/g;
//    var name = val;
...
//    }
    ...
    if (get.callEnd === "true") {
        ...
        var point = false;
        var btns = document.querySelectorAll(".califica");
        for (var i = 0; i < btns.length; i++) {
            var btn = btns[i];
            btn.addEventListener("click", function () {
                point = this.getAttribute("data-val");
            });
        }
        document.querySelector(".send_califica").addEventListener("click", function () {
            if (!point) {
                return false;
            }
            var dat = {};
            dat.calificacion = point;
            dat.id_all = get.id_call;
            dat.terminal = get.terminal;
            dat.usuario_califica = get.usuario_califica;
            dat.asesor = get.asesor;
            dat.comentarios = document.querySelector(".textarea_califica").value;
            webrtcNw.califica(dat);
        });
...
//        }
    ...
    createLienzo: function () {
        ...
        if (self.debug)
            console.log("self.useForRingow", self.useForRingow);
        self.containerButtons = document.createElement("div");
        self.containerButtons.className = "containerButtons isOperatorRingow_" + self.isOperatorRingow + " useForRingow_" + self.useForRingow;
...
//                        }
        ...
        self.init = false;
//        console.log("setReady only one time");
//        var sum = 0;
...
//            console.log("self.init", self.init);
//            if (self.init === true) {
//                return false;
//            }
            if (self.debug)
...
//                self.showChat();
                ...
                if (self.isOperatorRingow === "true" || self.isOperatorRingow === true) {
                    ...
                    if (langOrigin !== null && langOrigin !== "" && langTranslate !== null && langTranslate !== "" && langOrigin !== langTranslate) {
                        self.translateText(val, langOrigin, langTranslate, function (textTranslate) {
                            self.lastTextTraducido = textTranslate.source;
                            continueReceivedMsg(textTranslate.translation);
                        });
                    } else {
                        continueReceivedMsg(val);
                    }
                } else {
                    continueReceivedMsg(val);
                }
                ...
                function continueReceivedMsg(val) {
                    self.addMessage(val, false, false, false, data.payload.user, "NO");
...
//            d.innerHTML = "<img src='" + self.domain + "/nwlib6/icons/lg.-text-entering-comment-loader.gif' style='height: 56px;position: relative;display: inline-block;position: absolute;top: -39px;left: -10px;z-index: 0;'><span>" + user + " está escribiendo...</span>";
    ...
    endCall: function (initial) {
        ...
        if (initial !== true) {
            window.location = "?callEnd=true&id_call=" + self.id_call + "&terminal=" + self.terminal + "&usuario_califica=" + self.myUser + "&asesor=" + self.heUser;
        }
    },
    close: function () {
        var self = this;
...
//        var con = document.querySelector(".containerButtons");
        ...
        self.terminarBeforeBtn = document.createElement("button");
        self.terminarBeforeBtn.className = "terminar_before";
        self.terminarBeforeBtn.innerHTML = "<i class='material-icons'>call_end</i>";
        self.terminar_before_container.appendChild(self.terminarBeforeBtn);
        self.terminarBeforeBtn.onclick = function () {
        ...
        self.btnShareScreen = document.createElement("button");
        self.btnShareScreen.className = "btnShareScreen";
        self.btnShareScreen.innerHTML = "<i class='material-icons'>screen_share</i>";
        con.appendChild(self.btnShareScreen);
        self.btnShareScreen.onclick = function () {
            if (self.webrtc.getLocalScreen()) {
...
//        document.querySelector(".containerChat").appendChild(d);
        ...
        self.dictation.innerHTML = "<img src='" + self.domain + "/nwlib6/nwproject/modules/webrtc/v2/icons/mic.svg' />";
        self.dictation.onclick = function () {
            self.startDictation(".messageBox");
        };
...
//        html += "<i class='material-icons iconsEnc settingsEnc'>settings</i> ";
        html += "<img class='iconsEnc settingsEnc' src='" + self.domain + "/nwlib6/nwproject/modules/webrtc/v2/icons/settings.png' style='position: absolute;right: 28px;' /> ";
        ...
        html += "<div class='myName' style='display: none;'>myName: " + self.myName + "</div>";
//        document.querySelector(".encChat").innerHTML = html;
        self.encChat.innerHTML = html;

        document.querySelector(".settingsEnc").addEventListener("click", function () {
            self.openSettings();
        });
...
//            self.removeClass(".containerChat", "containerChatVisible");
    ...
    openSettings: function () {
        ...
        sel.innerHTML = "<img src='" + self.domain + "/nwlib6/icons/svg/close.svg' />";
...
//            self.tokens = [];
    ...
    saveMsgSrv: function (texto, myname) {
        ...
        var textoSave = texto;
        textoSave = self.renderHTML(textoSave, true);
...
//                "restricted_package_name":""
    ...
    translateText: function (text, langOrigin, langTranslate, callback) {
        ...
        oReq.onload = reqListener;
        oReq.onerror = reqError;
        oReq.open('POST', self.domain + '/nwlib6/nwproject/modules/webrtc/v2/rpc/translateText.php', true);
        oReq.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        oReq.send(data);
    },
...
//            cssMain += "";
    ...
    califica: function (dat) {
        var self = this;
        var data = "";
        data += "id=" + dat.id_call;
        data += "&asesor=" + dat.asesor;
        data += "&usuario_califica=" + dat.usuario_califica;
        data += "&terminal=" + dat.terminal;
        data += "&calificacion=" + dat.calificacion;
        data += "&comentarios=" + dat.comentarios;
        function reqListener() {
            console.log(this.responseText);
            var data = JSON.parse(this.responseText);
            alert(data);
        ...
        oReq.onload = reqListener;
        oReq.onerror = reqError;
        oReq.open('POST', self.domain + '/app/nwchat/js/califica.php', true);
        oReq.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        oReq.send(data);
    },
    ...
    startDictation: function (div) {
        ...
        if (window.hasOwnProperty('webkitSpeechRecognition')) {
            ...
            var da = window.localStorage.getItem(self.nameConference + "send_auto_voice_text");
            if (da === "SI") {
            ...
            var da = window.localStorage.getItem(self.nameConference + "select_voice_text_detener");
            if (da === "SI") {
                stopOverVoice = true;
            } else
            if (da === "NO") {
            ...
            var d = document.createElement("div");
            d.innerHTML = "<div class='speak_mic_int' style='position: relative;margin: auto;background-color: #fff;padding: 15px 40px;border-radius: 5px;font-size: 25px;'><img src='" + self.domain + "/nwlib6/nwproject/modules/webrtc/v2/icons/mic.svg' /> Escuchando...</div>";
            ...
            d.innerHTML = "<img src='" + self.domain + "/nwlib6/nwproject/modules/webrtc/v2/icons/stop_blanco.png' /> Detener</div>";
            d.onclick = function () {
                recognition.stop();
                self.removeDictation();
            };
