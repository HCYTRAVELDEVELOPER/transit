var srv = {
    ...
    initialize: function () {
        ...
        self.domain = rtcNw.domain;

        self.is_group = rtcNw.is_group;
...
//        }
    ...
    start: function () {
        ...
        if (rtcNw.useBot === true) {
            self.bot = true;
        }

        if (self.tipoChat === "VIDEOAUDIO") {
            return false;
        }

        if (self.evalueData(windowStorage.getItem(self.room + "_nombre_cliente"))) {
            self.nombre_cliente = windowStorage.getItem(self.room + "_nombre_cliente");
            self.bot_ha_pedido_nombre = true;
            self.bot_ha_saludado = true;
            self.bot_ha_indicado_pedir_datos = true;
        }
        if (self.evalueData(windowStorage.getItem(self.room + "_celular_cliente"))) {
            self.celular_cliente = windowStorage.getItem(self.room + "_celular_cliente");
            self.bot_ha_pedido_celular = true;
            self.bot_ha_saludado = true;
            self.bot_ha_indicado_pedir_datos = true;
        }
        if (self.evalueData(windowStorage.getItem(self.room + "_correo_cliente"))) {
            self.correo_cliente = windowStorage.getItem(self.room + "_correo_cliente");
            self.bot_ha_pedido_correo = true;
            self.bot_ha_saludado = true;
            self.bot_ha_indicado_pedir_datos = true;
        }
        if (self.evalueData(windowStorage.getItem(self.room + "_documento_cliente"))) {
            self.documento_cliente = windowStorage.getItem(self.room + "_documento_cliente");
    ...
    loadAllMessagesServer: function () {
        ...
        self.saveOrUpdate(function (r) {
            ...
            if (typeof r.msg !== "undefined") {
                if (r.msg.length >= 10) {
                    var btnan = document.querySelector(".btn_msgs_antiguos");
                    if (!btnan) {
                        var da = document.createElement("button");
                        da.className = "btn_msgs_antiguos";
                        da.innerHTML = "Mensajes más antiguos";
                        da.onclick = function () {
                            self.limitLoadMessages = self.limitLoadMessages + 10;
                            self.loadAllMessagesServer();
                        };
                        rtcNw.containerMessages.appendChild(da);
                    }
                    if (r.msg.length === self.totalLoadMessages) {
                        btnan.innerHTML = "No hay más mensajes";
                    }
                    self.totalLoadMessages = r.msg.length;
                }
                var msgs = r.msg;
                self.onlyAddMsgCanvas(msgs);
            }
            if (self.firtsTimeLoadMessages === false) {
                self.activeFuncs();
            }
            self.firtsTimeLoadMessages = true;
        });
    },
    activeFuncs: function () {
        var self = this;
...
//        self.changeStatusCall("LLAMANDO", "chat");
        ...
        self.btnCallAgain.onclick = function () {
            ...
            rtcNw.addMessage({text: msg, type: "other", name: self.myID_no_clean, cleanHTML: false});
...
//                if (user === self.myID || user === "auto") {
            if (user === self.myID) {
                var myuser = self.myID_no_clean;
                if (user === "auto") {
                    myuser = "auto";
                }
                var cleanHTML = elemen.querySelector(".message__cleanHTML").innerHTML;
                self.lastMsg = msg;
                self.lastUserSendMessage = myuser;
...
//                self.resetDataLocal();
    ...
    onlyAddMsgCanvas: function (msgs) {
        ...
        for (var i = 0; i < msgs.length; i++) {
            ...
            rtcNw.addMessage({text: msg.texto, type: type, name: msg.usuario, date: msg.fecha, recibido: msg.recibido, visto: msg.leido, cleanHTML: false});
        }
        setTimeout(function () {
    ...
    resetDataLocal: function () {
        var self = this;
        windowStorage.removeItem(self.room + "_nombre_cliente");
        windowStorage.removeItem(self.room + "_celular_cliente");
        windowStorage.removeItem(self.room + "_correo_cliente");
        windowStorage.removeItem(self.room + "_documento_cliente");
    ...
    setTerminalKey: function () {
        ...
        if (rau.length > 1) {
            ...
            if (self.debug)
                console.log("id_call", self.id_call);
            ...
            self.tipoDeUso = "ringow_need_config";
        ...
        if (rau.length > 1) {
            ...
            var r = self.room.split("_mode_calling_");
            if (r.length > 1) {
                self.tipoChat = "VIDEOAUDIO";
            }
    ...
    saveMsgSrv: function () {
        ...
        var textoSave = self.lastMsg;
        if (self.cleanHTML === "true" || self.cleanHTML === true) {
            textoSave = utils.renderHTML(self.lastMsg, true);
        }
...
//                self.readMsgCall("recibido");
    ...
    proccessError: function (error) {
        var self = this;
        var dato = error.error.message;
        function reqListener() {
            var data = JSON.parse(this.responseText);
            console.log(data);
        }
        ...
        oReq.onload = reqListener;
        oReq.onerror = reqError;
        oReq.open('POST', self.domain + '/nwlib6/nwproject/modules/webrtc/v4/srv/serverError.php', true);
        oReq.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        oReq.send(dato);
    },
    saveOrUpdate: function (callback) {
        var self = this;
        var id = self.room;
        if (self.evalueData(windowStorage.getItem("conversation_room_" + id))) {
            var ras = windowStorage.getItem("conversation_room_" + id);
        ...
        function reqListener() {
            ...
            windowStorage.setItem("conversation_room_" + id, JSON.stringify(data));
...
//            var name ='RecordRTC-' + year + month + date + '-' + self.room + '-' + getRandomString() + '.' + fileExtension;
    ...
    botValidateResponse: function () {
        ...
        if (self.bot_ha_pedido_nombre === true && self.nombre_cliente === false) {
            ...
            windowStorage.setItem(self.room + "_nombre_cliente", self.nombre_cliente);
        }
        if (self.bot_ha_pedido_celular === true && self.celular_cliente === false) {
            self.celular_cliente = msgOther;
            windowStorage.setItem(self.room + "_celular_cliente", self.celular_cliente);
        }
        if (self.bot_ha_pedido_correo === true && self.correo_cliente === false) {
            self.correo_cliente = msgOther;
            windowStorage.setItem(self.room + "_correo_cliente", self.correo_cliente);
        }
        if (self.bot_ha_pedido_identificacion === true && self.documento_cliente === false) {
            self.documento_cliente = msgOther;
            windowStorage.setItem(self.room + "_documento_cliente", self.documento_cliente);
        }
        if (self.debug) {
            console.log("botValidateResponse", self.config);
            console.log("self.bot_ha_indicado_pedir_datos", self.bot_ha_indicado_pedir_datos);
            console.log("self.bot_ha_pedido_nombre", self.config.registro_usar_nombre);
        }
        ...
        if (self.nombre_cliente !== false && self.celular_cliente !== false && self.correo_cliente !== false) {
            ...
            self.botOff();
            setTimeout(function () {
                console.log("Bot dispara la llamada");
                self.changeStatusCall("LLAMANDO", "chat");
            }, 1000);
        }
    ...
    botSendMessage: function () {
        ...
        setTimeout(function () {
            ...
            setTimeout(function () {
                console.log("mensaje", self.bot_message);
