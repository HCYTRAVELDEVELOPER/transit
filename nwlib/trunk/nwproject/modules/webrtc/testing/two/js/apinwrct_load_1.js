...
function nwRct() {
    ...
    function startConversation(userOne, userTwo, type, mode, isgroup, openForm, dataOpenForm, lienzo) {
        ...
        if (openForm === true) {
            ...
            url += "&texto_bienvenida=" + dataOpenForm.texto_bienvenida;
...
cursor: pointer;\n\
    ...
    padding: 0px;' class='userfoto_ringow'></span>";
            ...
            enc += "<span  onclick='javascript: minimizeWinWrtc(\"." + classt + "\")' style='max-height: 15px;overflow:hidden;cursor: pointer;font-size: 13px;font-weight: bold;text-align: left;display: block;line-height: normal;' class='linkencUsRing minimize2Winwrc_" + classt + "'>" + usernametwo_show_enc + " <span class='dataencringreg'>" + country + " " + city + "</span></span>";
            enc += "<span  onclick='javascript: openWinWrtc(\"." + classt + "\")' style='cursor: pointer;font-size: 13px;font-weight: bold;text-align: left;display: none;line-height: normal;' class='linkencUsRing openWinwrc_" + classt + "'>" + usernametwo_show_enc + "</span>";
            enc += "<span class='closeWinwrc minimizeWinwrc_" + classt + "' style='position: absolute;top: -3px;right: 20px;cursor: pointer;font-weight: bold;font-size: " + fontsizemenos + "px;line-height: normal;' onclick='javascript: minimizeWinWrtc(\"." + classt + "\")'>-</span>";
            if (remove === true) {
                url += "&useinapp=true";
                enc += "<span class='closeWinwrc closeWinwrc_" + classt + "' style='font-size: " + fontsize + "px;position: absolute;top: 2px;right: 3px;cursor: pointer;line-height: normal;' onclick='javascript: closeWinWrtc(\"." + classt + "\", false)'>X</span>";
            } else {
                enc += "<span class='closeWinwrc closeWinwrc_" + classt + "' style='font-size: " + fontsize + "px;position: absolute;top: 2px;right: 3px;cursor: pointer;line-height: normal;' onclick='javascript: closeWinWrtc(\"." + classt + "\")'>X</span>";
            }
            enc += "</div>";
            var con = document.querySelector("." + classkok);
            ...
            if (isMobileRingow()) {
                ...
                css += "height: " + bottom + "px;width: 300px;float:left;margin-left:10px;margin-right:10px;border-radius: 25px 5px 15px 5px;" + position;
            }
            var iframeshow = enc;
            iframeshow += loadingRingow();
            iframeshow += "<iframe src='" + url + "' class='iframeclassconrt iframeclassconrt_" + classt + "' style='position: relative;width: 100%;height: 100%;border:0;z-index: 2;'></iframe>";
            var f = document.createElement("div");
...
//                                userone.username_show = userone_user_show;
            ...
            if (r.tipo === "activeInactiveWindRing") {
                ...
                //listado ringow
                var all = document.querySelectorAll(".l_ultimas_llamadas .colsMobil");
                if (all) {
                    for (var i = 0; i < all.length; i++) {
                        var el = all[i];
                        ringowRemoveClass(el, "bloqueactiveall", true);
                    }
                }
                if (typeof r.classt !== "undefined") {
                    var us = r.classt;
                    ...
                    if (ot) {
                        ...
                        //listado ringow
                        var op = document.querySelector(".rowonline_" + us.replace("conversnwrtc_", ""));
                        if (op) {
                            ringowAddClass(op, "bloqueactiveall");
                        }
