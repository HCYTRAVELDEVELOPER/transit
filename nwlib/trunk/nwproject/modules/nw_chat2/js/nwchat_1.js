...
//            return;
    ...
    var show = false;
    var automensaje = false;
    if (typeof pr.mensaje_al_visitante !== "undefined") {
        if (pr.mensaje_al_visitante != "") {
            show = true;
            if (pr.mensaje_al_visitante.indexOf("automensaje__") != -1 && typeof pr.atiende === "object") {
                automensaje = true;
            }
        }
    }
    var tim = 10500;
    if (typeof pr.tiempo_abrir_chat_auto !== "undefined") {
        if (pr.tiempo_abrir_chat_auto !== null && pr.tiempo_abrir_chat_auto !== "" && pr.tiempo_abrir_chat_auto !== false) {
            tim = parseInt(pr.tiempo_abrir_chat_auto);
        }
    }
    if (typeof pr.abrir_chat_automaticamente !== "undefined") {
        if (pr.abrir_chat_automaticamente === "SI") {
            automensaje = true;
        }
    }
    if (automensaje === true) {
        if (__launchFirstAutomatic === true) {
            return;
        }
        if (!isMobileNwChat()) {
        ...
        setTimeout(function () {
            pr.tipo = "chat";
            pr.estado = "EN LINEA";
            pushInsideBoxRingow(pr);
        }, tim);
        __launchFirstAutomatic = true;
    }
    if (show === true && automensaje === false) {
        pr.tipo = "chat";
        pr.estado = "EN LINEA";
    }
    if (!pushInsideBoxRingow(pr)) {
//        return;
    }
//    pushOutsideBoxRingow(pr);
}
function pushInsideBoxRingow(pr) {
...
function pushOutsideBoxRingow(pr) {
    ...
    if (typeof config !== "undefined") {
        if (typeof config.foto_autorespondedor !== "undefined") {
            cssFoto += "background-image:url(" + __domainNwChat + pr.config.foto_autorespondedor + ");";
        }
    }
...
window.addEventListener('message', function (e) {
    if (e.data == "loadAllFrameMonRingow") {
//        var frame = document.querySelector('#iframeNwChatMoonIframe');
//        frame.contentWindow.postMessage(false, '*');
        return;
    }
