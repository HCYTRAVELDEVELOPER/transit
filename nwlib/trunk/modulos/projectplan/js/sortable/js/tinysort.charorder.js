                ...
                , sAllCharNotLatin = ''
                    ...
                    if (!bIsLatin) {
                        if (sChar == '[') { // find replace chars: ë will sort similar to e
                            ...
                            for (j = 0, m = sReplaces.length; j < m; j++)
                                fnAddNonLatinChar(sReplaces[j], sLastChar);
//							for (j=0,m=sReplaces.length;j<m;j++) fnAddNonLatinChar(sLastChar,sReplaces[j]);
                    ...
                    if (i + 1 === l)
                        rxNotLatin = new RegExp('[' + sAllCharNotLatin + ']', 'gi'); // make regex to test for chars
                    else if (bIsLatin)
                        sCharLatin = sChar;
                }
            }
...
//					console.log('iAchr',iAchr,a[k],'\tiBchr',iBchr,b[k]); // log
                    if (sortReturn = criterium.iAsc * (iAchr < iBchr ? -1 : (iAchr > iBchr ? 1 : 0)))
                        break;
                }
            }
        }
