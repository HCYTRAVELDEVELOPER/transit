...
$cb = new NWDbQuery($db);
$where__ = "";
$where__ .= " where b.estado<>'CERRADO' and a.user_responsable=:user";
$where__ .= " group by c.id,c.nombre,b.id";
$sqlb = "select c.id,b.nombre,b.id as id_p
    from projectplan_responsables a
    left join projectplan_enc b on (a.id_enc=b.id) 
    left join proyectos c on (b.id_proyecto=c.id) 
    " . $where__;
$cb->prepare($sqlb);
    ...
    for ($i = 0; $i < $cb->size(); $i++) {
        ...
        <script>
            $("#_notification<?php echo $r["id"] ?>").click(function() {
                listTaskPr(<?php echo $r["id"]; ?>,<?php echo $r["id_p"]; ?>);
            });
        </script>
        <?php
        echo "<div class='box_projects_lista'><p id='_notification" . $r["id"] . "'>" . $r["nombre"] . "</p>";
        echo "<div class='div_none_style'><a class='see_cronograme' onclick='javascript: SeeCronograma(" . $r["id_p"] . ");' >Cronograma</a></div></div>";
    }
    ?>
</div>
<div class="carga"></div>
