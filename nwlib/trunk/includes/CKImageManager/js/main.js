/*
 * Copyright 2010 - Andrés Flórez. All rights reserved.
 *
 * main.js
 *
...
function actualiza(folder) {
    getImagesFind(folder, $("#buscar_text").val());
}
...
function getImagesFind(folder, find) {
    ...
    //SACAR VARIABLE DE MANERA NO ASYNCRONA
    $.ajax({
        url: 'php/gd.php',
        async: false,
        dataType: 'json',
    ...
    function(data) {
        ...
        if (imagenes.length > 0) {
            ...
            $.each(imagenes, function(i, img) {
                if (img.image != null) {
                    var image = fRight(img.image, img.image.length - dominio.length);
                    var code = "<li class='img_thumb' id='li_" + i + "'>";
                    code += "<div class='thumb_top'><div class='img-titulo ui-widget-header'>" + img.name + "</div>";
                    if (gdexists == "YES") {
                        code += "<img src='" + img.image + "' width='" + img.width + "' height='" + img.height + "' alt='" + img.image + "' >";
                    } else {///nwlib/includes/phpthumb/phpThumb.php?src=" + image + "&amp;w=" + img.width + "
                        code += "<img src='" + img.image + "' width='" + img.width + "' height='" + img.height + "' alt='" + img.image + "'>";
                    }
            ...
            $("li.img_thumb").draggable({
                ...
                if ($("#file_type").val() == "Images") {
                    if ($("#check_mini").is(":checked")) {
                        ok($(this).find("img").attr("src"));
                    } else {
                        ok($(this).find("img").attr("alt"));
                    }
            ...
            $("#thumbs ul").empty();
            var file_description = "imagenes";
            if ($("#file_type").val() == "Flash") {
                file_description = "animaciones en flash"
            }
            var code = "<li class='important_message'>No hay " + file_description + " en esta carpeta";
            code += "<p><a href='#' class='uploadLink'>Suba una nueva</a></p>";
            code += "</li>";
            $(code).appendTo("#thumbs ul");
            prepareLinks();
        }
...
function prepareLinks() {
    ...
    $("#actualiza").click(function() {
        getImagesFind("", $("#buscar_text").val());
    });

    $("#buscar_text").keyup(function(event) {
        if (event.keyCode == 13) {
            $("#buscar_boton").click();
        }
    });
    ...
    $("#buscar_boton").click(function() {
        ...
        if (curFolder != "") {
            getImagesFind(curFolder, $("#buscar_text").val());
        } else {
            getImagesFind("", $("#buscar_text").val());
        }
        $("#buscar_text").focus();
    ...
    $(".uploadLink").click(function() {
        ...
        if (curFolder != "") {
            abrir = window.open('http://' + dominio + '/nwlib/includes/CKImageManager/uploader/init.php?folder=' + curFolder, 'Subir_imagenes', 'width=500,height=400,scrollbars=YES');
        } else {
            abrir = window.open('http://' + dominio + '/nwlib/includes/CKImageManager/uploader/init.php', 'Subir_imagenes', 'width=500,height=400,scrollbars=YES');
        }
...
function prepareDialogs() {
    ...
    $("#promptDialog2").dialog({
        ...
        width: 400,
        draggable: false,
        title: "Borrar carpeta seleccionada",
        resizable: false,
        buttons: {
            "Ok": function() {
...
function moveFile(file, newFolder) {
    ...
    function(data) {
        ...
        if (result.error != 1) {
            getImagesFind($("#current_folder").text(), $("#buscar_text").val());
        } else {
            alert(result.msg);
        }
    },
    ...
    function(data) {
        ...
        if (imagenes.length > 0) {
            ...
            $.each(imagenes, function(i, img) {
                ...
                code += "<a class='detailLink ui-icon ui-icon-zoomin' title='Ver imagen en tamaño real' target='_blank' href='" + imgUrl + "'>View larger</a>";
                code += "<a class='renameLink ui-icon ui-icon-pencil' title='Renombrar' href='" + img.name + "'>Rename image</a>";
                code += "<a class='deleteLink ui-icon ui-icon-trash' title='Borrar' href='" + img.name + "'>Delete image</a></div>";
                code += "<span>" + img.name + "</span></li>";
                $(code).appendTo("#thumbs ul");
