...
(function() {
	var phrases = {
		'progressOverall': 'Progreso total ({total})',
		'currentTitle': 'Archivo en proceso',
		'currentFile': 'Subiendo "{name}"',
		'currentProgress': 'Subir: {bytesLoaded} con {rate}, {timeRemaining} faltan.',
		'fileName': '{name}',
		'remove': 'Eliminar',
		'removeTitle': 'Click para remover esta entrada.',
		'fileError': 'Subida sin éxito',
		'validationErrors': {
			'duplicate': 'El archivo <em>{name}</em> ya está subido, no se pueden subir duplicados.',
			'sizeLimitMin': 'El archivo <em>{name}</em> (<em>{size}</em>) es muy pequeño, el tamaño mínimo es {fileSizeMin}.',
			'sizeLimitMax': 'El archivo <em>{name}</em> (<em>{size}</em>) es muy grande, el tamaño máximo es <em>{fileSizeMax}</em>.',
			'fileListMax': 'El archivo <em>{name}</em> no se puede agregar, algunos <em>{fileListMax} archivos</em> están excedidos.',
			'fileListSizeMax': 'El archivo <em>{name}</em> (<em>{size}</em>) es muy grande, está excedido en <em>{fileListSizeMax}</em> .'
		},
		'errors': {
			'httpStatus': 'El servidor contestó HTTP-Status <code>#{code}</code>',
			'securityError': 'Erro de seguridad ({text})',
			'ioError': 'Fallo al subir o descargar ({text})'
		}
	};
