 ...
 * @license		MIT License
 *
 * @author		Andrés Flórez <mail [at] assders [dot] de>
 * @copyright	Authors
 *
 */
/**
...
if ($error) {
    ...
    if (isset($_POST["folder"]) || $_POST["folder"] != "") {
        ...
        $return['src'] = $_SERVER["DOCUMENT_ROOT"] . '/imagenes' . $_POST["folder"] . "/" . $_FILES['Filedata']['name'];

        // ... and if available, we get image data
        $info = @getimagesize($_FILES['Filedata']['tmp_name']);
        
        if (!move_uploaded_file($_FILES['Filedata']['tmp_name'], $_SERVER["DOCUMENT_ROOT"] . '/imagenes' . $_POST["folder"] . "/" . $_FILES['Filedata']['name'])) {
            $return = array(
                'status' => '0',
                'error' => $error
            );
        ...
        $return['src'] = $_SERVER["DOCUMENT_ROOT"] . '/imagenes/' . $_FILES['Filedata']['name'];

        // ... and if available, we get image data
        $info = @getimagesize($_FILES['Filedata']['tmp_name']);

        if (!move_uploaded_file($_FILES['Filedata']['tmp_name'], $_SERVER["DOCUMENT_ROOT"] . '/imagenes/' . $_FILES['Filedata']['name'])) {
            $return = array(
                'status' => '0',
                'error' => $error
            );
