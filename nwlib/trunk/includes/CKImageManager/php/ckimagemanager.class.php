...
//include($_SERVER['DOCUMENT_ROOT'] . "/soporte/nwlib/utiles.php");
...
class CKImageManager {
    ...
    function getFiles($folder, $file_type) {
        ...
        foreach ($dirs as $d) {
            ...
            if (!is_dir($f) && in_array(strtolower($ext), $exts)) {
                ...
                if ($palabra != "") {
                    $num = strstr(strtolower($d), strtolower($palabra));
                ...
                if ($file_type == "Images") {
                    ...
                    $files_array[] = array(
                        "fileUrl" => $this->fixUrl($this->config["base_url"] . "/" . $folder . "/" . basename($f)),
                        "image" => $this->fixUrl("http://" . $_SERVER["HTTP_HOST"] . "/nwlib/CKImageManager/img/folder.png"),
                        "name" => basename($f),
    ...
    function getExtensions($type) {
        switch ($type) {
            case "Images":
                return array("jpg", "jpeg", "gif", "png", "rar", "zip");
                break;
            case "Flash":
                return array("swf");
                break;
            case "Files":
                return array("bmp", "BMP", "tiff", "mp3", "htm", "rar", "zip", "txt", "doc", "xls", "pdf", "mov", "avi", "flv", "jpg", "jpeg", "gif", "png", "rar", "WMA", "docx", "wma", "pptx", "xlsx", "wmv", "WMV");
        }
    }
    ...
    function deleteFolderTree($folder) {
        ...
        foreach (scandir($path) as $item) {
            if ($item == '.' || $item == '..')
                continue;
            if (!$this->deleteFolderTree($folder . DIRECTORY_SEPARATOR . $item))
                return false;
        }
