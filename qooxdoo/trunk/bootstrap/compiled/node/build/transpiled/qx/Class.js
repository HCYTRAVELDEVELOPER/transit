  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.Class", {
    statics: {
       ...
       * should be included.
       */
      __P_60_0: true ? qx.core.Property : null,
      ...
      */
      ...
      define(name, config) {
        try {
          return this.__P_60_1(name, config);
        } catch (ex) {
      ...
      __P_60_1(name, config) {
        ...
        // Create the class
        var clazz = this.__P_60_2(name, config.type, config.extend, config.statics, config.construct, config.destruct, config.include); // Initialise class and constructor/destructor annotations


        ["@", "@construct", "@destruct"].forEach(function (id) {
          this.__P_60_3(clazz, id, null, config[id]);
        }, this); // Members, properties, events and mixins are only allowed for non-static classes

        if (config.extend) {
          // Attach properties
          if (config.properties) {
            this.__P_60_4(clazz, config.properties, true);
          } // Attach members


          if (config.members) {
            this.__P_60_5(clazz, config.members, true, true, false);
          } // Process events


          if (config.events) {
            this.__P_60_6(clazz, config.events, true);
          } // Include mixins
          ...
          if (config.include) {
            for (var i = 0, l = config.include.length; i < l; i++) {
              this.__P_60_7(clazz, config.include[i], false);
            }
          }
        ...
        if (config.implement) {
          for (var i = 0, l = config.implement.length; i < l; i++) {
            this.__P_60_8(clazz, config.implement[i]);
          }
        }
          ...
          qx.Bootstrap.addPendingDefer(clazz, function () {
            ...
            config.defer(clazz, clazz.prototype, {
              add(name, config) {
                ...
                qx.Class.__P_60_4(clazz, properties, true);
              }
       ...
       */
      include(clazz, mixin) {
        qx.Class.__P_60_7(clazz, mixin, false);
      },
       ...
       */
      patch(clazz, mixin) {
        qx.Class.__P_60_7(clazz, mixin, true);
      ...
      */
      ...
      __P_60_9: qx.core.Environment.select("qx.debug", {
        true: {
          "@": "object",
          "@construct": "object",
          "@destruct": "object",
      ...
      __P_60_10: qx.core.Environment.select("qx.debug", {
        true: {
          "@": "object",
          type: "string",
          // String
       ...
       */
      __P_60_11: qx.core.Environment.select("qx.debug", {
        true(name, config) {
          // Validate type
          ...
          var allowed = config.type === "static" ? this.__P_60_10 : this.__P_60_9;
       ...
       */
      __P_60_12: qx.core.Environment.select("qx.debug", {
        true(clazz) {
       ...
       */
      __P_60_2(name, type, extend, statics, construct, destruct, mixins) {
        ...
        if (!extend && true) {
          ...
          if (extend) {
            ...
            if (!construct) {
              construct = this.__P_60_13();
            }

            clazz = this.__P_60_14(construct, name, type); // Add singleton getInstance()
          ...
          if (statics) {
            ...
            for (var i = 0, a = Object.keys(statics), l = a.length; i < l; i++) {
              ...
              this.__P_60_3(clazz, "statics", key, statics["@" + key]);
            }
          }
        } // Create namespace
      ...
      */
       ...
       */
      __P_60_4(clazz, properties, patch) {
        ...
        for (var name in properties) {
          ...
          if (config.event !== undefined) {
            ...
            this.__P_60_6(clazz, event, patch);
          } // Remember inheritable properties


          if (config.inheritable) {
            this.__P_60_0.$$inheritable[name] = true;

            if (!proto.$$refreshInheritables) {
              this.__P_60_0.attachRefreshInheritables(clazz);
            }
          }

          if (!config.refine) {
            this.__P_60_0.attachMethods(clazz, name, config);
          } // Add annotations


          this.__P_60_3(clazz, "properties", name, config["@"]);
        }
      },
       ...
       */
      __P_60_15: qx.core.Environment.select("qx.debug", {
        true(clazz, name, config, patch) {
          ...
          var allowed = config.group ? this.__P_60_0.$$allowedGroupKeys : this.__P_60_0.$$allowedKeys;
       ...
       */
      __P_60_5(clazz, members, patch, base, wrap) {
        var proto = clazz.prototype;
        var key, member;
        ...
        for (var i = 0, a = Object.keys(members), l = a.length; i < l; i++) {
          ...
          if (key.charAt(0) === "@") {
            ...
            if (members[annoKey] === undefined) {
              this.__P_60_3(clazz, "members", annoKey, members[key]);
            }
          ...
          this.__P_60_3(clazz, "members", key, members["@" + key]);
        }
      },
