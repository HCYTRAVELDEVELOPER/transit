    ...
    members: {
      ...
      async run(application) {
        ...
        await new Promise((resolve, reject) => {
          var time = new Date();
          this.__P_40_2 = fs.createWriteStream(path.join(this.__P_40_0.getOutputDir(), this.getOutputTo()));
          this.write(`// Generated declaration file at ${time}\n`);
          this.writeBase().then(async () => {
            var analyser = this.__P_40_0.getAnalyser();

            this.__P_40_3 = new qx.tool.utils.IndexedArray();

            if (application) {
              application.getDependencies().forEach(classname => {
                if (classname != "q" && classname != "qxWeb") {
                  this.__P_40_3.push(classname);
                }
              });
            } else {
            ...
            var next = () => {
              if (classIndex >= this.__P_40_3.getLength()) {
                return undefined;
              }

              var className = this.__P_40_3.getItem(classIndex++);
      ...
      loadAPIFile(classname) {
        ...
        if (this.__P_40_4[classname]) {
          return Promise.resolve(this.__P_40_4[classname]);
        }

        var fileName = path.join(this.__P_40_0.getOutputDir(), "transpiled", classname.replace(/\./g, "/") + ".json");
        return readFile(fileName, "UTF-8").then(content => this.__P_40_4[classname] = JSON.parse(content)).catch(err => qx.tool.compiler.Console.error("Error parsing " + classname + ": " + err.stack));
      },
      ...
      writeConstructor(methodMeta) {
        this.write(this.__P_40_1 + "constructor (" + this.serializeParameters(methodMeta, true) + ");\n");
      },
      ...
      writeMethods(methods, classMeta, isStatic = false) {
        ...
        var IGNORE = qx.tool.compiler.targets.TypeScriptWriter.IGNORE[this.__P_40_5.className];
        ...
        for (var name in methods) {
          ...
          if (methodMeta.type == "function") {
            ...
            if (hasDescription) {
              this.write(this.__P_40_1 + "/**\n");
              methodMeta.jsdoc["@description"][0].body.split("\n").forEach(line => {
                this.write(this.__P_40_1 + " * " + line + "\n");
              });
              this.write(this.__P_40_1 + " */\n");
            }

            this.write(this.__P_40_1 + (hideMethod ? "// " : "") + decl + ";" + comment + "\n");
          }
        }
      },
      ...
      async writeClass(meta) {
        this.__P_40_5 = meta; // qx.tool.compiler.Console.info("Processing class " + meta.packageName + "." + meta.name);
