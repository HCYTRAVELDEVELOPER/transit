  ...
  qx.Class.define("qx.tool.compiler.resources.AbstractMatcher", {
    ...
    construct(match) {
      ...
      if (match) {
        if (qx.lang.Type.isArray(match)) {
          ...
          this.__P_34_0 = filename => match.some(elem => filename.endsWith(elem));
        } else if (typeof match == "string") {
          if (match[0] != ".") {
            match = "." + match;
          }

          this.__P_34_0 = filename => filename.endsWith(match);
        } else {
          this.__P_34_0 = filename => match.test(filename);
        }
      }
    },
    ...
    members: {
      ...
      matches(filename, library) {
        return this.__P_34_0 !== null && this.__P_34_0(filename);
      }
