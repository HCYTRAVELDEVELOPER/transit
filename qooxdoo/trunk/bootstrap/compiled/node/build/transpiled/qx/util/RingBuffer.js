  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.util.RingBuffer", {
    ...
    members: {
       ...
       */
      addEntry(entry) {
        this.__P_92_4[this.__P_92_0] = entry;
        this.__P_92_0 = this.__P_92_6(this.__P_92_0, 1); //Count # of stored entries

        var max = this.getMaxEntries();

        if (this.__P_92_1 < max) {
          this.__P_92_1++;
        } //Count # of stored elements since last mark call


        if (this.__P_92_2 && this.__P_92_3 < max) {
          this.__P_92_3++;
        }
      },
      ...
      getEntries(count, startingFromMark) {
        ...
        if (count > this.__P_92_1) {
          count = this.__P_92_1;
        } // Trim count so it does not exceed last call to mark (if mark was called
        // and startingFromMark was true)


        if (startingFromMark && this.__P_92_2 && count > this.__P_92_3) {
          count = this.__P_92_3;
        }

        if (count > 0) {
          var indexOfYoungestElementInHistory = this.__P_92_6(this.__P_92_0, -1);

          var startIndex = this.__P_92_6(indexOfYoungestElementInHistory, -count + 1);

          var result;

          if (startIndex <= indexOfYoungestElementInHistory) {
            //Requested segment not wrapping around ringbuffer boundary, get in one run
            result = this.__P_92_4.slice(startIndex, indexOfYoungestElementInHistory + 1);
          } else {
            //Requested segment wrapping around ringbuffer boundary, get two parts & concat
            result = this.__P_92_4.slice(startIndex, this.__P_92_1).concat(this.__P_92_4.slice(0, indexOfYoungestElementInHistory + 1));
          }
        } else {
          result = [];
        }
       ...
       */
      clear() {
        this.__P_92_4 = new Array(this.getMaxEntries());
        this.__P_92_1 = 0;
        this.__P_92_3 = 0;
        this.__P_92_0 = 0;
      },
