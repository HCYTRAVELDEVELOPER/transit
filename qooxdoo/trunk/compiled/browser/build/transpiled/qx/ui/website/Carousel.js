  ...
  qx.Bootstrap.define("qx.ui.website.Carousel", {
    ...
    members: {
      ...
      init: function init() {
        ...
        this.__P_15_2 = qxWeb.create("<div>").addClass(prefix + "-container").appendTo(this);
        this.__P_15_1 = qxWeb.create("<div>").addClass("qx-hbox").setStyle("height", "100%").appendTo(this.__P_15_2);
        this.__P_15_3 = [];
        this.__P_15_5 = qxWeb.create("<div>").addClasses([prefix + "-pagination", "qx-hbox", "qx-flex1"]).setStyle("visibility", "excluded").appendTo(this);

        if (this._ie9) {
          this.__P_15_1.setStyle("display", "table");

          this.__P_15_5.setStyle("textAlign", "center");
        } else {
      ...
      addPage: function addPage(child) {
        child.addClasses(["qx-flex1", this.getCssPrefix() + "-page"]).appendTo(this.__P_15_1);

        if (this.find("." + this.getCssPrefix() + "-page").length > this.__P_15_3.length) {
          var paginationLabel = this._createPaginationLabel();

          this.__P_15_3.push(paginationLabel);

          this.__P_15_5.append(paginationLabel);
        }
        ...
        if (this._getPages().length === 3 && !this._ie9) {
          this.__P_15_2.translate([-this.getWidth() + "px", 0, 0]);
        }
      ...
      removePage: function removePage(child) {
        ...
        this.__P_15_3.splice(child.priorPosition, 1)[0].remove();

        for (var i = 0; i < this.__P_15_3.length; i++) {
          this.__P_15_3[i].getChildren(".label").setHtml(i + 1 + "");
        }
      ...
      _update: function _update() {
        ...
        if (!this._ie9) {
          ...
          if (direction == "right") {
            left = this._getPositionLeft() - this.__P_15_2.getWidth();
          ...
          if (left !== undefined) {
            ...
            this.__P_15_2.translate([-left + "px", 0, 0]); // animate to the new page
      ...
      _updateWidth: function _updateWidth() {
        ...
        this.__P_15_1.setStyle("width", containerWidth + "px"); // set the width of all pages to the carousel width
      ...
      _onTrackStart: function _onTrackStart() {
        ...
        this.__P_15_4 = this._getPositionLeft();

        this.__P_15_2 // stop the current scroll animation
        .stop() // correct the scroll position as the stopped animation
        // resets to its initial value
        .translate([-Math.round(this.__P_15_4) + "px", 0, 0]);
      },
      ...
      _onTrack: function _onTrack(e) {
        ...
        if (e.delta.axis == "x" && this._getPages().length > 2) {
          this.__P_15_2.translate([-(this.__P_15_4 - e.delta.x) + "px", 0, 0]);
        }
      },
      ...
      _onTrackEnd: function _onTrackEnd() {
        if (this.__P_15_4 == null || this.__P_15_6) {
        ...
        window.setTimeout(function () {
          if (this._getPages().length < 3 || this.__P_15_2.isPlaying()) {
            return;
          }
      ...
      _onPaginationLabelTap: function _onPaginationLabelTap(e) {
        this.__P_15_3.forEach(function (label, index) {
          if (label[0] === e.currentTarget) {
            ...
            this.__P_15_2.translate([-activeIndex * this.getWidth() + "px", 0, 0]);

            this.__P_15_6 = true; // animate to the desired page

            this._translateTo((activeIndex + distance) * this.getWidth());

            this.__P_15_2.once("animationEnd", function (page) {
              this.__P_15_6 = false; // set the viewport back to the default position
      ...
      _updatePagination: function _updatePagination() {
        ...
        this._getPages().length < 2 ? this.__P_15_5.setStyle("visibility", "excluded") : this.__P_15_5.setStyle("visibility", "visible");

        this.__P_15_5.find("." + this.getCssPrefix() + "-pagination-label").removeClass("active");

        var pages = this._getPages();

        this.__P_15_3[pages.indexOf(this.getActive())].addClass("active");
      },
      ...
      _getPages: function _getPages() {
        return this.__P_15_1.find("." + this.getCssPrefix() + "-page");
      },
      ...
      _getPositionLeft: function _getPositionLeft() {
        var containerRect = this.__P_15_2[0].getBoundingClientRect();
...
//# sourceMappingURL=Carousel.js.map?dt=1658886776352