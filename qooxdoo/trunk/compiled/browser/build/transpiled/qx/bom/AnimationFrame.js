  ...
  qx.Bootstrap.define("qx.bom.AnimationFrame", {
    ...
    members: {
      ...
      startSequence: function startSequence(duration) {
        this.__P_71_0 = false;
        var start = window.performance && performance.now ? performance.now() + qx.bom.AnimationFrame.__P_71_1 : Date.now();
    ...
    statics: {
      ...
      request: function request(callback, context) {
        ...
        var cb = function cb(time) {
          ...
          if (time < 1e10) {
            time = qx.bom.AnimationFrame.__P_71_1 + time;
          }
    ...
    defer: function defer(statics) {
      ...
      statics.__P_71_1 = window.performance && performance.timing && performance.timing.navigationStart; // if not, simply use the current time

      if (!statics.__P_71_1) {
        statics.__P_71_1 = Date.now();
      }
    }
  });
...
//# sourceMappingURL=AnimationFrame.js.map?dt=1658886795613