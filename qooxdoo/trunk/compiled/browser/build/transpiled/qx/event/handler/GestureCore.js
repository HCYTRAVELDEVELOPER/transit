  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.event.handler.GestureCore", {
    ...
    construct: function construct(target, emitter) {
      this.__P_50_0 = target;
      this.__P_50_1 = emitter;
      this.__P_50_2 = {};
      this.__P_50_3 = {};
      this.__P_50_4 = {};
      this.__P_50_5 = {};
    ...
    members: {
      ...
      _initObserver: function _initObserver() {
        qx.event.handler.GestureCore.GESTURE_EVENTS.forEach(function (gestureType) {
          qxWeb(this.__P_50_0).on(gestureType, this.checkAndFireGesture, this);
        }.bind(this));

        if (qx.core.Environment.get("engine.name") == "mshtml" && qx.core.Environment.get("browser.documentmode") < 9) {
          qxWeb(this.__P_50_0).on("dblclick", this._onDblClick, this);
      ...
      _stopObserver: function _stopObserver() {
        qx.event.handler.GestureCore.GESTURE_EVENTS.forEach(function (pointerType) {
          qxWeb(this.__P_50_0).off(pointerType, this.checkAndFireGesture, this);
        }.bind(this));

        if (qx.core.Environment.get("engine.name") == "mshtml" && qx.core.Environment.get("browser.documentmode") < 9) {
          qxWeb(this.__P_50_0).off("dblclick", this._onDblClick, this);
        }
      ...
      gestureBegin: function gestureBegin(domEvent, target) {
        if (this.__P_50_2[domEvent.pointerId]) {
          this.__P_50_17(this.__P_50_2[domEvent.pointerId]);

          delete this.__P_50_2[domEvent.pointerId];
        }
        /*
        ...
        this.__P_50_2[domEvent.pointerId] = {
          ...
          longTapTimer: window.setTimeout(this.__P_50_18.bind(this, domEvent, target), qx.event.handler.GestureCore.LONGTAP_TIME)
        };

        if (domEvent.isPrimary) {
          this.__P_50_9 = false;
          this.__P_50_8 = target;

          this.__P_50_19("trackstart", domEvent, target);
        } else {
          this.__P_50_9 = true;

          if (Object.keys(this.__P_50_2).length === 2) {
            this.__P_50_10 = this._calcAngle();
            this.__P_50_12 = this._calcDistance();
          }
        }
      },
      ...
      gestureMove: function gestureMove(domEvent, target) {
        var gesture = this.__P_50_2[domEvent.pointerId];
        ...
        if (gesture) {
          ...
          if (Object.keys(this.__P_50_2).length === 2) {
            this.__P_50_20(domEvent, gesture.target);

            this.__P_50_21(domEvent, gesture.target);
          }

          if (!this.__P_50_9) {
            this.__P_50_19("track", domEvent, gesture.target);
      ...
      _hasIntermediaryHandler: function _hasIntermediaryHandler(target) {
        while (target && target !== this.__P_50_0) {
      ...
      gestureFinish: function gestureFinish(domEvent, target) {
        ...
        var gesture = this.__P_50_2[domEvent.pointerId]; // delete the long tap
        ...
        this.__P_50_22(gesture.velocityX, gesture.velocityY, domEvent, gesture.target);

        this.__P_50_19("trackend", domEvent, gesture.target);
        ...
        if (gesture.isTap) {
          ...
          if (Object.keys(this.__P_50_3).length > 0) {
            // delete old tap entries
            var limit = Date.now() - qx.event.handler.GestureCore.DOUBLETAP_TIME;

            for (var time in this.__P_50_3) {
              if (time < limit) {
                delete this.__P_50_3[time];
              } else {
                var lastTap = this.__P_50_3[time];

                var isBelowDoubleTapDistance = this.__P_50_23(lastTap.x, lastTap.y, domEvent.clientX, domEvent.clientY, domEvent.getPointerType());
          ...
          var swipe = this.__P_50_24(domEvent, target);
      ...
      gestureCancel: function gestureCancel(id) {
        if (this.__P_50_2[id]) {
          this.__P_50_17(this.__P_50_2[id]);

          delete this.__P_50_2[id];
        }

        if (this.__P_50_5[id]) {
          this.stopMomentum(this.__P_50_5[id]);
          delete this.__P_50_5[id];
        }
      },
      ...
      updateGestureTarget: function updateGestureTarget(id, target) {
        this.__P_50_2[id].target = target;
      },
       ...
       */
      __P_50_22: function __P_50_22(deltaX, deltaY, domEvent, target, time) {
        var oldTimeoutId = domEvent.timeoutId;

        if (!time && this.__P_50_5[domEvent.pointerId]) {
          // new roll impulse started, stop the old one
          this.stopMomentum(this.__P_50_5[domEvent.pointerId]);
        } // do nothing if we don't need to scroll


        if (Math.abs(deltaY) < 1 && Math.abs(deltaX) < 1 || this.__P_50_4[oldTimeoutId] || !this.getWindow()) {
          delete this.__P_50_4[oldTimeoutId];
          delete this.__P_50_5[domEvent.pointerId];
          return;
        }
        ...
        var timeoutId = qx.bom.AnimationFrame.request(qx.lang.Function.bind(function (deltaX, deltaY, domEvent, target, time) {
          this.__P_50_22(deltaX, deltaY, domEvent, target, time);
        ...
        this.__P_50_5[domEvent.pointerId] = timeoutId;
      ...
      _calcAngle: function _calcAngle() {
        ...
        for (var pointerId in this.__P_50_2) {
          var gesture = this.__P_50_2[pointerId];
       ...
       */
      __P_50_23: function __P_50_23(x1, y1, x2, y2, type) {
      ...
      _fireEvent: function _fireEvent(domEvent, type, target) {
        ...
        if (qx.core.Environment.get("event.dispatchevent")) {
        ...
        } else if (this.__P_50_1) {
          evt = new qx.event.type.dom.Custom(type, domEvent, {
            target: this.__P_50_0,
            currentTarget: this.__P_50_0,
            srcElement: this.__P_50_0,
            swipe: domEvent.swipe,
            scale: domEvent.scale,
            angle: domEvent.angle,
            delta: domEvent.delta,
          ...
          this.__P_50_1.emit(type, domEvent);
        }
      },
       ...
       */
      __P_50_24: function __P_50_24(domEvent, target) {
        var gesture = this.__P_50_2[domEvent.pointerId];
       ...
       * @param target {Element} event target
       */
      __P_50_19: function __P_50_19(type, domEvent, target) {
       ...
       */
      __P_50_25: function __P_50_25(domEvent, target, rollFactor) {
        domEvent.delta = {
       ...
       * @param target {Element} event target
       */
      __P_50_26: function __P_50_26(target) {
        ...
        this.__P_50_16 = new Date().getTime();
        var reLength = this.__P_50_6.length;

        for (var i = 0; i < reLength; i++) {
          var domEvent = this.__P_50_6[i];

          this.__P_50_25(domEvent, target, rollFactor);
        }
       ...
       */
      __P_50_27: function __P_50_27() {
        if (this.__P_50_6.length > qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_THRESHOLD) {
      ...
      _fireRoll: function _fireRoll(domEvent, type, target) {
        ...
        if (domEvent.type === qx.core.Environment.get("event.mousewheel").type) {
          if (this._isTouchPadDetectionEnabled()) {
            ...
            if (this.__P_50_16 > 0 && now - this.__P_50_16 > detectionTimeout) {
              // The detection timeout was reached. A new detection step should occur.
              this.__P_50_15 = false;
              this.__P_50_6 = [];
              this.__P_50_16 = 0;
            }
              ...
              if (this.__P_50_6.length === 0) {
                ...
                qx.event.Timer.once(function () {
                  if (!this.__P_50_15) {
                    ...
                    this.__P_50_26(target);
                  }
                }, this, qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_PERIOD + 50);
              }

              this.__P_50_6.push(domEvent);

              this.__P_50_14++;

              if (now - this.__P_50_13 > qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_PERIOD) {
                this.__P_50_27();
              }
            }
            ...
            this.__P_50_25(domEvent, target, qx.event.handler.GestureCore.ROLL_FACTOR);
          }
        } else {
          var gesture = this.__P_50_2[domEvent.pointerId];
          domEvent.delta = {
       ...
       * @param target {Element} event target
       */
      __P_50_20: function __P_50_20(domEvent, target) {
        if (!domEvent.isPrimary) {
          var angle = this._calcAngle();

          domEvent.angle = Math.round((angle - this.__P_50_10) % 360);

          this._fireEvent(domEvent, "rotate", this.__P_50_8);
        }
      },
       ...
       * @param target {Element} event target
       */
      __P_50_21: function __P_50_21(domEvent, target) {
        if (!domEvent.isPrimary) {
          var distance = this._calcDistance();

          var scale = distance / this.__P_50_12;
          domEvent.scale = Math.round(scale * 100) / 100;

          this._fireEvent(domEvent, "pinch", this.__P_50_8);
        }
      },
       ...
       * @param target {Element} event target
       */
      __P_50_18: function __P_50_18(domEvent, target) {
        var gesture = this.__P_50_2[domEvent.pointerId];
       ...
       */
      __P_50_17: function __P_50_17(gesture) {
      ...
      dispose: function dispose() {
        for (var gesture in this.__P_50_2) {
          this.__P_50_17(gesture);
        }

        this._stopObserver();

        this.__P_50_0 = this.__P_50_1 = null;
      }
    }
  });
...
//# sourceMappingURL=GestureCore.js.map?dt=1658886784230