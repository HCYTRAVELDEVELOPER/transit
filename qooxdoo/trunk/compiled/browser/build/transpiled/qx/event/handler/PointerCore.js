  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.event.handler.PointerCore", {
    ...
    construct: function construct(target, emitter) {
      this.__P_52_1 = target;
      this.__P_52_2 = emitter;
      this.__P_52_3 = [];
      this.__P_52_4 = [];
      this.__P_52_5 = [];
      ...
      if (engineName == "mshtml" && docMode == 10) {
        ...
        this.__P_52_3 = ["MSPointerDown", "MSPointerMove", "MSPointerUp", "MSPointerCancel", "MSPointerOver", "MSPointerOut", "pointerdown", "pointermove", "pointerup", "pointercancel", "pointerover", "pointerout"];
        ...
        this.__P_52_3 = ["pointerdown", "pointermove", "pointerup", "pointercancel", "pointerover", "pointerout"];
      ...
      if (!qx.core.Environment.get("event.mspointer")) {
        if (qx.core.Environment.get("device.touch")) {
          this.__P_52_3 = ["touchstart", "touchend", "touchmove", "touchcancel"];

          this._initObserver(this._onTouchEvent);
        }

        this.__P_52_3 = ["mousedown", "mouseup", "mousemove", "mouseover", "mouseout", "contextmenu"];
    ...
    members: {
      ...
      _initObserver: function _initObserver(callback, useEmitter) {
        this.__P_52_7 = qx.lang.Function.listener(callback, this);

        this.__P_52_3.forEach(function (type) {
          if (useEmitter && qx.dom.Node.isDocument(this.__P_52_1)) {
            if (!this.__P_52_1.$$emitter) {
              this.__P_52_1.$$emitter = new qx.event.Emitter();
            }

            this.__P_52_1.$$emitter.on(type, this.__P_52_7);
          } else {
            qx.bom.Event.addNativeListener(this.__P_52_1, type, this.__P_52_7);
          }
        }.bind(this));
      },
      ...
      _onTouchEvent: function _onTouchEvent(domEvent) {
        ...
        if (domEvent.touches.length < this.__P_52_5.length) {
          // Firing pointer cancel for previously active touches.
          for (var i = this.__P_52_5.length - 1; i >= 0; i--) {
            var cancelEvent = new qx.event.type.dom.Pointer("pointercancel", domEvent, {
              identifier: this.__P_52_5[i].identifier,
              target: domEvent.target,
              pointerType: "touch",
              pointerId: this.__P_52_5[i].identifier + 2
            });
          ...
          this.__P_52_9 = null; // cleanup of active touches array.

          this.__P_52_5 = []; // Do nothing after pointer cancel.

          return;
        }

        if (domEvent.type == "touchstart" && this.__P_52_9 === null) {
          this.__P_52_9 = changedTouches[0].identifier;
        }
        ...
        for (var i = 0, l = changedTouches.length; i < l; i++) {
          ...
          if (domEvent.type == "touchend" || domEvent.type == "touchcancel") {
            ...
            if (this.__P_52_9 == touch.identifier) {
              this.__P_52_9 = null;
            }
          }
        }
      ...
      _determineActiveTouches: function _determineActiveTouches(type, changedTouches) {
        if (type == "touchstart") {
          for (var i = 0; i < changedTouches.length; i++) {
            this.__P_52_5.push(changedTouches[i]);
          }
        } else if (type == "touchend" || type == "touchcancel") {
          var updatedActiveTouches = [];

          for (var i = 0; i < this.__P_52_5.length; i++) {
            var add = true;

            for (var j = 0; j < changedTouches.length; j++) {
              if (this.__P_52_5[i].identifier == changedTouches[j].identifier) {
                add = false;
                break;
              }
            }
            ...
            if (add) {
              updatedActiveTouches.push(this.__P_52_5[i]);
            }
          }

          this.__P_52_5 = updatedActiveTouches;
        }
      },
      ...
      _isSimulatedMouseEvent: function _isSimulatedMouseEvent(x, y) {
        var touch = qx.event.handler.PointerCore.__P_52_0;

        if (touch) {
          var timeSinceTouch = new Date().getTime() - touch.time;
          var dist = qx.event.handler.PointerCore.SIM_MOUSE_DISTANCE;
          var distX = Math.abs(x - qx.event.handler.PointerCore.__P_52_0.x);
          var distY = Math.abs(y - qx.event.handler.PointerCore.__P_52_0.y);
      ...
      _stopObserver: function _stopObserver() {
        for (var i = 0; i < this.__P_52_3.length; i++) {
          qx.bom.Event.removeNativeListener(this.__P_52_1, this.__P_52_3[i], this.__P_52_7);
        }
      },
      ...
      dispose: function dispose() {
        ...
        this.__P_52_1 = this.__P_52_2 = null;
      }
    }
  });
...
//# sourceMappingURL=PointerCore.js.map?dt=1658886784680