  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.dev.FakeServer", {
    ...
    members: {
      ...
      configure: function configure(responseData) {
        responseData.forEach(function (item) {
          ...
          for (var i = 0, l = this.__P_49_0.length; i < l; i++) {
            var old = this.__P_49_0[i];
            hasResponse = old[0] == response[0] && old[1] == response[1];
          }

          if (!hasResponse) {
            this.__P_49_0.push(response);
          }

          this.respondWith(item.method, urlRegExp, item.response);
        }.bind(this));

        var filter = this.__P_49_3 = this.__P_49_4();
      ...
      addFilter: function addFilter(filter) {
        this.__P_49_1.FakeXMLHttpRequest.addFilter(filter);
      },
      ...
      removeFilter: function removeFilter(filter) {
        qx.lang.Array.remove(this.__P_49_1.FakeXMLHttpRequest.filters, filter);
      },
      ...
      removeResponse: function removeResponse(method, url) {
        qx.lang.Array.remove(this.__P_49_1.FakeXMLHttpRequest.filters, this.__P_49_3);
        var urlRegExp = url instanceof RegExp ? url : this._getRegExp(url);
        this.__P_49_0 = this.__P_49_0.filter(function (response) {
          return response[0] != method || response[1].toString() != urlRegExp.toString();
        });
        this.__P_49_2.responses = this.__P_49_2.responses.filter(function (response) {
          return response.method != method || response.url.toString() != urlRegExp.toString();
        });
        this.removeFilter(this.__P_49_3);
        this.__P_49_3 = this.__P_49_4();
        this.addFilter(this.__P_49_3);
      },
      ...
      getFakeServer: function getFakeServer() {
        if (!this.__P_49_2) {
          var sinon = this.__P_49_1 = qx.dev.unit.Sinon.getSinon();
          sinon.FakeXMLHttpRequest.useFilters = true;
          this.__P_49_2 = sinon.sandbox.useFakeServer();
          this.__P_49_2.autoRespond = true;
        }
       ...
       */
      __P_49_4: function __P_49_4() {
        var responses = this.__P_49_0;
    ...
    destruct: function destruct() {
      this.restore();
      this.__P_49_2 = this.__P_49_1 = null;
    }
  });
  qx.dev.FakeServer.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=FakeServer.js.map?dt=1658886783941