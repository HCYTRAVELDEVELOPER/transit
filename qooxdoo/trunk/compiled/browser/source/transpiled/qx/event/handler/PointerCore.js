  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.event.handler.PointerCore", {
    ...
    construct(target, emitter) {
      this.__defaultTarget__P_52_1 = target;
      this.__emitter__P_52_2 = emitter;
      this.__eventNames__P_52_3 = [];
      this.__buttonStates__P_52_4 = [];
      this.__activeTouches__P_52_5 = [];
      ...
      if (engineName == "mshtml" && docMode == 10) {
        ...
        this.__eventNames__P_52_3 = ["MSPointerDown", "MSPointerMove", "MSPointerUp", "MSPointerCancel", "MSPointerOver", "MSPointerOut", "pointerdown", "pointermove", "pointerup", "pointercancel", "pointerover", "pointerout"];

        this._initPointerObserver();
      } else {
        if (qx.core.Environment.get("event.mspointer")) {
          this.__nativePointerEvents__P_52_6 = true;
        }

        this.__eventNames__P_52_3 = ["pointerdown", "pointermove", "pointerup", "pointercancel", "pointerover", "pointerout"];
      ...
      if (!qx.core.Environment.get("event.mspointer")) {
        if (qx.core.Environment.get("device.touch")) {
          this.__eventNames__P_52_3 = ["touchstart", "touchend", "touchmove", "touchcancel"];

          this._initObserver(this._onTouchEvent);
        }

        this.__eventNames__P_52_3 = ["mousedown", "mouseup", "mousemove", "mouseover", "mouseout", "contextmenu"];
    ...
    members: {
      __defaultTarget__P_52_1: null,
      __emitter__P_52_2: null,
      __eventNames__P_52_3: null,
      __nativePointerEvents__P_52_6: false,
      __wrappedListener__P_52_7: null,
      __lastButtonState__P_52_8: 0,
      __buttonStates__P_52_4: null,
      __primaryIdentifier__P_52_9: null,
      __activeTouches__P_52_5: null,
      _processedFlag: null,
      ...
      _initObserver(callback, useEmitter) {
        this.__wrappedListener__P_52_7 = qx.lang.Function.listener(callback, this);

        this.__eventNames__P_52_3.forEach(function (type) {
          if (useEmitter && qx.dom.Node.isDocument(this.__defaultTarget__P_52_1)) {
            if (!this.__defaultTarget__P_52_1.$$emitter) {
              this.__defaultTarget__P_52_1.$$emitter = new qx.event.Emitter();
            }

            this.__defaultTarget__P_52_1.$$emitter.on(type, this.__wrappedListener__P_52_7);
          } else {
            qx.bom.Event.addNativeListener(this.__defaultTarget__P_52_1, type, this.__wrappedListener__P_52_7);
          }
        }.bind(this));
      },
      ...
      _onPointerEvent(domEvent) {
        ...
        if (!this.__nativePointerEvents__P_52_6) {
      ...
      _onTouchEvent(domEvent) {
        ...
        if (domEvent.touches.length < this.__activeTouches__P_52_5.length) {
          // Firing pointer cancel for previously active touches.
          for (var i = this.__activeTouches__P_52_5.length - 1; i >= 0; i--) {
            var cancelEvent = new qx.event.type.dom.Pointer("pointercancel", domEvent, {
              identifier: this.__activeTouches__P_52_5[i].identifier,
              target: domEvent.target,
              pointerType: "touch",
              pointerId: this.__activeTouches__P_52_5[i].identifier + 2
            });
          ...
          this.__primaryIdentifier__P_52_9 = null; // cleanup of active touches array.

          this.__activeTouches__P_52_5 = []; // Do nothing after pointer cancel.

          return;
        }

        if (domEvent.type == "touchstart" && this.__primaryIdentifier__P_52_9 === null) {
          this.__primaryIdentifier__P_52_9 = changedTouches[0].identifier;
        }
        ...
        for (var i = 0, l = changedTouches.length; i < l; i++) {
          ...
          if (domEvent.type == "touchend" || domEvent.type == "touchcancel") {
            ...
            if (this.__primaryIdentifier__P_52_9 == touch.identifier) {
              this.__primaryIdentifier__P_52_9 = null;
            }
          }
        }
      },
      ...
      _onMouseEvent(domEvent) {
        ...
        if (domEvent.type == "mousedown") {
          this.__buttonStates__P_52_4[domEvent.which] = 1;
        } else if (domEvent.type == "mouseup") {
          if (qx.core.Environment.get("os.name") == "osx" && qx.core.Environment.get("engine.name") == "gecko") {
            if (this.__buttonStates__P_52_4[domEvent.which] != 1 && domEvent.ctrlKey) {
              this.__buttonStates__P_52_4[1] = 0;
            }
          }

          this.__buttonStates__P_52_4[domEvent.which] = 0;
        }

        var type = qx.event.handler.PointerCore.MOUSE_TO_POINTER_MAPPING[domEvent.type];
        var target = qx.bom.Event.getTarget(domEvent);
        var buttonsPressed = qx.lang.Array.sum(this.__buttonStates__P_52_4);
        var mouseProps = {
          pointerType: "mouse",
          pointerId: 1
        }; // if the button state changes but not from or to zero

        if (this.__lastButtonState__P_52_8 != buttonsPressed && buttonsPressed !== 0 && this.__lastButtonState__P_52_8 !== 0) {
          var moveEvt = new qx.event.type.dom.Pointer("pointermove", domEvent, mouseProps);

          this._fireEvent(moveEvt, "pointermove", target);
        }

        this.__lastButtonState__P_52_8 = buttonsPressed; // pointerdown should only trigger form the first pressed button.
      ...
      _determineActiveTouches(type, changedTouches) {
        if (type == "touchstart") {
          for (var i = 0; i < changedTouches.length; i++) {
            this.__activeTouches__P_52_5.push(changedTouches[i]);
          }
        } else if (type == "touchend" || type == "touchcancel") {
          var updatedActiveTouches = [];

          for (var i = 0; i < this.__activeTouches__P_52_5.length; i++) {
            var add = true;

            for (var j = 0; j < changedTouches.length; j++) {
              if (this.__activeTouches__P_52_5[i].identifier == changedTouches[j].identifier) {
                add = false;
                break;
              }
            }
            ...
            if (add) {
              updatedActiveTouches.push(this.__activeTouches__P_52_5[i]);
            }
          }

          this.__activeTouches__P_52_5 = updatedActiveTouches;
        }
      },
      ...
      _isSimulatedMouseEvent(x, y) {
        var touch = qx.event.handler.PointerCore.__lastTouch__P_52_0;

        if (touch) {
          var timeSinceTouch = new Date().getTime() - touch.time;
          var dist = qx.event.handler.PointerCore.SIM_MOUSE_DISTANCE;
          var distX = Math.abs(x - qx.event.handler.PointerCore.__lastTouch__P_52_0.x);
          var distY = Math.abs(y - qx.event.handler.PointerCore.__lastTouch__P_52_0.y);
       ...
       */
      _stopObserver() {
        for (var i = 0; i < this.__eventNames__P_52_3.length; i++) {
          qx.bom.Event.removeNativeListener(this.__defaultTarget__P_52_1, this.__eventNames__P_52_3[i], this.__wrappedListener__P_52_7);
        }
      },
      ...
      dispose() {
        ...
        this.__defaultTarget__P_52_1 = this.__emitter__P_52_2 = null;
      }
...
//# sourceMappingURL=PointerCore.js.map?dt=1658886718851