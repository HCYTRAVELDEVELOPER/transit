  ...
  qx.Bootstrap.define("qx.bom.AnimationFrame", {
    ...
    members: {
      ...
      startSequence(duration) {
        this.__canceled__P_72_0 = false;
        var start = window.performance && performance.now ? performance.now() + qx.bom.AnimationFrame.__start__P_72_1 : Date.now();

        var cb = function cb(time) {
          if (this.__canceled__P_72_0) {
            this.id = null;
            return;
          } // final call
    ...
    statics: {
      ...
      request(callback, context) {
        ...
        var cb = function cb(time) {
          ...
          if (time < 1e10) {
            time = qx.bom.AnimationFrame.__start__P_72_1 + time;
          }
    ...
    defer(statics) {
      ...
      statics.__start__P_72_1 = window.performance && performance.timing && performance.timing.navigationStart; // if not, simply use the current time

      if (!statics.__start__P_72_1) {
        statics.__start__P_72_1 = Date.now();
      }
    }
...
//# sourceMappingURL=AnimationFrame.js.map?dt=1658886726614