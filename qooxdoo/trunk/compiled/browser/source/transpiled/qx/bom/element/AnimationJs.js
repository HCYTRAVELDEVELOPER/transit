  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.bom.element.AnimationJs", {
    statics: {
       ...
       */
      __maxStepTime__P_56_0: 30,

      /**
       * The supported CSS units.
       */
      __units__P_56_1: ["%", "in", "cm", "mm", "em", "ex", "pt", "pc", "px"],

      /** The used keys for transforms. */
      __transitionKeys__P_56_2: {
        scale: true,
        rotate: true,
        skew: true,
        translate: true
      ...
      _animate(el, desc, duration, reverse) {
        ...
        var keys = this.__getOrderedKeys__P_56_3(keyFrames);

        var stepTime = this.__getStepTime__P_56_4(duration, keys);

        var steps = parseInt(duration / stepTime, 10);

        this.__normalizeKeyFrames__P_56_5(keyFrames, el);

        var delta = this.__calculateDelta__P_56_6(steps, stepTime, keys, keyFrames, duration, desc.timing);
        ...
        handle.i = 0;
        handle.initValues = {};
        handle.repeatSteps = this.__applyRepeat__P_56_7(steps, desc.repeat);
       ...
       * @param el {Element} The element to animate.
       */
      __normalizeKeyFrames__P_56_5(keyFrames, el) {
          ...
          for (var name in keyFrames[percent]) {
            ...
            if (units[name] == undefined) {
              ...
              if (typeof item == "string") {
                units[name] = this.__getUnit__P_56_8(item);
              } else {
                units[name] = "";
              }
            }
        ...
        for (var percent in keyFrames) {
          ...
          for (var name in units) {
            if (frame[name] == undefined) {
              ...
              if (frame[name] === "" && this.__units__P_56_1.indexOf(units[name]) != -1) {
       ...
       */
      __normalizeKeyFrameTransforms__P_56_9(frame) {
        frame = qx.lang.Object.clone(frame);
        var transforms;

        for (var name in frame) {
          if (name in this.__transitionKeys__P_56_2) {
            if (!transforms) {
              transforms = {};
            }
       ...
       */
      __calculateDelta__P_56_6(steps, stepTime, keys, keyFrames, duration, timing) {
        var delta = new Array(steps);
        var keyIndex = 1;
        delta[0] = this.__normalizeKeyFrameTransforms__P_56_9(keyFrames[0]);
            ...
            if (name in this.__transitionKeys__P_56_2) {
              ...
              if (qx.Bootstrap.isArray(last[name])) {
                ...
                for (var j = 0; j < next[name].length; j++) {
                  ...
                  transforms[name][j] = this.__getNextValue__P_56_10(item, last[name], timing, x);
                }
              } else {
                var x = calculationIndex / stepsToNext;
                transforms[name] = this.__getNextValue__P_56_10(nItem, last[name], timing, x);
              } // color values
              ...
              var x = calculationIndex / stepsToNext;
              delta[i][name] = this.__getNextValue__P_56_10(nItem, last[name], timing, x);
            } else {
        ...
        delta[delta.length - 1] = this.__normalizeKeyFrameTransforms__P_56_9(keyFrames[100]);
        return delta;
      },
       ...
       */
      __getNextValue__P_56_10(nextItem, lastItem, timing, x) {
        var range = parseFloat(nextItem) - parseFloat(lastItem);
        return parseFloat(lastItem) + range * qx.bom.AnimationFrame.calculateTiming(timing, x) + this.__getUnit__P_56_8(nextItem);
      },
      ...
      play(handle) {
        ...
        var id = window.setInterval(function () {
          ...
          qx.bom.element.AnimationJs.__applyStyles__P_56_11(handle.el, values);
      ...
      stop(handle) {
        ...
        if (keep != undefined && !handle.stopped) {
          ...
          this.__applyStyles__P_56_11(el, this.__normalizeKeyFrameTransforms__P_56_9(desc.keyFrames[keep]));
        } else {
          this.__applyStyles__P_56_11(el, initValues);
        }
       ...
       */
      __applyRepeat__P_56_7(steps, repeat) {
        if (repeat == undefined) {
          return steps;
        }
       ...
       */
      __applyStyles__P_56_11(el, styles) {
       ...
       */
      __getStepTime__P_56_4(duration, keys) {
        // get min difference
        var minDiff = 100;
        ...
        while (stepTime > this.__maxStepTime__P_56_0) {
          stepTime = stepTime / 2;
        }
       ...
       */
      __getOrderedKeys__P_56_3(keyFrames) {
...
//# sourceMappingURL=AnimationJs.js.map?dt=1658886720360