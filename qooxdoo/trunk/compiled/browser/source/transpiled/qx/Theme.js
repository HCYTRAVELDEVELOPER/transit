    ...
    statics: {
      ...
      */
      ...
      define(name, config) {
        ...
        config.include = this.__normalizeArray__P_54_0(config.include);
        config.patch = this.__normalizeArray__P_54_0(config.patch); // Validate incoming data

        {
          this.__validateConfig__P_54_1(name, config);
        } // Create alias
        ...
        this.__convert__P_54_2(theme, config);

        this.__initializeAliases__P_54_3(theme, config); // Store class reference in global class registry
       ...
       */
      __normalizeArray__P_54_0(objectOrArray) {
        if (!objectOrArray) {
          return [];
        }
       ...
       * @param config {Map} config structure
       */
      __initializeAliases__P_54_3(theme, config) {
      ...
      */
       ...
       */
      __extractType__P_54_4(config) {
        for (var i = 0, keys = this.__inheritableKeys__P_54_5, l = keys.length; i < l; i++) {
          if (config[keys[i]]) {
            return keys[i];
          }
        }
       ...
       */
      __convert__P_54_2(theme, config) {
        var type = this.__extractType__P_54_4(config); // Use theme key from extended theme if own one is not available
      ...
      __inheritableKeys__P_54_5: ["colors", "borders", "decorations", "fonts", "icons", "widgets", "appearances", "meta"],

      /** @type {Map} allowed keys in theme definition */
      __allowedKeys__P_54_6: qx.core.Environment.select("qx.debug", {
        true: {
          title: "string",
          // String
          aliases: "object",
      ...
      __metaKeys__P_54_7: qx.core.Environment.select("qx.debug", {
        true: {
          color: "object",
          border: "object",
          decoration: "object",
       ...
       */
      __validateConfig__P_54_1: qx.core.Environment.select("qx.debug", {
        true(name, config) {
          var allowed = this.__allowedKeys__P_54_6;
          ...
          if (config.meta) {
            ...
            for (var key in config.meta) {
              ...
              if (this.__metaKeys__P_54_7[key] === undefined) {
                throw new Error('The key "' + key + '" is not allowed inside a meta theme block.');
              }

              if (typeof value !== this.__metaKeys__P_54_7[key]) {
      ...
      patch(theme, mixinTheme) {
        this.__checkForInvalidTheme__P_54_8(mixinTheme);

        var type = this.__extractType__P_54_4(mixinTheme);

        if (type !== this.__extractType__P_54_4(theme)) {
       ...
       */
      __checkForInvalidTheme__P_54_8(mixinTheme) {
...
//# sourceMappingURL=Theme.js.map?dt=1658886719390