  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.dev.StackTrace", {
    statics: {
      ...
      getStackTraceFromError(error) {
        ...
        if (traceProp === "stack") {
          ...
          while ((hit = lineRe.exec(error.stack)) != null) {
            url = hit[1];
            lineNumber = hit[2];
            className = this.__fileNameToClassName__P_153_0(url);
            trace.push(className + ":" + lineNumber);
          }

          if (trace.length > 0) {
            return this.__formatStackTrace__P_153_1(trace);
          }
          /*
          ...
          while ((hit = lineRe.exec(error.stack)) != null) {
            ...
            if (fileMatch) {
              className = this.__fileNameToClassName__P_153_0(fileMatch[2]);
              trace.push(className + fileMatch[3]);
            } else {
              fileMatch = fileRe.exec(hit[1]);

              if (fileMatch) {
                className = this.__fileNameToClassName__P_153_0(fileMatch[1]);
          ...
          for (var i = 0; i < lines.length; i++) {
            ...
            if (reResult && reResult.length >= 2) {
              ...
              fileName = this.__fileNameToClassName__P_153_0(reResult[2]);
              trace.push(fileName + ":" + lineNumber);
            }
          }
        } else if (error.sourceURL && error.line) {
          // Safari
          trace.push(this.__fileNameToClassName__P_153_0(error.sourceURL) + ":" + error.line);
        }
       ...
       */
      __fileNameToClassName__P_153_0(fileName) {
        ...
        return qx.dev.StackTrace.__fileNameToClassNameDefault__P_153_2(fileName);
      },
       ...
       */
      __fileNameToClassNameDefault__P_153_2(fileName) {
       ...
       */
      __formatStackTrace__P_153_1(trace) {
