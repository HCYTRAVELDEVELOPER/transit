  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.tool.cli.ConfigDb", {
    ...
    members: {
       ...
       */
      async load() {
        this.__db__P_4_1 = (await qx.tool.utils.Json.loadJsonAsync(this.getPath())) || {};
      },
      ...
      async save() {
        ...
        await qx.tool.utils.Json.saveJsonAsync(this.getPath(), this.__db__P_4_1);
      },
      ...
      setOverride(key, value) {
        if (value === undefined) {
          delete this.__overrides__P_4_0[key];
        } else {
          this.__overrides__P_4_0[key] = value;
        }
      },
      ...
      db(path, defaultValue) {
        if (path) {
          let override = this.__overrides__P_4_0[path];

          if (override) {
            return override;
          }

          var result = this.__db__P_4_1;
          var segs = path.split(".");
        ...
        return this.__db__P_4_1;
      }

    },

    defer(statics) {
      statics.__directory__P_4_2 = path.join(require("os").homedir(), ".qooxdoo/");
    },

    statics: {
      /** Singleton default instance */
      __instance__P_4_3: null,

      /** The directory where config files (any any other temporary/cached data) is kept */
      __directory__P_4_2: null,
       ...
       */
      async getInstance() {
        let db = qx.tool.cli.ConfigDb.__instance__P_4_3;

        if (!db) {
          db = qx.tool.cli.ConfigDb.__instance__P_4_3 = new qx.tool.cli.ConfigDb();
       ...
       */
      getDirectory() {
        return this.__directory__P_4_2;
      },
