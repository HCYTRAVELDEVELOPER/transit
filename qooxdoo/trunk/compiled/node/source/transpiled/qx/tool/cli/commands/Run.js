    ...
    members: {
      ...
      async process() {
        ...
        this.addListener("made", async e => {
          if (this.__process__P_13_0) {
            if (!restartNeeded) {
              return;
            }

            try {
              await qx.tool.utils.Utils.killTree(this.__process__P_13_0.pid);
            } catch (ex) {//Nothing
            }

            this.__process__P_13_0 = null;
          }

          console.log("Starting application: " + cmd);

          let child = this.__process__P_13_0 = require("child_process").exec(cmd);
