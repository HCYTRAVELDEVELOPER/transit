(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.tool.cli.commands.Lint", {
    ...
    members: {
      async process() {
        await this.__applyFixes__P_11_0();
        ...
        lintOptions.globals = Object.assign(lintOptions.globals || {}, await this.__addGlobals__P_11_1(config));
        ...
        if (this.argv.printConfig) {
          ...
          if (report.errorCount > 0 || report.warningCount > 0) {
            ...
            const s = formatter.format(report); // If there are too many errors, the pretty formatter is appallingly slow so if the
            ...
            if (report.errorCount + report.warningCount > maxDefaultFormatErrorCount) {
              if (!this.argv.format) {
                qx.tool.compiler.Console.info(`Total errors and warnings exceed ${maxDefaultFormatErrorCount}, switching to "compact" style report`);
                outputFormat = "compact";
              } else {
                qx.tool.compiler.Console.info(`Total errors and warnings exceed ${maxDefaultFormatErrorCount}, the report may take some time to generate.`);
              }
            }
       ...
       * @return {Promise<void>}
       */
      async __addGlobals__P_11_1(data) {
        let result = {};
