(function () {
  ...
  qx.Class.define("qx.tool.compiler.resources.ScssFile", {
    ...
    construct(target, library, filename) {
      ...
      this.__library__P_37_0 = library;
      this.__filename = filename;
      this.__target__P_37_1 = target;
      this.__sourceFiles__P_37_2 = {};
      this.__importAs__P_37_3 = {};
    },
    ...
    members: {
      __library__P_37_0: null,
      __filename: null,
      __outputDir__P_37_4: null,
      __absLocations__P_37_5: null,
      __sourceFiles__P_37_2: null,
      __importAs__P_37_3: null,
      ...
      async compile(outputFilename) {
        this.__outputDir__P_37_4 = path.dirname(outputFilename);
        this.__absLocations__P_37_5 = {};
        let inputFileData = await this.loadSource(this.__filename, this.__library__P_37_0);
        await new qx.Promise((resolve, reject) => {
          sass.render({
            ...
            importer: (url, prev, done) => {
              let contents = this.__sourceFiles__P_37_2[url];

              if (!contents) {
                let tmp = this.__importAs__P_37_3[url];

                if (tmp) {
                  contents = this.__sourceFiles__P_37_2[tmp];
                }
              }
            ...
            functions: {
              "qooxdooUrl($filename, $url)": ($filename, $url, done) => this.__qooxdooUrlImpl__P_37_6($filename, $url, done)
            }
          }, (error, result) => {
            if (error) {
        ...
        return Object.keys(this.__sourceFiles__P_37_2);
      },
      ...
      _analyseFilename(url, currentFilename) {
        ...
        let library = this.__target__P_37_1.getAnalyser().getLibraries().find(library => filename.startsWith(path.resolve(library.getRootDir())));
      ...
      reloadSource(filename) {
        ...
        delete this.__sourceFiles__P_37_2[filename];
      ...
      async loadSource(filename, library) {
        ...
        if (!exists) {
          this.__sourceFiles__P_37_2[absFilename] = null;
          return null;
        }

        if (this.__sourceFiles__P_37_2[absFilename] !== undefined) {
          return qx.Promise.resolve(this.__sourceFiles__P_37_2[absFilename]);
        }
        ...
        contents = contents.replace(/@import\s+["']([^;]+)["']/gi, (match, p1, offset) => {
          ...
          let newLibrary = this.__target__P_37_1.getAnalyser().findLibrary(pathInfo.namespace);
        ...
        contents = contents.replace(/\burl\s*\(\s*([^\)]+)*\)/gi, (match, url) => {
          ...
          if (pathInfo) {
            ...
            if (pathInfo.namespace) {
              let targetFile = path.relative(process.cwd(), path.join(this.__target__P_37_1.getOutputDir(), "resource", pathInfo.filename));
              let relative = path.relative(this.__outputDir__P_37_4, targetFile);
              return `url("${relative}")`;
            }
          }
        ...
        this.__sourceFiles__P_37_2[absFilename] = contents;
        this.__importAs__P_37_3[filename] = absFilename;
      ...
      __qooxdooUrlImpl__P_37_6($filename, $url, done) {
