(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.tool.cli.commands.Compile", {
    ...
    members: {
      __gauge__P_8_0: null,
      __makers__P_8_1: null,
      __config__P_8_2: null,
      __libraries__P_8_3: null,
      __outputDirWasCreated__P_8_4: false,
...
Framework: v${await this.getQxVersion()} in ${await this.getQxPath()}`);
        ...
        if (this.argv["machine-readable"]) {
          ...
          if (this.argv["feedback"]) {
            ...
            this.__gauge__P_8_0 = new Gauge();

            this.__gauge__P_8_0.setTheme(ourTheme);

            this.__gauge__P_8_0.show("Compiling", 0);
            ...
            qx.tool.compiler.Console.getInstance().setWriter((str, msgId) => {
              ...
              if (!msgId || msgId.type !== "message") {
                this.__gauge__P_8_0.hide();

                qx.tool.compiler.Console.log(colorOn + TYPES[(msgId || {}).type || "error"] + ": " + str);

                this.__gauge__P_8_0.show();
              } else {
                this.__gauge__P_8_0.show(colorOn + str);
              }
            });
          }
        }

        if (this.__gauge__P_8_0) {
          this.addListener("writingApplications", () => this.__gauge__P_8_0.show("Writing Applications", 0));
          this.addListener("writtenApplications", () => this.__gauge__P_8_0.show("Writing Applications", 1));
          this.addListener("writingApplication", evt => this.__gauge__P_8_0.pulse("Writing Application " + evt.getData().appMeta.getApplication().getName()));
          this.addListener("compilingClass", evt => this.__gauge__P_8_0.pulse("Compiling " + evt.getData().classFile.getClassName()));
          this.addListener("minifyingApplication", evt => this.__gauge__P_8_0.pulse("Minifying " + evt.getData().application.getName() + " " + evt.getData().filename));
        } else {
        ...
        this.addListener("making", evt => {
          if (this.__gauge__P_8_0) {
            this.__gauge__P_8_0.show("Compiling", 1);
          } else {
        ...
        this.addListener("writtenApplications", e => {
          if (this.argv.verbose) {
            ...
            Object.values(this.__libraries__P_8_3).forEach(lib => qx.tool.compiler.Console.log(`   ${lib.getNamespace()} (${lib.getRootDir()})`));
          }
        });
        await this._loadConfigAndStartMaking();

        if (!this.argv.watch) {
          let success = this.__makers__P_8_1.every(maker => maker.getSuccess());

          let hasWarnings = this.__makers__P_8_1.every(maker => maker.getHasWarnings());

          if (success && hasWarnings && this.argv.warnAsError) {
            success = false;
          }

          if (!this.argv.deploying && !this.argv["machine-readable"] && this.argv["feedback"] && this.__outputDirWasCreated__P_8_4 && this.argv.target === "build") {
      ...
      async _loadConfigAndStartMaking() {
        var config = this.__config__P_8_2 = await qx.tool.cli.Cli.getInstance().getParsedArgs();

        if (!config) {
          throw new qx.tool.utils.Utils.UserError("Error: Cannot find any configuration");
        }

        var makers = this.__makers__P_8_1 = await this.createMakersFromConfig(config);
        ...
        await qx.Promise.all(makers.map(async maker => {
          ...
          if (!(await fs.existsAsync(maker.getOutputDir()))) {
            this.__outputDirWasCreated__P_8_4 = true;
          }
      ...
      async createMakersFromConfig(data) {
        ...
        let libraries = this.__libraries__P_8_3 = {};
        ...
        let errors = await this.__checkDependencies__P_8_5(Object.values(libraries), data.packages);
       ...
       * @private
       */
      async __checkDependencies__P_8_5(libs, packages) {
        const Console = qx.tool.compiler.Console.getInstance();
       ...
       */
      getMaker() {
        if (this.__makers__P_8_1.length == 1) {
          return this.__makers__P_8_1[0];
        }

        throw new Error("Cannot get a single maker - there are " + this.__makers__P_8_1.length + " available");
      },
      ...
      getMakersForApp(appName) {
        return this.__makers__P_8_1.filter(maker => {
       ...
       */
      getLibraries() {
        return this.__libraries__P_8_3;
      }
