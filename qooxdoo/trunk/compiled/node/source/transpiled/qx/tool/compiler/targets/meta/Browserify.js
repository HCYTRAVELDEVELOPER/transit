(function () {
  ...
  qx.Class.define("qx.tool.compiler.targets.meta.Browserify", {
    ...
    construct(appMeta) {
      ...
      this.__appMeta__P_44_0 = appMeta;
      this.setNeedsWriteToDisk(true);
    },

    members: {
      __appMeta__P_44_0: null,
      ...
      async writeSourceCodeToStream(ws) {
        ...
        const db = this.__appMeta__P_44_0.getAnalyser().getDatabase(); // Get a Set of unique `require`d CommonJS module names from all classes
        ...
        if (hasCommonjsModules) {
          await this.__browserify__P_44_1(commonjsModules, references, ws);
        }
      ...
      async __browserify__P_44_1(commonjsModules, references, ws) {
        let b;

        const browserify = require("browserify");

        const builtins = require("browserify/lib/builtins.js"); // For some reason, `process` is not require()able, but `_process` is.
