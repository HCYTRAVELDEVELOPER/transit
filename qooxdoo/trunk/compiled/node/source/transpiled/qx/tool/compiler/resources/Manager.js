(function () {
  ...
  qx.Class.define("qx.tool.compiler.resources.Manager", {
    ...
    construct(analyser) {
      ...
      this.__analyser__P_36_0 = analyser;
      this.__dbFilename__P_36_1 = analyser.getResDbFilename() || "resource-db.json";
      this.__loaders__P_36_2 = [new qx.tool.compiler.resources.ImageLoader(), new qx.tool.compiler.resources.MetaLoader()];
      this.__converters__P_36_3 = [new qx.tool.compiler.resources.ScssConverter(), new qx.tool.compiler.resources.ScssIncludeConverter()];
    },

    members: {
      /** {String} filename of database */
      __dbFilename__P_36_1: null,

      /** {Object} Database */
      __db__P_36_4: null,

      /** the used analyser */
      __analyser__P_36_0: null,

      /** {Map{String,Library}} Lookup of libraries, indexed by resource URI */
      __librariesByResourceUri__P_36_5: null,

      /** {String[]} Array of all resource URIs, sorted alphabetically (ie these are the keys in __librariesByResourceUri) */
      __allResourceUris__P_36_6: null,

      /** {ResourceLoader[]} list of resource loaders, used to add info to the database */
      __loaders__P_36_2: null,

      /** {ResourceConverter[]} list of resource converters, used to copy resources to the target */
      __converters__P_36_3: null,

      /**
       * Loads the cached database
       */
      async loadDatabase() {
        this.__db__P_36_4 = (await qx.tool.utils.Json.loadJsonAsync(this.__dbFilename__P_36_1)) || {};
      },
      ...
      async saveDatabase() {
        ...
        return qx.tool.utils.Json.saveJsonAsync(this.__dbFilename__P_36_1, this.__db__P_36_4);
      },
      ...
      findLibraryForResource(uri) {
        ...
        if (result.length > 1) {
          qx.tool.compiler.Console.error(`Cannot determine a single library for the URI '${uri}'; ` + `found ${result.map(l => l.getNamespace()).join(",")} returning first library`);
        }
      ...
      findLibrariesForResource(uri) {
        const findLibrariesForResourceImpl = () => {
          ...
          if (path.isAbsolute(uri)) {
            let library = this.__analyser__P_36_0.getLibraries().find(lib => uri.startsWith(path.resolve(lib.getRootDir())));
          ...
          if (pos !== -1) {
            ...
            let library = this.__analyser__P_36_0.findLibrary(ns);
          ...
          if (pos === -1) {
            let library = this.__librariesByResourceUri__P_36_5[uri] || null;
            return library;
          } // Strip wildcard
          ...
          if (!this.__allResourceUris__P_36_6) {
            this.__allResourceUris__P_36_6 = Object.keys(this.__librariesByResourceUri__P_36_5).sort();
          }

          var thisUriPos = qx.tool.utils.Values.binaryStartsWith(this.__allResourceUris__P_36_6, uri);

          if (thisUriPos > -1) {
            let libraries = {};

            for (; thisUriPos < this.__allResourceUris__P_36_6.length; thisUriPos++) {
              var thisUri = this.__allResourceUris__P_36_6[thisUriPos];
              ...
              if (pos !== -1) {
                ...
                if (!libraries[ns]) {
                  libraries[ns] = this.__analyser__P_36_0.findLibrary(ns);
                }
              }
            }
      ...
      async findAllResources() {
        var t = this;
        var db = this.__db__P_36_4;

        if (!db.resources) {
          db.resources = {};
        }

        t.__librariesByResourceUri__P_36_5 = {};
        this.__allResourceUris__P_36_6 = null;
        this.__assets__P_36_7 = {};
        await qx.Promise.all(t.__analyser__P_36_0.getLibraries().map(async library => {
        ...
        await qx.tool.utils.Promisify.poolEachOf(Object.values(this.__assets__P_36_7), 10, async asset => {
          await asset.load();
          ...
          if (fileInfo.meta) {
            for (var altPath in fileInfo.meta) {
              ...
              let otherAsset = this.__assets__P_36_7[lib.getNamespace() + ":" + altPath];
          ...
          if (fileInfo.dependsOn) {
            ...
            fileInfo.dependsOn.forEach(str => {
              let otherAsset = this.__assets__P_36_7[str];
       ...
       */
      __addAsset__P_36_8(asset) {
        this.__assets__P_36_7[asset.toUri()] = asset;
        ...
        filename.split("/").forEach((seg, index) => {
          ...
          tmp += seg;
          let current = this.__librariesByResourceUri__P_36_5[tmp];
          ...
          if (current) {
            if (qx.lang.Type.isArray(current)) {
              ...
              current = this.__librariesByResourceUri__P_36_5[tmp] = [current, library];
            }
          } else {
            this.__librariesByResourceUri__P_36_5[tmp] = library;
          }
        });
        asset.setLoaders(this.__loaders__P_36_2.filter(loader => loader.matches(filename, library)));
        asset.setConverters(this.__converters__P_36_3.filter(converter => converter.matches(filename, library)));
      },
      ...
      getAsset(srcPath, create, isThemeFile) {
        ...
        let asset = this.__assets__P_36_7[library.getNamespace() + ":" + srcPath];
      ...
      getAssetsForPaths(srcPaths) {
        var db = this.__db__P_36_4; // Generate a lookup that maps the resource name to the meta file that
        ...
        srcPaths.forEach(srcPath => {
          ...
          if (pos > -1) {
            ...
            let tmp = this.__analyser__P_36_0.findLibrary(ns);
          ...
          libraries.forEach(library => {
            ...
            resourceNames.forEach(resourceName => {
              ...
              let asset = this.__assets__P_36_7[library.getNamespace() + ":" + resourceName];
