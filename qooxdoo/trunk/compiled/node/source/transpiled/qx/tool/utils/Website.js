  ...
  qx.Class.define("qx.tool.utils.Website", {
    ...
    members: {
      ...
      async startWatcher() {
        ...
        this._watcher.on("change", filename => this.__onFileChange__P_55_5("change", filename));

        this._watcher.on("add", filename => this.__onFileChange__P_55_5("add", filename));

        this._watcher.on("unlink", filename => this.__onFileChange__P_55_5("unlink", filename));

        this._watcher.on("ready", async () => {
          await this.triggerRebuild(true);
          this.__watcherReady__P_55_1 = true;
        });
      ...
      async stopWatcher() {
        if (this._watcher) {
          ...
          this._watcher = null;
          this.__watcherReady__P_55_1 = false;
        }
      },
      ...
      async waitForRebuildComplete() {
        if (this.__rebuildPromise__P_55_6) {
          await this.__rebuildPromise__P_55_6;
        }
      },
       ...
       */
      __onFileChange__P_55_5(type, filename) {
        if (this.__watcherReady__P_55_1) {
      ...
      triggerRebuild(immediate) {
        if (this.__rebuilding__P_55_3) {
          this.__needsRebuild__P_55_4 = true;
          return;
        }
        ...
        let rebuilderImpl = async () => {
          ...
          if (this.__needsRebuild__P_55_4) {
            this.__needsRebuild__P_55_4 = false;
            await rebuilderImpl();
          }
        };

        let rebuilder = async () => {
          this.__rebuilding__P_55_3 = true;

          try {
            this.__rebuildPromise__P_55_6 = rebuilderImpl();
            await this.__rebuildPromise__P_55_6;
            this.__rebuildPromise__P_55_6 = null;
          } finally {
            this.__rebuilding__P_55_3 = false;
          }
        };

        if (this.__rebuildTimer__P_55_2) {
          clearTimeout(this.__rebuildTimer__P_55_2);
          this.__rebuildTimer__P_55_2 = null;
        }

        this.__rebuildTimer__P_55_2 = setTimeout(rebuilder, immediate ? 1 : 250);
      },
      ...
      async loadPartials(files, metalsmith) {
        ...
        for (let filename of files) {
          ...
          if (unused) {// this is simply to avoid linting errors until https://github.com/qooxdoo/qooxdoo/issues/461 is fixed
          }
