  ...
  qx.Class.define("qx.tool.cli.commands.package.Update", {
    ...
    members: {
      ...
      async process() {
        ...
        this.__names__P_20_0 = []; // export only
        ...
        let github = cfg.db("github", {}); // Create the cache
      ...
      async updateFromGitHubAPI(token) {
        ...
        let repo_lookup = {};
        let repos_data = this.getCache().repos.data; // iterate over repositories
        ...
        for (let repo of repos) {
          ...
          this.__names__P_20_0.push(name);
          ...
          .filter(r => this.argv["all-versions"] ? true : semver.valid(r.tag_name, true)) // attach a clean version number
          .map(r => {
        ...
        if (!this.argv.repository) {
          this.getCache().repos.list = this.__names__P_20_0.sort();
        }
