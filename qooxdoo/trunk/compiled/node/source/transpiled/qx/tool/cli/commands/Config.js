(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.tool.cli.commands.Config", {
    ...
    members: {
      __describe__P_9_0(key) {
        var data = qx.tool.cli.commands.Config.KNOWN_VALUES[key];
        return data && data.desc;
      },

      __checkKey__P_9_1(argv) {
        if (!argv.quiet) {
          let desc = this.__describe__P_9_0(argv.key);
      ...
      async cmdSet(argv) {
        ...
        let keyInfo = this.__breakout__P_9_2(argv.key);
      ...
      async cmdList(argv) {
        ...
        keys = keys.map(key => ({
          key: key,
          value: cfg.db(key),
          description: this.__describe__P_9_0(key) || "Unrecognised key"
        })); // Display each value
