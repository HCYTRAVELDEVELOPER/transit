  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.tool.config.Utils", {
    ...
    statics: {
       ...
       */
      async getQxPath() {
        if (this.__qxPathPromise__P_51_0) {
          return await this.__qxPathPromise__P_51_0;
        }
        ...
        this.__qxPathPromise__P_51_0 = getQxPathImpl();
        return await this.__qxPathPromise__P_51_0;
      },
      ...
      async getAppQxVersion(baseDir = null) {
        ...
        let manifestModel = await qx.tool.config.Manifest.getInstance().set({
          baseDir,
          warnOnly: true,
          validate: false
        }).load();
        ...
        if (!qxVersion || !semver.valid(qxVersion)) {
          throw new Error(`Cannot determine the qooxdoo version used to compile the application. ` + `Please specify a caret or tilde range for the requires.${manifestRequiresKey} key in the Manifest")`);
        }
