  ...
  qx.Class.define("qx.tool.cli.commands.package.Install", {
    ...
    members: {
      ...
      async process() {
        ...
        await this.__updateCache__P_16_1();
        ...
        if (!this.argv.uri && !this.argv.fromPath) {
          if (lockfileModel.getValue("libraries").length) {
            await this.__downloadLibrariesInLockfile__P_16_2();
          } else {
            await this.__installDependenciesFromManifest__P_16_3(manifestModel.getData());
        ...
        if (this.argv.fromPath) {
          ...
          await this.__installFromPath__P_16_4(uri, this.argv.fromPath, saveToManifest);
        } else if (!id || qx.lang.Type.isString(id) && id.startsWith("v")) {
          // install library/libraries from GitHub release
          await this.__installFromRelease__P_16_5(uri, id, this.argv.save);
        } else {
          // install library from GitHub code tree
          await this.__installFromTree__P_16_6(uri, id, this.argv.save);
        }
       ...
       * @private
       */
      async __installFromRelease__P_16_5(uri, tag_name, writeToManifest) {
        let qxVersion = (await this.getAppQxVersion()).replace("-beta", "");

        let {
          repo_name,
          package_path
        } = this.__getUriInfo__P_16_7(uri);
        ...
        if (!tag_name) {
          ...
          if (!tag_name) {
            qx.tool.compiler.Console.warn(`'${repo_name}' has no (stable) release compatible with qooxdoo version ${qxVersion}.
        ...
        let {
          download_path
        } = await this.__download__P_16_8(repo_name, tag_name); // iterate over contained libraries
        ...
        for (let {
          ...
          found = true;
          await this.__updateInstalledLibraryData__P_16_9(library_uri, tag_name, download_path, writeToManifest);
        }
       ...
       * @private
       */
      async __installFromTree__P_16_6(uri, hash, writeToManifest) {
        ...
        let {
          download_path
        } = await this.__download__P_16_8(repo_name, hash);
        await this.__updateInstalledLibraryData__P_16_9(uri, hash, download_path, writeToManifest);
      },
       ...
       * @private
       */
      async __installFromPath__P_16_4(uri, dir, writeToManifest = false) {
        let qxVersion = await this.getAppQxVersion();
        ...
        await this.__updateInstalledLibraryData__P_16_9(uri, undefined, dir, writeToManifest);
      },
       ...
       * @private
       */
      async __updateInstalledLibraryData__P_16_9(uri, id, download_path, writeToManifest) {
        let {
          repo_name,
          package_path
        } = uri ? this.__getUriInfo__P_16_7(uri) : {
          repo_name: "",
          package_path: ""
        };
        ...
        let appsInstalled = await this.__installApplication__P_16_10(library_path);

        if (!appsInstalled && this.argv.verbose) {
          qx.tool.compiler.Console.info(`>>> No applications installed for ${uri}.`);
        }

        let depsInstalled = await this.__installDependenciesFromPath__P_16_11(library_path);
       ...
       */
      async __installDependenciesFromPath__P_16_11(downloadPath) {
        ...
        return this.__installDependenciesFromManifest__P_16_3(manifest);
      },
       ...
       * @return {Promise<Boolean>} Wether any libraries were installed
       */
      async __installDependenciesFromManifest__P_16_3(manifest) {
            ...
            case "@qooxdoo/framework":
              {
                ...
                if (!semver.satisfies(qxVersion, lib_range, {
                  ...
                  throw new qx.tool.utils.Utils.UserError(`Library '${lib_uri}' needs @qooxdoo/framework version ${lib_range}, found ${qxVersion}`);
                }
            ...
            default:
              {
                ...
                if (semver.validRange(lib_range)) {
                  let {
                    tag
                  } = this.__getHighestCompatibleVersion__P_16_12(lib_uri, lib_range);
                  ...
                  if (!(await this.isInstalled(lib_uri, tag))) {
                    await this.__installFromRelease__P_16_5(lib_uri, tag, false);
                    break;
                  }
                ...
                if (!(await this.isInstalled(lib_uri, lib_range))) {
                  try {
                    await this.__installFromTree__P_16_6(lib_uri, lib_range, false);
                    break;
                  } catch (e) {
       ...
       * @private
       */
      __getHighestCompatibleVersion__P_16_12(lib_uri, lib_range) {
        let {
          repo_name
        } = this.__getUriInfo__P_16_7(lib_uri);
       ...
       */
      async __installApplication__P_16_10(downloadPath) {
       ...
       * @private
       */
      async __downloadLibrariesInLockfile__P_16_2() {
        if (this.argv.verbose) {
        ...
        return qx.Promise.all(libraries.filter(lib => lib.repo_name && lib.repo_tag).map(lib => this.__download__P_16_8(lib.repo_name, lib.repo_tag)));
      },
       ...
       */
      async __download__P_16_8(repo_name, treeish = null, force = false) {
