(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.data.controller.Form", {
    ...
    members: {
      __objectController__P_76_1: null,
      __bindingOptions__P_76_0: null,
      ...
      addBindingOptions(name, model2target, target2model) {
        this.__bindingOptions__P_76_0[name] = [model2target, target2model]; // return if not both, model and target are given
        ...
        var targetProperty = this.__isModelSelectable__P_76_2(item) ? "modelSelection[0]" : "value"; // remove the binding

        this.__objectController__P_76_1.removeTarget(item, targetProperty, name); // set up the new binding with the options


        this.__objectController__P_76_1.addTarget(item, targetProperty, name, !this._selfUpdate, model2target, target2model);
      },
        ...
        for (var name in items) {
          ...
          for (var i = 0; i < names.length; i++) {
            ...
            if (i + 1 == names.length) {
              ...
              if (this.__bindingOptions__P_76_0[name] && this.__bindingOptions__P_76_0[name][1]) {
                itemValue = this.__bindingOptions__P_76_0[name][1].converter(itemValue);
              }
      ...
      updateModel() {
        ...
        for (var name in items) {
          var item = items[name];
          var sourceProperty = this.__isModelSelectable__P_76_2(item) ? "modelSelection[0]" : "value";
          var options = this.__bindingOptions__P_76_0[name];
          options = options && this.__bindingOptions__P_76_0[name][1];
      ...
      _applyTarget(value, old) {
        ...
        if (old != null) {
          this.__tearDownBinding__P_76_3(old);
      ...
      _applyModel(value, old) {
        ...
        if (this.__objectController__P_76_1 != null && value == null) {
          this.__objectController__P_76_1.setModel(null);
        } // first, get rid off all bindings (avoids wrong data population)


        if (this.__objectController__P_76_1 != null && this.getTarget() != null) {
          var items = this.getTarget().getItems();

          for (var name in items) {
            var item = items[name];
            var targetProperty = this.__isModelSelectable__P_76_2(item) ? "modelSelection[0]" : "value";

            this.__objectController__P_76_1.removeTarget(item, targetProperty, name);
          }
        } // set the model of the object controller if available


        if (this.__objectController__P_76_1 != null) {
          this.__objectController__P_76_1.setModel(value);
       ...
       * up bidirectional.
       */
      __setUpBinding__P_76_4() {
        // create the object controller
        if (this.__objectController__P_76_1 == null) {
          this.__objectController__P_76_1 = new qx.data.controller.Object(this.getModel());
        } // get the form items
        ...
        for (var name in items) {
          var item = items[name];
          var targetProperty = this.__isModelSelectable__P_76_2(item) ? "modelSelection[0]" : "value";
          var options = this.__bindingOptions__P_76_0[name]; // try to bind all given items in the form

          try {
            if (options == null) {
              this.__objectController__P_76_1.addTarget(item, targetProperty, name, !this._selfUpdate);
            } else {
              this.__objectController__P_76_1.addTarget(item, targetProperty, name, !this._selfUpdate, options[0], options[1]);
       ...
       */
      __tearDownBinding__P_76_3(oldTarget) {
       ...
       */
      __isModelSelectable__P_76_2(item) {
    ...
    destruct() {
      ...
      if (this.__objectController__P_76_1) {
        this.__objectController__P_76_1.dispose();
      }
    }
