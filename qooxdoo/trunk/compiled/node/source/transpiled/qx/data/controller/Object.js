  ...
  qx.Class.define("qx.data.controller.Object", {
    ...
    */
    ...
    construct(model) {
      ...
      this.__bindings__P_79_0 = {}; // create an array to store all current targets

      this.__targets__P_79_1 = [];
    ...
    */
    members: {
      // private members
      __targets__P_79_1: null,
      __bindings__P_79_0: null,
      ...
      _applyModel(value, old) {
        // for every target
        for (var i = 0; i < this.__targets__P_79_1.length; i++) {
          // get the properties
          var targetObject = this.__targets__P_79_1[i][0];
          var targetProperty = this.__targets__P_79_1[i][1];
          var sourceProperty = this.__targets__P_79_1[i][2];
          var bidirectional = this.__targets__P_79_1[i][3];
          var options = this.__targets__P_79_1[i][4];
          var reverseOptions = this.__targets__P_79_1[i][5]; // remove it from the old if possible

          if (old != undefined && !old.isDisposed()) {
            this.__removeTargetFrom__P_79_2(targetObject, targetProperty, sourceProperty, old);
          } // add it to the new if available


          if (value != undefined) {
            this.__addTarget__P_79_3(targetObject, targetProperty, sourceProperty, bidirectional, options, reverseOptions);
          } else {
       ...
       *   {@link qx.data.SingleValueBinding} class.
       */
      addTarget(targetObject, targetProperty, sourceProperty, bidirectional, options, reverseOptions) {
        // store the added target
        this.__targets__P_79_1.push([targetObject, targetProperty, sourceProperty, bidirectional, options, reverseOptions]); // delegate the adding
       ...
       *   {@link qx.data.SingleValueBinding} class.
       */
      __addTarget__P_79_3(targetObject, targetProperty, sourceProperty, bidirectional, options, reverseOptions) {
        ...
        if (this.__bindings__P_79_0[targetHash] == undefined) {
          this.__bindings__P_79_0[targetHash] = [];
        }

        this.__bindings__P_79_0[targetHash].push([id, idReverse, targetProperty, sourceProperty, options, reverseOptions]);
      },
      ...
      removeTarget(targetObject, targetProperty, sourceProperty) {
        this.__removeTargetFrom__P_79_2(targetObject, targetProperty, sourceProperty, this.getModel()); // delete the target in the targets reference


        for (var i = 0; i < this.__targets__P_79_1.length; i++) {
          if (this.__targets__P_79_1[i][0] == targetObject && this.__targets__P_79_1[i][1] == targetProperty && this.__targets__P_79_1[i][2] == sourceProperty) {
            this.__targets__P_79_1.splice(i, 1);
          }
        }
      },
       ...
       *   comes.
       */
      __removeTargetFrom__P_79_2(targetObject, targetProperty, sourceProperty, sourceObject) {
        ...
        var currentListing = this.__bindings__P_79_0[targetObject.toHashCode()]; // if no binding is stored
