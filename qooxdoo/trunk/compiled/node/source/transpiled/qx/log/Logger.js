  ...
  qx.Bootstrap.define("qx.log.Logger", {
    statics: {
      ...
      */
       ...
       */
      setTreshold(value) {
        this.__buffer__P_115_1.setMaxMessages(value);
      },
       ...
       */
      getTreshold() {
        return this.__buffer__P_115_1.getMaxMessages();
      },
      ...
      */
      ...
      __appenders__P_115_2: [],

      /** @type {Map} Map of all known appenders by name */
      __appendersByName__P_115_3: {},
      ...
      register(appender) {
        ...
        var id = this.__id__P_115_5++;
        var appenderName = appender.appenderName || appender.classname;
        this.__appenders__P_115_2[id] = appender;
        this.__appendersByName__P_115_3[appenderName] = appender;
        appender.$$id = id; // Insert previous messages

        var entries = this.__buffer__P_115_1.getAllLogEvents();

        for (var i = 0, l = entries.length; i < l; i++) {
          var entry = entries[i];

          var appenders = this.__getAppenders__P_115_6(entry.loggerName, entry.level);
      ...
      unregister(appender) {
        ...
        var appenderName = appender.appenderName || appender.classname;
        delete this.__appendersByName__P_115_3[appenderName];
        delete this.__appenders__P_115_2[id];
        delete appender.$$id;
      },
      ...
      addFilter(logger, appenderName, level) {
        ...
        this.__filters__P_115_4.push({
          loggerMatch: logger,
          level: level || this.__level__P_115_0,
      ...
      */
      ...
      debug(object, message) {
        qx.log.Logger.__log__P_115_7("debug", arguments);
      },
       ...
       */
      info(object, message) {
        qx.log.Logger.__log__P_115_7("info", arguments);
      },
       ...
       */
      warn(object, message) {
        qx.log.Logger.__log__P_115_7("warn", arguments);
      },
      ...
      error(object, message) {
        qx.log.Logger.__log__P_115_7("error", arguments);
      },
      ...
      trace(object, message) {
        if (qx.log.Logger.isLoggerEnabled("trace", object)) {
          ...
          qx.log.Logger.__log__P_115_7("trace", args);
        }
      },
        ...
        if (qx.core.Environment.get("qx.debug")) {
          ...
          while (clazz.classname !== baseclass.classname) {
            if (clazz.prototype.hasOwnProperty(methodName)) {
              this.warn(`The method ' ${clazz.classname}.${methodName}' overrides a deprecated method:
       ...
       */
      clear() {
        this.__buffer__P_115_1.clearHistory();
      },
      ...
      */
      ...
      __buffer__P_115_1: new qx.log.appender.RingBuffer(50),
       ...
       */
      __getLoggerName__P_115_10(object) {
        if (object) {
          if (object.classname) {
      ...
      isLoggerEnabled(level, object) {
        var loggerName = this.__getLoggerName__P_115_10(object);

        var appenders = this.__getAppenders__P_115_6(loggerName, level);
      ...
      __log__P_115_7(level, args) {
        ...
        for (var i = start, l = args.length; i < l; i++) {
          items.push(this.__serialize__P_115_11(args[i], true));
        } // Build entry
        ...
        this.__buffer__P_115_1.process(entry); // Send to appenders
       ...
       */
      __getAppenders__P_115_6(className, level) {
        var levels = this.__levels__P_115_8; // If no filters, then all appenders apply

        if (!this.__filters__P_115_4.length) {
          // Check the default level
          if (levels[level] < levels[this.__level__P_115_0]) {
            return [];
          }

          return this.__appendersByName__P_115_3;
        } // Check the cache


        var cacheId = className + "|" + level;
        var appenders = this.__appendersCache__P_115_9[cacheId];
        ...
        for (var i = 0; i < this.__filters__P_115_4.length; i++) {
          var filter = this.__filters__P_115_4[i]; // Filters only apply to certain levels
          ...
          if (!filter.loggerMatch || filter.loggerMatch.test(className)) {
            if (filter.appenderName) {
              appenders[filter.appenderName] = this.__appendersByName__P_115_3[filter.appenderName];
            } else {
              return this.__appendersCache__P_115_9[cacheId] = this.__appendersByName__P_115_3;
            }
          }
        }

        return this.__appendersCache__P_115_9[cacheId] = appenders;
      },
       ...
       * <code>trace</code>.
       */
      __serialize__P_115_11(value, deep) {
        var type = this.__detect__P_115_12(value);
          ...
          case "array":
            if (deep) {
              ...
              for (var i = 0, l = value.length; i < l; i++) {
                ...
                text.push(this.__serialize__P_115_11(value[i], false));
              }
            } else {
              text = "[...(" + value.length + ")]";
            }
          ...
          case "map":
            if (deep) {
              ...
              for (var i = 0, l = sorted.length; i < l; i++) {
                ...
                key = sorted[i];
                temp = this.__serialize__P_115_11(value[key], false);
                temp.key = key;
                text.push(temp);
              }
            } else {
    ...
    defer(statics) {
      ...
      for (var i = 0; i < logs.length; i++) {
        statics.__log__P_115_7(logs[i][0], logs[i][1]);
      }
