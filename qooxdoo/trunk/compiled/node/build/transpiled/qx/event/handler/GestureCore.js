  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.event.handler.GestureCore", {
    ...
    construct(target, emitter) {
      this.__P_198_0 = target;
      this.__P_198_1 = emitter;
      this.__P_198_2 = {};
      this.__P_198_3 = {};
      this.__P_198_4 = {};
      this.__P_198_5 = {};
    ...
    members: {
      ...
      _initObserver() {
        qx.event.handler.GestureCore.GESTURE_EVENTS.forEach(function (gestureType) {
          qxWeb(this.__P_198_0).on(gestureType, this.checkAndFireGesture, this);
        }.bind(this));

        if (qx.core.Environment.get("engine.name") == "mshtml" && qx.core.Environment.get("browser.documentmode") < 9) {
          qxWeb(this.__P_198_0).on("dblclick", this._onDblClick, this);
      ...
      _stopObserver() {
        qx.event.handler.GestureCore.GESTURE_EVENTS.forEach(function (pointerType) {
          qxWeb(this.__P_198_0).off(pointerType, this.checkAndFireGesture, this);
        }.bind(this));

        if (qx.core.Environment.get("engine.name") == "mshtml" && qx.core.Environment.get("browser.documentmode") < 9) {
          qxWeb(this.__P_198_0).off("dblclick", this._onDblClick, this);
        }
      ...
      gestureBegin(domEvent, target) {
        if (this.__P_198_2[domEvent.pointerId]) {
          this.__P_198_17(this.__P_198_2[domEvent.pointerId]);

          delete this.__P_198_2[domEvent.pointerId];
        }
        /*
        ...
        this.__P_198_2[domEvent.pointerId] = {
          ...
          longTapTimer: window.setTimeout(this.__P_198_18.bind(this, domEvent, target), qx.event.handler.GestureCore.LONGTAP_TIME)
        };

        if (domEvent.isPrimary) {
          this.__P_198_9 = false;
          this.__P_198_8 = target;

          this.__P_198_19("trackstart", domEvent, target);
        } else {
          this.__P_198_9 = true;

          if (Object.keys(this.__P_198_2).length === 2) {
            this.__P_198_10 = this._calcAngle();
            this.__P_198_12 = this._calcDistance();
          }
        }
      },
      ...
      gestureMove(domEvent, target) {
        var gesture = this.__P_198_2[domEvent.pointerId];
        ...
        if (gesture) {
          ...
          if (Object.keys(this.__P_198_2).length === 2) {
            this.__P_198_20(domEvent, gesture.target);

            this.__P_198_21(domEvent, gesture.target);
          }

          if (!this.__P_198_9) {
            this.__P_198_19("track", domEvent, gesture.target);
      ...
      _hasIntermediaryHandler(target) {
        while (target && target !== this.__P_198_0) {
      ...
      gestureFinish(domEvent, target) {
        ...
        var gesture = this.__P_198_2[domEvent.pointerId]; // delete the long tap
        ...
        this.__P_198_22(gesture.velocityX, gesture.velocityY, domEvent, gesture.target);

        this.__P_198_19("trackend", domEvent, gesture.target);
        ...
        if (gesture.isTap) {
          ...
          if (Object.keys(this.__P_198_3).length > 0) {
            // delete old tap entries
            var limit = Date.now() - qx.event.handler.GestureCore.DOUBLETAP_TIME;

            for (var time in this.__P_198_3) {
              if (time < limit) {
                delete this.__P_198_3[time];
              } else {
                var lastTap = this.__P_198_3[time];

                var isBelowDoubleTapDistance = this.__P_198_23(lastTap.x, lastTap.y, domEvent.clientX, domEvent.clientY, domEvent.getPointerType());
          ...
          var swipe = this.__P_198_24(domEvent, target);
       ...
       */
      gestureCancel(id) {
        if (this.__P_198_2[id]) {
          this.__P_198_17(this.__P_198_2[id]);

          delete this.__P_198_2[id];
        }

        if (this.__P_198_5[id]) {
          this.stopMomentum(this.__P_198_5[id]);
          delete this.__P_198_5[id];
        }
      },
      ...
      updateGestureTarget(id, target) {
        this.__P_198_2[id].target = target;
      },
       ...
       */
      __P_198_22(deltaX, deltaY, domEvent, target, time) {
        var oldTimeoutId = domEvent.timeoutId;

        if (!time && this.__P_198_5[domEvent.pointerId]) {
          // new roll impulse started, stop the old one
          this.stopMomentum(this.__P_198_5[domEvent.pointerId]);
        } // do nothing if we don't need to scroll


        if (Math.abs(deltaY) < 1 && Math.abs(deltaX) < 1 || this.__P_198_4[oldTimeoutId] || !this.getWindow()) {
          delete this.__P_198_4[oldTimeoutId];
          delete this.__P_198_5[domEvent.pointerId];
          return;
        }
        ...
        var timeoutId = qx.bom.AnimationFrame.request(qx.lang.Function.bind(function (deltaX, deltaY, domEvent, target, time) {
          this.__P_198_22(deltaX, deltaY, domEvent, target, time);
        ...
        this.__P_198_5[domEvent.pointerId] = timeoutId;
      ...
      _calcAngle() {
        ...
        for (var pointerId in this.__P_198_2) {
          var gesture = this.__P_198_2[pointerId];
      ...
      _fireEvent(domEvent, type, target) {
        ...
        if (qx.core.Environment.get("event.dispatchevent")) {
        ...
        } else if (this.__P_198_1) {
          evt = new qx.event.type.dom.Custom(type, domEvent, {
            target: this.__P_198_0,
            currentTarget: this.__P_198_0,
            srcElement: this.__P_198_0,
            swipe: domEvent.swipe,
            scale: domEvent.scale,
            angle: domEvent.angle,
            delta: domEvent.delta,
          ...
          this.__P_198_1.emit(type, domEvent);
        }
      },
       ...
       * @param target {Element} event target
       */
      __P_198_19(type, domEvent, target) {
       ...
       */
      __P_198_25(domEvent, target, rollFactor) {
        domEvent.delta = {
      ...
      __P_198_26(target) {
        ...
        this.__P_198_16 = new Date().getTime();
        var reLength = this.__P_198_6.length;

        for (var i = 0; i < reLength; i++) {
          var domEvent = this.__P_198_6[i];

          this.__P_198_25(domEvent, target, rollFactor);
        }
       ...
       */
      __P_198_27() {
        if (this.__P_198_6.length > qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_THRESHOLD) {
          qx.util.Wheel.IS_TOUCHPAD = true;
        } else {
          qx.util.Wheel.IS_TOUCHPAD = false;
        }

        this.__P_198_15 = true;
      },
      ...
      _fireRoll(domEvent, type, target) {
        ...
        if (domEvent.type === qx.core.Environment.get("event.mousewheel").type) {
          if (this._isTouchPadDetectionEnabled()) {
            ...
            if (this.__P_198_16 > 0 && now - this.__P_198_16 > detectionTimeout) {
              // The detection timeout was reached. A new detection step should occur.
              this.__P_198_15 = false;
              this.__P_198_6 = [];
              this.__P_198_16 = 0;
            }
              ...
              if (this.__P_198_6.length === 0) {
                ...
                qx.event.Timer.once(function () {
                  if (!this.__P_198_15) {
                    ...
                    this.__P_198_26(target);
                  }
                }, this, qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_PERIOD + 50);
              }

              this.__P_198_6.push(domEvent);

              this.__P_198_14++;

              if (now - this.__P_198_13 > qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_PERIOD) {
                this.__P_198_27();
              }
            }
            ...
            this.__P_198_25(domEvent, target, qx.event.handler.GestureCore.ROLL_FACTOR);
          }
        } else {
          var gesture = this.__P_198_2[domEvent.pointerId];
          domEvent.delta = {
       ...
       * @param target {Element} event target
       */
      __P_198_20(domEvent, target) {
        if (!domEvent.isPrimary) {
          var angle = this._calcAngle();

          domEvent.angle = Math.round((angle - this.__P_198_10) % 360);

          this._fireEvent(domEvent, "rotate", this.__P_198_8);
        }
      },
       ...
       * @param target {Element} event target
       */
      __P_198_21(domEvent, target) {
        if (!domEvent.isPrimary) {
          var distance = this._calcDistance();

          var scale = distance / this.__P_198_12;
          domEvent.scale = Math.round(scale * 100) / 100;

          this._fireEvent(domEvent, "pinch", this.__P_198_8);
        }
      },
       ...
       */
      dispose() {
        for (var gesture in this.__P_198_2) {
          this.__P_198_17(gesture);
        }

        this._stopObserver();

        this.__P_198_0 = this.__P_198_1 = null;
      }
...
//# sourceMappingURL=GestureCore.js.map