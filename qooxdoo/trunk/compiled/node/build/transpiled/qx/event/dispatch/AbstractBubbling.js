    ...
    members: {
      ...
      */
      ...
      dispatchEvent(target, event, type) {
        ...
        var self = this;
        var tracker = {};
        var __P_244_0 = false; //(event._type == "pointerup" && event._target.className === "qx-toolbar-button-checked");

        var __P_244_1 = function () {};
        ...
        qx.event.Utils.catch(tracker, function () {
          ...
          __P_244_1("Aborted serial=" + serial + ", type=" + event.getType());
        }); // capturing phase
        ...
        qx.event.Utils.then(tracker, function () {
          ...
          __P_244_1("captureList=" + captureList.length);

          return qx.event.Utils.series(captureList, function (localList, i) {
            __P_244_1("captureList[" + i + "]: localList.length=" + localList.length);
            ...
            var result = qx.event.Utils.series(localList, function (listener, listenerIndex) {
              ...
              if (!self._manager.isBlacklisted(listener.unique)) {
                __P_244_1("captureList[" + i + "] => localList[" + listenerIndex + "] callListener");

                return listener.handler.call(context, event);
              } else {
                __P_244_1("captureList[" + i + "] => localList[" + listenerIndex + "] is blacklisted");
              }
            }, true);
        ...
        qx.event.Utils.then(tracker, function () {
          ...
          __P_244_1("targetList=" + targetList.length);

          return qx.event.Utils.series(targetList, function (localList, i) {
            __P_244_1("targetList[" + i + "] localList.length=" + localList.length);

            var result = qx.event.Utils.series(localList, function (listener, listenerIndex) {
              __P_244_1("targetList[" + i + "] -> localList[" + listenerIndex + "] callListener");

              context = listener.context || target;

              __P_244_1("Calling target serial=" + serial + ", type=" + event.getType());
        ...
        qx.event.Utils.then(tracker, function () {
          ...
          __P_244_1("bubbleList=" + bubbleList.length);

          return qx.event.Utils.series(bubbleList, function (localList, i) {
            __P_244_1("bubbleList[" + i + "] localList.length=" + localList.length);

            var currentTarget = bubbleTargets[i];
            event.setCurrentTarget(currentTarget);
            var result = qx.event.Utils.series(localList, function (listener, listenerIndex) {
              __P_244_1("bubbleList[" + i + "] -> localList[" + listenerIndex + "] callListener");
        ...
        if (__P_244_0) {
          if (tracker.promise) {
            __P_244_1("events promised");

            qx.event.Utils.then(tracker, function () {
              __P_244_1("events promised done");
            });
          } else {
            __P_244_1("events done");
          }
        }
