  ...
  qx.Bootstrap.define("qx.event.Emitter", {
    ...
    members: {
      ...
      on(name, listener, ctx) {
        var id = qx.event.Emitter.__P_189_0.length;

        this.__P_189_3(name).push({
          listener: listener,
          ctx: ctx,
          id: id,
          name: name
        ...
        qx.event.Emitter.__P_189_0.push({
          name: name,
          listener: listener,
          ctx: ctx
        });
      ...
      off(name, listener, ctx) {
        var storage = this.__P_189_3(name);
        ...
        for (var i = storage.length - 1; i >= 0; i--) {
          ...
          if (entry.listener == listener && entry.ctx == ctx) {
            storage.splice(i, 1);
            qx.event.Emitter.__P_189_0[entry.id] = null;
            return entry.id;
          }
        }
       ...
       */
      offById(id) {
        var entry = qx.event.Emitter.__P_189_0[id];
       ...
       */
      emit(name, data) {
        var storage = this.__P_189_3(name).concat();
       ...
       */
      getEntryById(id) {
        for (var name in this.__P_189_1) {
          var store = this.__P_189_1[name];
       ...
       */
      __P_189_3(name) {
        if (this.__P_189_1 == null) {
          this.__P_189_1 = {};
        }

        if (this.__P_189_1[name] == null) {
          this.__P_189_1[name] = [];
        }

        return this.__P_189_1[name];
      }
