  ...
  qx.Class.define("qx.ui.core.queue.Manager", {
    statics: {
      ...
      scheduleFlush(job) {
        ...
        self.__P_181_2[job] = true;

        if (!self.__P_181_0) {
          self.__P_181_1 = false;
      ...
      flush() {
        ...
        self.__P_181_4 = true; // Cancel timeout if called manually

        self.__P_181_1 = true;
        var jobs = self.__P_181_2;

        self.__P_181_5(function () {
          // Process jobs
       ...
       */
      __P_181_5: qx.core.Environment.select("qx.debug", {
        ...
        false(callback, finallyCode) {
          ...
          try {
            ...
            if (self.__P_181_3 <= self.MAX_RETRIES) {
              self.scheduleFlush();
            } else {
              throw new Error("Fatal Error: Flush terminated " + (self.__P_181_3 - 1) + " times in a row" + " due to exceptions in user code. The application has to be reloaded!");
            }
    ...
    */
    defer(statics) {
      ...
      qx.event.Registration.addListener(window, "useraction", qx.core.Environment.get("event.touch") ? statics.__P_181_6 : statics.flush);
    }
