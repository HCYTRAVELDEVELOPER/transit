  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.io.ImageLoader", {
    statics: {
      ...
      __P_88_2: /\.(png|gif|jpg|jpeg|bmp)\b/i,

      /** @type {RegExp} Image types of a data URL */
      __P_88_3: /^data:image\/(png|gif|jpg|jpeg|bmp)\b/i,
       ...
       */
      isLoaded(source) {
        var entry = this.__P_88_0[source];
      ...
      getFormat(source) {
        ...
        if (!entry || !entry.format) {
          var result = this.__P_88_3.exec(source);

          if (result != null) {
            // If width and height aren't defined, provide some defaults
            var width = entry && qx.lang.Type.isNumber(entry.width) ? entry.width : this.__P_88_1.width;
            var height = entry && qx.lang.Type.isNumber(entry.height) ? entry.height : this.__P_88_1.height;
            entry = {
              loaded: true,
              format: result[1],
              width: width,
      ...
      load(source, callback, context) {
        ...
        if (entry.loaded || entry.loading || entry.failed) {
          ...
          var boundCallback = qx.lang.Function.listener(this.__P_88_4, this, el, source); // Assign callback to element
      ...
      __P_88_4() {
        var callback = qx.core.Environment.select("qx.globalErrorHandling", {
          true: qx.event.GlobalError.observeMethod(this.__P_88_5),
          false: this.__P_88_5
        });
       ...
       */
      __P_88_5(event, element, source) {
        // Shorthand
        var entry = this.__P_88_0[source]; // [BUG #9149]: When loading a SVG IE11 won't have
        ...
        if (event.type === "load" && isImageAvailable(element)) {
          ...
          var result = this.__P_88_2.exec(source);
