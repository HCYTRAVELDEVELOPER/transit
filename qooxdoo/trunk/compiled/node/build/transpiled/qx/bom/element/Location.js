  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.bom.element.Location", {
    statics: {
       ...
       */
      __P_225_3: qx.core.Environment.select("engine.name", {
        mshtml(elem) {
          // Find body element
          ...
          if (!qx.core.Environment.get("browser.quirksmode")) {
            left += this.__P_225_1(body, "borderLeftWidth");
            top += this.__P_225_1(body, "borderTopWidth");
          }
      ...
      get(elem, mode) {
        if (elem.tagName == "BODY") {
          var location = this.__P_225_5(elem);

          var left = location.left;
          var top = location.top;
        } else {
          var body = this.__P_225_3(elem);

          var offset = this.__P_225_4(elem); // Reduce by viewport scrolling.
          // Hint: getBoundingClientRect returns the location of the
          // element in relation to the viewport which includes
          // the scrolling


          var scroll = this.__P_225_2(elem);
        ...
        if (mode) {
          ...
          if (mode == "padding" || mode == "scroll") {
            ...
            if (overX == "scroll" || overX == "auto") {
              right += elem.scrollWidth - elementWidth + this.__P_225_1(elem, "borderLeftWidth") + this.__P_225_1(elem, "borderRightWidth");
            }

            var overY = qx.bom.element.Style.get(elem, "overflowY");

            if (overY == "scroll" || overY == "auto") {
              bottom += elem.scrollHeight - elementHeight + this.__P_225_1(elem, "borderTopWidth") + this.__P_225_1(elem, "borderBottomWidth");
            }
          }

          switch (mode) {
            case "padding":
              left += this.__P_225_1(elem, "paddingLeft");
              top += this.__P_225_1(elem, "paddingTop");
              right -= this.__P_225_1(elem, "paddingRight");
              bottom -= this.__P_225_1(elem, "paddingBottom");
            // no break here
            ...
            case "border":
              left += this.__P_225_1(elem, "borderLeftWidth");
              top += this.__P_225_1(elem, "borderTopWidth");
              right -= this.__P_225_1(elem, "borderRightWidth");
              bottom -= this.__P_225_1(elem, "borderBottomWidth");
              break;

            case "margin":
              left -= this.__P_225_1(elem, "marginLeft");
              top -= this.__P_225_1(elem, "marginTop");
              right += this.__P_225_1(elem, "marginRight");
              bottom += this.__P_225_1(elem, "marginBottom");
              break;
          }
        }
      ...
      __P_225_5(body) {
        var top = body.offsetTop;
        var left = body.offsetLeft;
        top += this.__P_225_1(body, "marginTop");
        left += this.__P_225_1(body, "marginLeft");

        if (qx.core.Environment.get("engine.name") === "gecko") {
          top += this.__P_225_1(body, "borderLeftWidth");
          left += this.__P_225_1(body, "borderTopWidth");
        }
