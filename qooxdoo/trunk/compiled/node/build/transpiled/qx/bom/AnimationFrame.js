  ...
  qx.Bootstrap.define("qx.bom.AnimationFrame", {
    ...
    members: {
      ...
      startSequence(duration) {
        this.__P_202_0 = false;
        var start = window.performance && performance.now ? performance.now() + qx.bom.AnimationFrame.__P_202_1 : Date.now();
    ...
    statics: {
      ...
      request(callback, context) {
        ...
        var cb = function (time) {
          ...
          if (time < 1e10) {
            time = qx.bom.AnimationFrame.__P_202_1 + time;
          }
    ...
    defer(statics) {
      ...
      statics.__P_202_1 = window.performance && performance.timing && performance.timing.navigationStart; // if not, simply use the current time

      if (!statics.__P_202_1) {
        statics.__P_202_1 = Date.now();
      }
    }
