  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.bom.element.AnimationJs", {
    statics: {
       ...
       */
      __P_262_1: ["%", "in", "cm", "mm", "em", "ex", "pt", "pc", "px"],
      ...
      _animate(el, desc, duration, reverse) {
        ...
        var keys = this.__P_262_3(keyFrames);

        var stepTime = this.__P_262_4(duration, keys);

        var steps = parseInt(duration / stepTime, 10);

        this.__P_262_5(keyFrames, el);

        var delta = this.__P_262_6(steps, stepTime, keys, keyFrames, duration, desc.timing);
        ...
        handle.i = 0;
        handle.initValues = {};
        handle.repeatSteps = this.__P_262_7(steps, desc.repeat);
          ...
          for (var name in keyFrames[percent]) {
            ...
            if (units[name] == undefined) {
              ...
              if (typeof item == "string") {
                units[name] = this.__P_262_8(item);
              } else {
                units[name] = "";
              }
            }
        ...
        for (var percent in keyFrames) {
          ...
          for (var name in units) {
            if (frame[name] == undefined) {
              ...
              if (frame[name] === "" && this.__P_262_1.indexOf(units[name]) != -1) {
       ...
       */
      __P_262_6(steps, stepTime, keys, keyFrames, duration, timing) {
        var delta = new Array(steps);
        var keyIndex = 1;
        delta[0] = this.__P_262_9(keyFrames[0]);
            ...
            if (name in this.__P_262_2) {
              ...
              if (qx.Bootstrap.isArray(last[name])) {
                ...
                for (var j = 0; j < next[name].length; j++) {
                  ...
                  transforms[name][j] = this.__P_262_10(item, last[name], timing, x);
                }
              } else {
                var x = calculationIndex / stepsToNext;
                transforms[name] = this.__P_262_10(nItem, last[name], timing, x);
              } // color values
              ...
              var x = calculationIndex / stepsToNext;
              delta[i][name] = this.__P_262_10(nItem, last[name], timing, x);
            } else {
        ...
        delta[delta.length - 1] = this.__P_262_9(keyFrames[100]);
        return delta;
      },
       ...
       */
      __P_262_10(nextItem, lastItem, timing, x) {
        var range = parseFloat(nextItem) - parseFloat(lastItem);
        return parseFloat(lastItem) + range * qx.bom.AnimationFrame.calculateTiming(timing, x) + this.__P_262_8(nextItem);
      },
      ...
      play(handle) {
        ...
        var id = window.setInterval(function () {
          ...
          qx.bom.element.AnimationJs.__P_262_11(handle.el, values);
      ...
      stop(handle) {
        ...
        if (keep != undefined && !handle.stopped) {
          ...
          this.__P_262_11(el, this.__P_262_9(desc.keyFrames[keep]));
        } else {
          this.__P_262_11(el, initValues);
        }
...
//# sourceMappingURL=AnimationJs.js.map