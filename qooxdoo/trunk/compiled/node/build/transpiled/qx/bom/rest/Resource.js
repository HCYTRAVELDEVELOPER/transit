  ...
  qx.Bootstrap.define("qx.bom.rest.Resource", {
    ...
    members: {
      ...
      getRequestsByAction(action) {
        var hasRequests = this.__P_171_0 !== null && action in this.__P_171_0;
        return hasRequests ? this.__P_171_0[action] : null;
      },
       ...
       */
      _getRequest() {
        return this.__P_171_8 === null ? new qx.bom.request.SimpleXhr() : this.__P_171_8();
      },
      ...
      __P_171_9(action) {
        ...
        if (!qx.lang.Type.isArray(this.__P_171_0[action])) {
          this.__P_171_0[action] = [];
        }

        qx.core.ObjectRegistry.register(req);

        this.__P_171_0[action].push(req);
      ...
      map(action, method, url, check) {
        this.__P_171_1[action] = [method, url, check]; // Track requests

        this.__P_171_0[action] = []; // Undefine generic getter when action is named "get"
        ...
        this.__P_171_10(action + "Success");

        this.__P_171_10(action + "Error");
      ...
      invoke(action, params, data) {
        var req = this.__P_171_9(action),
            params = params == null ? {} : params,
            config = this._getRequestConfig(action, params); // Cache parameters


        this.__P_171_1[action].params = params; // Check parameters

        this.__P_171_11(params, config.check); // Configure request


        this.__P_171_12(req, config, data); // Run configuration callback, passing in pre-configured request


        if (this.__P_171_6) {
          this.__P_171_6.call(this, req, action, params, data);
        } // Configure JSON request (content type may have been set in configuration callback)


        this.__P_171_13(req, config, data);
      ...
      abort(varargs) {
        if (qx.lang.Type.isNumber(varargs)) {
          ...
          var action = varargs;
          var reqs = this.__P_171_0[action];

          if (this.__P_171_0[action]) {
       ...
       */
      refresh(action) {
        this.invoke(action, this.__P_171_1[action].params);
      },
      ...
      poll(action, interval, params, immediately) {
        ...
        var intervalListener = function (scope) {
          return function () {
            var req = scope.__P_171_0[action][0];
      ...
      stopPollByAction(action) {
        if (action in this.__P_171_2) {
          var intervalId = this.__P_171_2[action].id;
      ...
      restartPollByAction(action) {
        if (action in this.__P_171_2) {
          var timer = this.__P_171_2[action];
          this.stopPollByAction(action);
      ...
      longPoll(action) {
        ...
        var handlerId = this.__P_171_3[action] = this.addListener(action + "Success", function longPollHandler() {
          if (res.isDisposed()) {
            return;
          }
      ...
      _getRequestConfig(action, params) {
        var route = this.__P_171_1[action]; // Not modify original params
        ...
        var method = route[0],
            url = this.__P_171_5 !== null ? this.__P_171_5 + route[1] : route[1],
            check = route[2],
      ...
      */
      ...
      destruct() {
        ...
        for (action in this.__P_171_0) {
          if (this.__P_171_0[action]) {
            this.__P_171_0[action].forEach(function (req) {
              req.dispose();
            });
          }
        }
        ...
        if (this.__P_171_2) {
          for (action in this.__P_171_2) {
            this.stopPollByAction(action);
          }
        }

        if (this.__P_171_3) {
          for (action in this.__P_171_3) {
            var id = this.__P_171_3[action];
            this.removeListenerById(id);
          }
        }

        this.__P_171_0 = this.__P_171_1 = this.__P_171_2 = null;
      }
