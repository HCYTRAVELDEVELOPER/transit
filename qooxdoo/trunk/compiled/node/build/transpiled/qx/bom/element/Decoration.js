  ...
  qx.Class.define("qx.bom.element.Decoration", {
    ...
    */
    statics: {
      ...
      __P_241_1: qx.core.Environment.select("engine.name", {
        mshtml: {
          "scale-x": true,
          "scale-y": true,
          scale: true,
      ...
      getTagName(repeat, source) {
        if (source && qx.core.Environment.get("css.alphaimageloaderneeded") && this.__P_241_1[repeat] && source.endsWith(".png")) {
          return "div";
        }

        return this.__P_241_2[repeat];
      },
      ...
      getAttributes(source, repeat, style) {
        ...
        if (qx.core.Environment.get("css.alphaimageloaderneeded") && this.__P_241_1[repeat] && format === "png") {
          var dimension = this.__P_241_3(source);

          this.__P_241_4(style, dimension.width, dimension.height);
          ...
          if (repeat === "scale") {
            result = this.__P_241_5(style, repeat, source);
          } else if (repeat === "scale-x" || repeat === "scale-y") {
            result = this.__P_241_6(style, repeat, source);
          } else {
            // Native repeats or "no-repeat"
            result = this.__P_241_7(style, repeat, source);
          }
        }
       ...
       * @return {Map} image URI and style infos
       */
      __P_241_6(style, repeat, sourceid) {
        var ResourceManager = qx.util.ResourceManager.getInstance();
        var clipped = ResourceManager.getCombinedFormat(sourceid);

        var dimension = this.__P_241_3(sourceid);
        ...
        if (clipped) {
          ...
          if (repeat === "scale-x") {
            style = this.__P_241_8(style, data, dimension.height);
          } else {
            style = this.__P_241_9(style, data, dimension.width);
          }
       ...
       * @return {Map} image URI and style infos
       */
      __P_241_7(style, repeat, sourceid) {
        var ResourceManager = qx.util.ResourceManager.getInstance();
        var clipped = ResourceManager.getCombinedFormat(sourceid);

        var dimension = this.__P_241_3(sourceid); // Double axis repeats cannot be clipped
        ...
        if (clipped && repeat !== "repeat") {
          ...
          this.__P_241_10(style, sourceid, repeat);
       ...
       * @param source {String} image source
       */
      __P_241_10(style, source, repeat) {
