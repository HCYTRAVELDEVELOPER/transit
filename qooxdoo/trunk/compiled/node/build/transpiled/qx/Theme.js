    ...
    statics: {
      ...
      */
      ...
      define(name, config) {
        ...
        config.include = this.__P_152_0(config.include);
        config.patch = this.__P_152_0(config.patch); // Validate incoming data
        ...
        this.__P_152_1(theme, config);

        this.__P_152_2(theme, config); // Store class reference in global class registry
      ...
      */
       ...
       */
      __P_152_3(config) {
        for (var i = 0, keys = this.__P_152_4, l = keys.length; i < l; i++) {
          if (config[keys[i]]) {
            return keys[i];
          }
        }
       ...
       */
      __P_152_1(theme, config) {
        var type = this.__P_152_3(config); // Use theme key from extended theme if own one is not available
      ...
      __P_152_4: ["colors", "borders", "decorations", "fonts", "icons", "widgets", "appearances", "meta"],

      /** @type {Map} allowed keys in theme definition */
      __P_152_5: qx.core.Environment.select("qx.debug", {
        true: {
          title: "string",
          // String
          aliases: "object",
      ...
      __P_152_6: qx.core.Environment.select("qx.debug", {
        true: {
          color: "object",
          border: "object",
          decoration: "object",
       ...
       */
      __P_152_7: qx.core.Environment.select("qx.debug", {
        true(name, config) {
          var allowed = this.__P_152_5;
      ...
      patch(theme, mixinTheme) {
        this.__P_152_8(mixinTheme);

        var type = this.__P_152_3(mixinTheme);

        if (type !== this.__P_152_3(theme)) {
