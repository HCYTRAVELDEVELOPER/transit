  ...
  qx.Class.define("qx.data.controller.Form", {
    ...
    members: {
      ...
      addBindingOptions(name, model2target, target2model) {
        this.__P_76_0[name] = [model2target, target2model]; // return if not both, model and target are given
        ...
        var targetProperty = this.__P_76_2(item) ? "modelSelection[0]" : "value"; // remove the binding

        this.__P_76_1.removeTarget(item, targetProperty, name); // set up the new binding with the options


        this.__P_76_1.addTarget(item, targetProperty, name, !this._selfUpdate, model2target, target2model);
      },
        ...
        for (var name in items) {
          ...
          for (var i = 0; i < names.length; i++) {
            ...
            if (i + 1 == names.length) {
              ...
              if (this.__P_76_0[name] && this.__P_76_0[name][1]) {
                itemValue = this.__P_76_0[name][1].converter(itemValue);
              }
      ...
      updateModel() {
        ...
        for (var name in items) {
          var item = items[name];
          var sourceProperty = this.__P_76_2(item) ? "modelSelection[0]" : "value";
          var options = this.__P_76_0[name];
          options = options && this.__P_76_0[name][1];
      ...
      _applyModel(value, old) {
        ...
        if (this.__P_76_1 != null && value == null) {
          this.__P_76_1.setModel(null);
        } // first, get rid off all bindings (avoids wrong data population)


        if (this.__P_76_1 != null && this.getTarget() != null) {
          var items = this.getTarget().getItems();

          for (var name in items) {
            var item = items[name];
            var targetProperty = this.__P_76_2(item) ? "modelSelection[0]" : "value";

            this.__P_76_1.removeTarget(item, targetProperty, name);
          }
        } // set the model of the object controller if available


        if (this.__P_76_1 != null) {
          this.__P_76_1.setModel(value);
      ...
      __P_76_4() {
        ...
        if (this.__P_76_1 == null) {
          this.__P_76_1 = new qx.data.controller.Object(this.getModel());
        } // get the form items
        ...
        for (var name in items) {
          var item = items[name];
          var targetProperty = this.__P_76_2(item) ? "modelSelection[0]" : "value";
          var options = this.__P_76_0[name]; // try to bind all given items in the form

          try {
            if (options == null) {
              this.__P_76_1.addTarget(item, targetProperty, name, !this._selfUpdate);
            } else {
              this.__P_76_1.addTarget(item, targetProperty, name, !this._selfUpdate, options[0], options[1]);
