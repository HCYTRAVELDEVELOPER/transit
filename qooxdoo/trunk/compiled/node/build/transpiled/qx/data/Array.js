  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.data.Array", {
    ...
    construct(param) {
      ...
      if (param == undefined) {
        this.__P_73_0 = []; // check for elements (create the array)
      } else if (arguments.length > 1) {
        // create an empty array and go through every argument and push it
        this.__P_73_0 = [];

        for (var i = 0; i < arguments.length; i++) {
          this.__P_73_0.push(arguments[i]);
        } // check for a number (length)

      } else if (typeof param == "number") {
        this.__P_73_0 = new Array(param); // check for an array itself
      } else if (param instanceof Array) {
        this.__P_73_0 = qx.lang.Array.clone(param); // error case
      } else {
        this.__P_73_0 = [];
        this.dispose();
      ...
      for (var i = 0; i < this.__P_73_0.length; i++) {
        this._applyEventPropagation(this.__P_73_0[i], null, i);
      } // update the length at startup


      this.__P_73_1(); // work against the console printout of the array
    ...
    */
    ...
    members: {
      ...
      concat(array) {
        ...
        if (array) {
          var newArray = this.__P_73_0.concat(array);
        } else {
          var newArray = this.__P_73_0.concat();
        }
       ...
       */
      join(connector) {
        return this.__P_73_0.join(connector);
      },
       ...
       */
      pop() {
        var item = this.__P_73_0.pop();

        this.__P_73_1(); // remove the possible added event listener
      ...
      push(varargs) {
        for (var i = 0; i < arguments.length; i++) {
          ...
          this.__P_73_1(); // apply to every pushed item an event listener for the bubbling
      ...
      reverse() {
        ...
        var oldArray = this.__P_73_0.concat();

        this.__P_73_0.reverse();

        this.__P_73_2(0, this.length);
      ...
      shift() {
        ...
        this.__P_73_2(0, this.length); // fire change bubbles event
       ...
       */
      slice(from, to) {
        return new qx.data.Array(this.__P_73_0.slice(from, to));
      },
      ...
      splice(startIndex, amount, varargs) {
        ...
        var oldLength = this.__P_73_0.length; // invoke the slice on the array

        var returnArray = this.__P_73_0.splice.apply(this.__P_73_0, arguments); // fire a change event for the length


        if (this.__P_73_0.length != oldLength) {
          this.__P_73_1();
        ...
        this.__P_73_2(startIndex + (arguments.length - 2) - amount, this.length); // fire the changeBubble event
      ...
      sort(func) {
        ...
        this.__P_73_0.sort.apply(this.__P_73_0, arguments); // prevent changeBubble event if nothing has been changed


        if (qx.lang.Array.equals(this.__P_73_0, oldArray) === true) {
          return;
        }
      ...
      unshift(varargs) {
        for (var i = arguments.length - 1; i >= 0; i--) {
          this.__P_73_0.unshift(arguments[i]);

          this.__P_73_1(); // apply to every item an event listener for the bubbling
       ...
       */
      setItem(index, item) {
        var oldItem = this.__P_73_0[index]; // ignore settings of already set items [BUG #4106]

        if (oldItem === item) {
          return;
        }

        this.__P_73_0[index] = item; // set an event listener for the bubbling

        this._registerEventChaining(item, oldItem, index); // only update the length if its changed


        if (this.length != this.__P_73_0.length) {
          this.__P_73_1();
       ...
       */
      indexOf(item) {
        return this.__P_73_0.indexOf(item);
      },
       ...
       */
      lastIndexOf(item) {
        return this.__P_73_0.lastIndexOf(item);
      },
       ...
       */
      toString() {
        if (this.__P_73_0 != null) {
          return this.__P_73_0.toString();
        }
      ...
      */
       ...
       */
      includes(item) {
        return this.__P_73_0.indexOf(item) !== -1;
      },
      ...
      removeAll() {
        ...
        for (var i = 0; i < this.__P_73_0.length; i++) {
          this._registerEventChaining(null, this.__P_73_0[i], i);
        ...
        var items = this.__P_73_0.concat(); // change the length


        this.__P_73_0.length = 0;

        this.__P_73_1(); // fire change bubbles event
      ...
      append(array) {
        ...
        var oldLength = this.__P_73_0.length;
        Array.prototype.push.apply(this.__P_73_0, array); // add a listener to the new items
        ...
        this.__P_73_1(); // fire change bubbles
      ...
      forEach(callback, context) {
        this.__P_73_0.forEach((element, index) => callback.call(context, element, index, this));
      },
      ...
      */
      ...
      filter(callback, self) {
        return new qx.data.Array(this.__P_73_0.filter(callback, self));
      },
       ...
       */
      map(callback, self) {
        return new qx.data.Array(this.__P_73_0.map(callback, self));
      },
       ...
       */
      some(callback, self) {
        return this.__P_73_0.some(callback, self);
      },
       ...
       */
      every(callback, self) {
        return this.__P_73_0.every(callback, self);
      },
      ...
      reduce(callback, initValue) {
        return this.__P_73_0.reduce(callback, initValue);
      },
      ...
      reduceRight(callback, initValue) {
        return this.__P_73_0.reduceRight(callback, initValue);
      },
      ...
      */
      ...
      __P_73_1() {
        var oldLength = this.length;
        this.length = this.__P_73_0.length;
      ...
      __P_73_2(from, to) {
        for (var i = from; i < to; i++) {
          this._registerEventChaining(this.__P_73_0[i], this.__P_73_0[i], i);
        }
      }
