...
Framework: v${await this.getQxVersion()} in ${await this.getQxPath()}`);
        ...
        if (this.argv["machine-readable"]) {
          ...
          if (this.argv["feedback"]) {
            ...
            this.__P_8_0 = new Gauge();

            this.__P_8_0.setTheme(ourTheme);

            this.__P_8_0.show("Compiling", 0);
        ...
        if (this.__P_8_0) {
          this.addListener("writingApplications", () => this.__P_8_0.show("Writing Applications", 0));
          this.addListener("writtenApplications", () => this.__P_8_0.show("Writing Applications", 1));
          this.addListener("writingApplication", evt => this.__P_8_0.pulse("Writing Application " + evt.getData().appMeta.getApplication().getName()));
          this.addListener("compilingClass", evt => this.__P_8_0.pulse("Compiling " + evt.getData().classFile.getClassName()));
          this.addListener("minifyingApplication", evt => this.__P_8_0.pulse("Minifying " + evt.getData().application.getName() + " " + evt.getData().filename));
        } else {
        ...
        if (!this.argv.watch) {
          let success = this.__P_8_1.every(maker => maker.getSuccess());

          let hasWarnings = this.__P_8_1.every(maker => maker.getHasWarnings());
       ...
       */
      getMaker() {
        if (this.__P_8_1.length == 1) {
          return this.__P_8_1[0];
        }

        throw new Error("Cannot get a single maker - there are " + this.__P_8_1.length + " available");
      },
      ...
      getMakersForApp(appName) {
        return this.__P_8_1.filter(maker => {
