    ...
    members: {
      ...
      async process() {
        ...
        for (let target of compileConfig.getValue("targets")) {
          await this.__P_7_0(path.join(process.cwd(), target.outputPath));

          if (target.deployPath) {
            await this.__P_7_0(path.join(process.cwd(), target.deployPath));
          }
        }

        await this.__P_7_0(path.join(process.cwd(), qx.tool.cli.commands.Package.cache_dir)); // check if we have to migrate files

        await this.checkMigrations();
      },

      async __P_7_0(pathToRemove) {
