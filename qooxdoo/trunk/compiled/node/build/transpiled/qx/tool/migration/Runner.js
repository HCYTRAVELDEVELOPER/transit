  ...
  qx.Class.define("qx.tool.migration.Runner", {
    ...
    members: {
      ...
      async runMigrations() {
        ...
        let migrationClasses = Object.getOwnPropertyNames(qx.tool.migration).filter(clazz => clazz.match(/^M[0-9_]+$/)).map(clazz => qx.Class.getByName("qx.tool.migration." + clazz));
        let applied = 0;
        let pending = 0;
        ...
        for (let Clazz of migrationClasses) {
          ...
          let skip = appQxVersion && !semver.lt(appQxVersion, migrationVersion) || qxVersion && semver.gt(migrationVersion, qxVersion);
          ...
          let migrationMethods = Object.getOwnPropertyNames(Clazz.prototype).filter(key => key.startsWith("migrate")).filter(key => typeof Clazz.prototype[key] == "function");
