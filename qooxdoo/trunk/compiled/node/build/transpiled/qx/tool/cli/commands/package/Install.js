  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.tool.cli.commands.package.Install", {
    ...
    members: {
      ...
      async process() {
        ...
        if (!this.argv.uri && !this.argv.fromPath) {
          if (lockfileModel.getValue("libraries").length) {
            ...
            await this.__P_16_3(manifestModel.getData());
        ...
        if (this.argv.fromPath) {
          ...
          await this.__P_16_4(uri, this.argv.fromPath, saveToManifest);
        } else if (!id || qx.lang.Type.isString(id) && id.startsWith("v")) {
          // install library/libraries from GitHub release
          await this.__P_16_5(uri, id, this.argv.save);
        } else {
          // install library from GitHub code tree
          await this.__P_16_6(uri, id, this.argv.save);
        }
       ...
       * @private
       */
      async __P_16_5(uri, tag_name, writeToManifest) {
        ...
        let {
          download_path
        } = await this.__P_16_8(repo_name, tag_name); // iterate over contained libraries
        ...
        for (let {
          ...
          found = true;
          await this.__P_16_9(library_uri, tag_name, download_path, writeToManifest);
        }
       ...
       * @private
       */
      async __P_16_6(uri, hash, writeToManifest) {
        ...
        let {
          download_path
        } = await this.__P_16_8(repo_name, hash);
        await this.__P_16_9(uri, hash, download_path, writeToManifest);
      },
       ...
       * @private
       */
      async __P_16_4(uri, dir, writeToManifest = false) {
        let qxVersion = await this.getAppQxVersion();
        ...
        await this.__P_16_9(uri, undefined, dir, writeToManifest);
      },
       ...
       * @private
       */
      async __P_16_9(uri, id, download_path, writeToManifest) {
        let {
          repo_name,
          package_path
        } = uri ? this.__P_16_7(uri) : {
          repo_name: "",
          package_path: ""
        };
        ...
        let appsInstalled = await this.__P_16_10(library_path);

        if (!appsInstalled && this.argv.verbose) {
          qx.tool.compiler.Console.info(`>>> No applications installed for ${uri}.`);
        }

        let depsInstalled = await this.__P_16_11(library_path);
            ...
            default:
              {
                ...
                if (semver.validRange(lib_range)) {
                  let {
                    tag
                  } = this.__P_16_12(lib_uri, lib_range);
                  ...
                  if (!(await this.isInstalled(lib_uri, tag))) {
                    await this.__P_16_5(lib_uri, tag, false);
                    break;
                  }
                ...
                if (!(await this.isInstalled(lib_uri, lib_range))) {
                  try {
                    await this.__P_16_6(lib_uri, lib_range, false);
                    break;
                  } catch (e) {
       ...
       * @private
       */
      __P_16_12(lib_uri, lib_range) {
        let {
          repo_name
        } = this.__P_16_7(lib_uri);
      ...
      async __P_16_2() {
        ...
        return qx.Promise.all(libraries.filter(lib => lib.repo_name && lib.repo_tag).map(lib => this.__P_16_8(lib.repo_name, lib.repo_tag)));
      },
       ...
       */
      async __P_16_8(repo_name, treeish = null, force = false) {
