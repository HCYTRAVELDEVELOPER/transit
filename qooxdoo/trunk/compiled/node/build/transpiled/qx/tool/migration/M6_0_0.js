  ...
  qx.Class.define("qx.tool.migration.M6_0_0", {
    ...
    members: {
      ...
      async migrateConfigFiles() {
        ...
        let cwd = process.cwd(); // rename configuration files from initial names
        // replace those static variables with verbatims

        let migrateFiles = [[path.join(cwd, pkg.lockfile.filename), path.join(cwd, "contrib.json")], [path.join(cwd, pkg.cache_dir), path.join(cwd, "contrib")], [path.join(qx.tool.cli.ConfigDb.getDirectory(), pkg.package_cache_name), path.join(qx.tool.cli.ConfigDb.getDirectory(), "contrib-cache.json")]]; // change names in .gitignore
      ...
      async migrateManifest() {
        let dryRun = this.getRunner().getDryRun();
        let verbose = this.getRunner().getVerbose(); // Update all Manifests
        ...
        for (const manifestModel of await qx.tool.config.Utils.getManifestModels()) {
          ...
          await this.updateSchemaUnlessDryRun(manifestModel, "https://qooxdoo.org/schema/Manifest-1-0-0.json"); // update qooxdoo version

          await this.updateQxDependencyUnlessDryRun(manifestModel); // save Manifest file
      ...
      async migrateCompileJson() {
        ...
        let newEsLintExtends = ["@qooxdoo/qx/browser", "@qooxdoo/qx", "@qooxdoo/jsdoc-disable"];
...
//# sourceMappingURL=M6_0_0.js.map