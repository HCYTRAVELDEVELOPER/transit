  ...
  qx.Class.define("qx.tool.compiler.app.Library", {
    ...
    members: {
      ...
      loadManifest(loadFromDir) {
        ...
        return this.__P_27_3 = this.__P_27_4(loadFromDir);
      },

      async __P_27_4(loadFromDir) {
            ...
            async.each(filenames, function (filename, cb) {
              ...
              fs.stat(path.join(folder, filename), function (err, stat) {
                ...
                if (extension == ".js" || extension == ".ts") {
                  t.__P_27_0[className] = "class";
                  t.__P_27_1[className] = extension;
                  classes.push(className);
                } else {
                  t.__P_27_0[filename] = "resource";
                }

                if (Boolean(packageName) && !t.__P_27_0[packageName]) {
                  t.__P_27_0[packageName] = "package";
                  var pos;
                  tmp = packageName;

                  while ((pos = tmp.lastIndexOf(".")) > -1) {
                    tmp = tmp.substring(0, pos);
                    t.__P_27_0[tmp] = "package";
                  }
                }
      ...
      getSymbolType(name) {
        ...
        var t = this;
        var type = this.__P_27_0[name];

        if (type) {
          return {
            symbolType: t.__P_27_0[name],
        ...
        let result = testEnvironment(this.__P_27_2[name]);

        if (result) {
          return result;
        }

        for (let key in this.__P_27_2) {
          let check = this.__P_27_2[key];
       ...
       */
      isClass(classname) {
        var type = this.__P_27_0[classname];
        return type === "class";
      },
      ...
      getSourceFileExtension(className) {
        return this.__P_27_1[className];
      },
