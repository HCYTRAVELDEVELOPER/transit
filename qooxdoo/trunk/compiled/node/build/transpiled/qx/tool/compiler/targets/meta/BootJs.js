  ...
  qx.Class.define("qx.tool.compiler.targets.meta.BootJs", {
    ...
    members: {
      ...
      addEmbeddedJs(jsMeta) {
        if (!this.__P_43_1[jsMeta.toHashCode()]) {
          this.__P_43_0.push(jsMeta);

          this.__P_43_1[jsMeta.toHashCode()] = jsMeta;
        }
      },
      ...
      async writeSourceCodeToStream(ws) {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          while (match = line.match(/\%\{([^}]+)\}/)) {
            ...
            if (keyword == "BootPart") {
              for (let j = 0; j < this.__P_43_0.length; j++) {
                this.__P_43_2.push(ws.getLineNumber());

                await this.__P_43_0[j].unwrap().writeSourceCodeToStream(ws);
                ws.write("\n");
              }
