  ...
  qx.Class.define("qx.util.format.DateFormat", {
    ...
    */
    ...
    construct(format, locale) {
      ...
      this.__P_141_0 = this.__P_141_1 = locale;

      if (format != null) {
        this.__P_141_2 = format.toString();

        if (this.__P_141_2 in qx.util.format.DateFormat.ISO_MASKS) {
          if (this.__P_141_2 === "isoUtcDateTime") {
            this.__P_141_3 = true;
          }

          this.__P_141_2 = qx.util.format.DateFormat.ISO_MASKS[this.__P_141_2];
        }
      } else {
        this.__P_141_2 = qx.locale.Date.getDateFormat("long", this.getLocale()) + " " + qx.locale.Date.getDateTimeFormat("HHmmss", "HH:mm:ss", this.getLocale());
      }
    },
    ...
    */
    statics: {
      ...
      getDateInstance() {
        ...
        if (DateFormat._dateInstance == null || DateFormat._dateInstance.__P_141_2 != format) {
      ...
      getDateTimeInstance() {
        ...
        if (DateFormat._dateTimeInstance == null || DateFormat._dateTimeInstance.__P_141_2 != format) {
    ...
    */
    members: {
      ...
      __P_141_10(date) {
        ...
        var thursdayDate = this.__P_141_9(date); // Get the year the thursday (and therefore the week) belongs to


        var weekYear = thursdayDate.getFullYear(); // Get the thursday of the week january 4th belongs to
        // (which defines week 1 of a year)

        var thursdayWeek1 = this.__P_141_9(new Date(weekYear, 0, 4)); // Calculate the calendar week
       ...
       */
      __P_141_11(date) {
        var thursdayDate = this.__P_141_9(date);

        var thursdayWeek1 = this.__P_141_9(new Date(date.getFullYear(), date.getMonth(), 4));
       ...
       */
      __P_141_15(weekYear, month, dayOfMonth) {
        var year;
        ...
        switch (month) {
          case 11:
            ...
            if (weekYear != this.__P_141_12(new Date(year, month, dayOfMonth))) {
              year = weekYear;
            }
      ...
      setLocale(value) {
        ...
        this.__P_141_1 = value === null ? this.__P_141_0 : value;
      },
      ...
      format(date) {
        ...
        for (var i = 0; i < this.__P_141_6.length; i++) {
          var currAtom = this.__P_141_6[i];
          ...
          if (currAtom.type == "literal") {
            ...
            switch (wildcardChar) {
              case "y":
                // Year
                if (wildcardSize == 2) {
                  replacement = this.__P_141_7(fullYear % 100, 2);
                } else {
              ...
              case "Y":
                // Year
                replacement = this.__P_141_12(date) + "";
              ...
              case "Q":
                ...
                if (wildcardSize == 1 || wildcardSize == 2) {
                  replacement = this.__P_141_7(parseInt(month / 4) + 1, wildcardSize);
                }
              ...
              case "D":
                // Day in year (e.g. 189)
                replacement = this.__P_141_7(this.__P_141_8(date), wildcardSize);
                break;

              case "d":
                // Day in month
                replacement = this.__P_141_7(dayOfMonth, wildcardSize);
                break;

              case "w":
                // Week in year (e.g. 27)
                replacement = this.__P_141_7(this.__P_141_10(date), wildcardSize);
                break;

              case "W":
                // Week in year (e.g. 27)
                replacement = this.__P_141_11(date);
                break;
              ...
              case "e":
                ...
                if (wildcardSize >= 1 && wildcardSize <= 2) {
                  replacement = this.__P_141_7(localeDayOfWeek, wildcardSize);
              ...
              case "M":
                // Month
                if (wildcardSize == 1 || wildcardSize == 2) {
                  replacement = this.__P_141_7(month + 1, wildcardSize);
                } else if (wildcardSize == 3) {
              ...
              case "H":
                // Hour in day (0-23)
                replacement = this.__P_141_7(hours, wildcardSize);
                break;

              case "k":
                // Hour in day (1-24)
                replacement = this.__P_141_7(hours == 0 ? 24 : hours, wildcardSize);
                break;

              case "K":
                // Hour in am/pm (0-11)
                replacement = this.__P_141_7(hours % 12, wildcardSize);
                break;

              case "h":
                // Hour in am/pm (1-12)
                replacement = this.__P_141_7(hours % 12 == 0 ? 12 : hours % 12, wildcardSize);
                break;

              case "m":
                // Minute in hour
                replacement = this.__P_141_7(minutes, wildcardSize);
                break;

              case "s":
                // Second in minute
                replacement = this.__P_141_7(seconds, wildcardSize);
                break;

              case "S":
                // Fractional second
                replacement = this.__P_141_7(ms, 3);
              ...
              case "z":
                ...
                if (wildcardSize >= 1 && wildcardSize <= 4) {
                  replacement = "GMT" + (timezoneSign > 0 ? "-" : "+") + this.__P_141_7(Math.abs(timezoneHours), 2) + ":" + this.__P_141_7(timezoneMinutes, 2);
                }
              ...
              case "Z":
                // RFC 822 time zone
                if (wildcardSize >= 1 && wildcardSize <= 3) {
                  replacement = (timezoneSign > 0 ? "-" : "+") + this.__P_141_7(Math.abs(timezoneHours), 2) + this.__P_141_7(timezoneMinutes, 2);
                } else {
                  replacement = "GMT" + (timezoneSign > 0 ? "-" : "+") + this.__P_141_7(Math.abs(timezoneHours), 2) + ":" + this.__P_141_7(timezoneMinutes, 2);
                }
       ...
       */
      parse(dateStr) {
        this.__P_141_17(); // Apply the regex


        var hit = this.__P_141_4.regex.exec(dateStr);

        if (hit == null) {
          throw new Error("Date string '" + dateStr + "' does not match the date format: " + this.__P_141_2);
        } // Apply the rules
        ...
        for (var i = 0; i < this.__P_141_4.usedRules.length; i++) {
          var rule = this.__P_141_4.usedRules[i];
          ...
          if (rule.pattern == "Y+") {
            ...
            for (var k = 0; k < this.__P_141_4.usedRules.length; k++) {
              if (this.__P_141_4.usedRules[k].pattern == "y+") {
          ...
          if (rule.pattern.indexOf("D") != -1) {
            ...
            for (var k = 0; k < this.__P_141_4.usedRules.length; k++) {
              if (this.__P_141_4.usedRules[k].pattern.indexOf("d") != -1) {
                dayRuleApplied = true;
                break;
              }
            }
        ...
        if (applyWeekYearAfterRule) {
          dateValues.year = this.__P_141_15(dateValues.weekYear, dateValues.month, dateValues.day);
        }

        if (applyDayOfYearAfterRule) {
          var dayAndMonth = this.__P_141_14(dateValues.dayOfYear, dateValues.year);
        ...
        if (this.__P_141_3 || dateValues.tzOffsetMins !== null) {
          ...
          if (this.__P_141_3 && (dateValues.month !== date.getUTCMonth() || dateValues.year !== date.getUTCFullYear())) {
       ...
       */
      __P_141_17() {
        if (this.__P_141_4 != null) {
          // We already have the parse feed
          return;
        }

        var format = this.__P_141_2; // Initialize the rules

        this.__P_141_18();

        this.__P_141_16(); // Get the used rules and construct the regex pattern


        var usedRules = [];
        var pattern = "^";

        for (var atomIdx = 0; atomIdx < this.__P_141_6.length; atomIdx++) {
          var currAtom = this.__P_141_6[atomIdx];
          ...
          if (currAtom.type == "literal") {
            ...
            for (var ruleIdx = 0; ruleIdx < this.__P_141_5.length; ruleIdx++) {
              var rule = this.__P_141_5[ruleIdx];

              if (this.__P_141_19(rule, wildcardChar, wildcardSize)) {
       ...
       */
      __P_141_19(rule, wildcardChar, wildcardSize) {
      ...
      __P_141_18() {
        ...
        if (this.__P_141_5 != null) {
          // The parse rules are already initialized
          return;
        }

        var rules = this.__P_141_5 = [];
