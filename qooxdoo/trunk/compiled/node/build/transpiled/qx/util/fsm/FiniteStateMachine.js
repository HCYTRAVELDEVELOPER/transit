  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.util.fsm.FiniteStateMachine", {
    ...
    construct(machineName) {
      ...
      this.__P_142_0 = {}; // The first state added will become the start state

      this.__P_142_1 = null; // Initialize the saved-states stack

      this.__P_142_2 = []; // Initialize the pending event queue

      this.__P_142_3 = []; // Initialize the blocked events queue

      this.__P_142_4 = []; // Create the friendlyToObject" object.  Each object has as its property
      // name, the friendly name of the object; and as its property value, the
      // object itself.

      this.__P_142_5 = {}; // Create the "friendlyToHash" object.  Each object has as its property
      // name, the friendly name of the object; and as its property value, the
      // hash code of the object.

      this.__P_142_6 = {}; // Create the "hashToFriendly" object.  Each object has as its property
      // name, the hash code of the object; and as its property value, the
      // friendly name of the object.

      this.__P_142_7 = {}; // Friendly names can be added to groups, for easy manipulation of
      // enabling and disabling groups of widgets.  Track which friendly names
      // are in which group.

      this.__P_142_8 = {}; // We also need to be able to map back from friendly name to the groups it
      // is in.
    ...
    members: {
      ...
      replaceState(state, bDispose) {
        ...
        var oldState = this.__P_142_0[stateName]; // Replace the old state with the new state object.

        this.__P_142_0[stateName] = state; // Did they request that the old state be disposed?
      ...
      addObject(friendlyName, obj, groupNames) {
        ...
        this.__P_142_6[friendlyName] = hash;
        this.__P_142_7[hash] = friendlyName;
        this.__P_142_5[friendlyName] = obj; // If no groupNames are specified, we're done.
        ...
        for (var i = 0; i < groupNames.length; i++) {
          ...
          if (!this.__P_142_8[groupName]) {
            // ... then create it.
            this.__P_142_8[groupName] = {};
          } // Add the friendly name to the list of names in this group


          this.__P_142_8[groupName][friendlyName] = true; // If the friendly name group mapping doesn't yet exist...

          if (!this.__P_142_9[friendlyName]) {
            // ... then create it.
            this.__P_142_9[friendlyName] = [];
          } // Append this group name to the list of groups this friendly name is
          // in


          this.__P_142_9[friendlyName].push(groupName);
        }
      },
      ...
      removeObject(friendlyName) {
        ...
        var objName;
        var bGroupEmpty;
        hash = this.__P_142_6[friendlyName]; // Delete references to any groups this friendly name was in

        if (this.__P_142_9[friendlyName]) {
          for (var i = 0; i < this.__P_142_9[friendlyName].length; i++) {
            groupName = this.__P_142_9[friendlyName][i];
            delete this.__P_142_8[groupName][friendlyName]; // Is the group empty now?

            bGroupEmpty = true;

            for (objName in this.__P_142_8[groupName]) {
            ...
            if (bGroupEmpty) {
              ...
              delete this.__P_142_8[groupName];
            }
          }

          delete this.__P_142_9[friendlyName];
        } // Delete the friendly name


        delete this.__P_142_7[hash];
        delete this.__P_142_6[friendlyName];
        delete this.__P_142_5[friendlyName];
      },
      ...
      getObject(friendlyName) {
        return this.__P_142_5[friendlyName];
      },
      ...
      getFriendlyName(obj) {
        ...
        return hash ? this.__P_142_7[hash] : null;
      },
      ...
      getGroupObjects(groupName) {
        ...
        for (var name in this.__P_142_8[groupName]) {
          a.push(name);
        }
       ...
       */
      displayAllObjects() {
        for (var friendlyName in this.__P_142_6) {
          var hash = this.__P_142_6[friendlyName];
          var obj = this.getObject(friendlyName);
          this.debug(friendlyName + " => " + hash);
          this.debug("  " + hash + " => " + this.__P_142_7[hash]);
      ...
      _getInternalData() {
        return {
          states: this.__P_142_0,
          startState: this.__P_142_1,
          eventQueue: this.__P_142_3,
          blockedEvents: this.__P_142_4,
          savedStates: this.__P_142_2,
          friendlyToObject: this.__P_142_5,
          friendlyToHash: this.__P_142_6,
          hashToFriendly: this.__P_142_7,
          groupToFriendly: this.__P_142_8,
          friendlyToGroups: this.__P_142_9
        };
      },
       ...
       */
      start() {
        this.__P_142_11 = false;
        var stateName = this.__P_142_1;
        ...
        this.__P_142_0[stateName].getAutoActionsBeforeOnentry()(this); // Run the entry function for the new state, if one is specified
        ...
        this.__P_142_0[stateName].getOnentry()(this, null); // Run the actionsAfterOnentry actions for the initial state
        ...
        this.__P_142_0[stateName].getAutoActionsAfterOnentry()(this);
      },
      ...
      pushState(state) {
        ...
        if (this.__P_142_2.length >= this.getMaxSavedStates()) {
        ...
        if (state === true) {
          ...
          this.__P_142_2.push(this.getState());
        } else if (state) {
          this.__P_142_2.push(state);
        } else {
          // Push the previous state onto the saved-state stack
          this.__P_142_2.push(this.getPreviousState());
        }
      },
      ...
      popState() {
        ...
        if (this.__P_142_2.length == 0) {
          // Nope. Programmer error.
          throw new Error("Saved-state stack is empty");
        }

        return this.__P_142_2.pop();
      },
      ...
      enqueueEvent(event, bAddAtHead) {
        ...
        if (bAddAtHead) {
          ...
          this.__P_142_3.push(event);
        } else {
          // Put event at the tail of the queue
          this.__P_142_3.unshift(event);
        }
      ...
      __P_142_12() {
        ...
        this.__P_142_10 = true; // Process each of the events on the event queue

        while (this.__P_142_3.length > 0) {
          // Pull the next event from the pending event queue
          var event = this.__P_142_3.pop(); // Run the finite state machine with this event


          var bDispose = this.__P_142_13(event); // If we didn't block (and re-queue) the event, dispose it.
      ...
      __P_142_13(event) {
        ...
        currentState = this.__P_142_0[thisState]; // Get a list of the transitions available from this state
        ...
        for (var t in transitions) {
          ...
          if (typeof nextState == "string") {
            ...
            if (!nextState in this.__P_142_0) {
          ...
          currentState = this.__P_142_0[this.getNextState()]; // set previousState and state, and clear nextState, for transition
          ...
          for (var i = 0; i < this.__P_142_4.length; i++) {
            e = this.__P_142_4.pop();

            this.__P_142_3.unshift(e);
          }
    ...
    destruct() {
      this._disposeArray("__P_142_3");

      this._disposeArray("__P_142_4");

      this.__P_142_2 = this.__P_142_0 = null;
    }
