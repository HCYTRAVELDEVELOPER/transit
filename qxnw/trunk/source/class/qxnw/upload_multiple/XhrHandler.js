 ...
 * 
 * Call qxnw.upload_multiple.XhrHandler.isSupported() to check whether this class
 * can be used (otherwise use FormHandler)
 */
qx.Class.define("qxnw.upload_multiple.XhrHandler", {
	...
	members: {
		...
		_createFile: function(input) {
	        ...
	        for (var i = 0; i < bomFiles.length; i++) {
			    ...
			    var id = "upload-" + this._getUniqueFileId(),
			    	...
			    	file = new qxnw.upload_multiple.File(bomFile, filename, id),
		...
		_doUpload: function(file) {
	        ...
	        file.setUserData("qxnw.upload_multiple.XhrHandler", xhr);
	        ...
	        xhr.onreadystatechange = function(){            
	            if (xhr.readyState == 4) {
	                ...
	                file.setUserData("qxnw.upload_multiple.XhrHandler", null);
		...
		_doCancel: function(file) {
			var xhr = file.getUserData("qxnw.upload_multiple.XhrHandler");
			if (xhr) {
				xhr.abort();
				file.setUserData("qxnw.upload_multiple.XhrHandler", null);
			}
