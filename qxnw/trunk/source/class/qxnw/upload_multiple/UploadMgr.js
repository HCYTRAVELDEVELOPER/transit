 ...
 * qxnw.upload_multiple.MUploadButton. 
 * 
 * Uploader will use XhrHandler to upload via XMLHttpRequest if supported or 
 * will fall back to FormHandler.
 */
qx.Class.define("qxnw.upload_multiple.UploadMgr", {
    extend: qx.core.Object,
    include: [qxnw.upload_multiple.MParameters],
    ...
    events: {
        /**
         * Fired when a file is added to the queue; data is the qxnw.upload_multiple.File
         */
        "addFile": "qx.event.type.Data",
        /**
         * Fired when a file starts to be uploaded; data is the qxnw.upload_multiple.File
         */
        "beginUpload": "qx.event.type.Data",
        /**
         * Fired when a file has been uploaded; data is the qxnw.upload_multiple.File
         */
        "completeUpload": "qx.event.type.Data",
        /**
         * Fired when a file upload has been cancelled; data is the qxnw.upload_multiple.File
         */
    ...
    members: {
         ...
         * instance of qxnw.upload_multiple.UploadButton (see qxnw.upload_multiple.MUploadButton
        ...
        addWidget: function(widget) {
            ...
            if (qx.core.Environment.get("engine.name") != "gecko") {
                keydownId = widget.addListener("keydown", function(evt) {
                    ...
                    if (key == "Tab" || key == "Shift" || key == "Control" || key == "Alt" || key == "Backspace" || key == "Delete" || key == "CapsLock") {
                        return;
                    }
                    var data = this.__widgetsData[widget.toHashCode()];
        ...
        _createInputElement: function(widget) {
            ...
            var elem = data.inputElement = new qxnw.upload_multiple.InputElement(widget, this.getMultiple(), name);
            elem.addListenerOnce("change", qx.lang.Function.bind(this._onInputChange, this, elem));
            elem.addListenerOnce("click", function(e) {
                if (!widget.isEnabled()) {
                    e.preventDefault();
                }
            });
        ...
        getUploadHandler: function() {
            if (!this.__uploadHandler) {
                if (qxnw.upload_multiple.XhrHandler.isSupported(this.isRequireMultipartFormData()))
                    this.__uploadHandler = new qxnw.upload_multiple.XhrHandler(this);
                else
                    this.__uploadHandler = new qxnw.upload_multiple.FormHandler(this);
            }
