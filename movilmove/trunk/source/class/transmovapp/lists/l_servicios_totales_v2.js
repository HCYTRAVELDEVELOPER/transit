qx.Class.define("transmovapp.lists.l_servicios_totales_v2", {
    extend: qxnw.lists,
    construct: function () {
        var self = this;
        this.base(arguments);
...
//        self.table.setRowHeight(180);
        self.table.setRowHeight(220);

        for (var i = 0; i < columns.length; i++) {
            var col = columns[i];
            if (col.caption == "todo_html") {
                continue;
            }
            self.hideColumn(col.caption);
        }

        self.table.getTableColumnModel().setColumnWidth(6, 800);
...
//        self.spinnerUpdate.setVisibility("excluded");
//        self.totalAllContainer.setVisibility("excluded");
        /*
         */

        if (typeof self.developerContainer != "undefined") {
            self.developerContainer.setVisibility("excluded");
            self.metaAllContainer.setVisibility("excluded");
            self.totalContainer.setVisibility("excluded");
            self.addToLeftOptions(self.minimizeOptionsButton);
            self.containerFilters.setVisibility("excluded");
            self.minimizeOptionsButton.setIcon(qxnw.config.execIcon("view-fullscreen"));
        }

        self.table.setHeaderCellsVisible(false);

        self.ui.estado.setMaxWidth(80);
        self.ui.estado.setMinWidth(80);
        self.ui.origen_estado.setMaxWidth(80);
        self.ui.origen_estado.setMinWidth(80);
        self.ui.ciudad_origen.setMaxWidth(90);
        self.ui.ciudad_origen.setMinWidth(90);
        self.ui.ciudad_destino.setMaxWidth(90);
        self.ui.ciudad_destino.setMinWidth(90);
//        self.ui.fecha_inicio.setMaxWidth(90);
//        self.ui.fecha_inicio.setMinWidth(90);
//        self.ui.fecha_final.setMaxWidth(90);
//        self.ui.fecha_final.setMinWidth(90);
        self.ui.empresa.setMaxWidth(90);
        self.ui.empresa.setMinWidth(80);
        ...
        self.table.addListener("cellTap", function (e) {
            ...
            const element = document.querySelector(".container_list_servicios_totales");
...
//                d.maximize();
        ...
        self.addListener("appear", function () {
            if (!self.creado) {
                qx.bom.element.Class.add(self.getContentElement().getDomElement(), "container_list_servicios_totales");
//                var fecha = new Date();
...
//                self.ui.fecha_inicio.setValue(faño + "-" + fsmes + "-" + fsdia);
                ...
                self.applyFilters();
//                self.initIntervalTraeNuevosServicios();
            }
            self.creado = true;
        });
...
//        self.ui.cleanFiltersButton.setVisibility("excluded");
...
//        self.ui.cleanFiltersButton.setIcon(qxnw.config.execIcon("zoom-fit-best"));
//        qxnw.utils.addClassToElement(self.ui.cleanFiltersButton, "btn_redAdmi");
//        qxnw.utils.addClassToElement(self.ui.cleanFiltersButton, "btn_others");
        self.ui.emailButton.setLabel(self.tr("Versión anterior"));
...
//        self.ui.emailButton.setMinHeight(30);
        self.ui.emailButton.addListener("click", function () {
            ...
            d.setParamRecord();
            main.addSubWindow("Histórico viajes (V.old)", d);
        });
        ...
        self.ui.newButton.setMaxWidth(80);
        self.ui.newButton.setMinWidth(80);
        ...
        self.ui.deleteButton.setShow("both");
        self.ui.deleteButton.setMaxWidth(100);
        self.ui.deleteButton.setMinWidth(100);
        self.ui.deleteButton.setMaxHeight(30);
        self.ui.deleteButton.setMinHeight(30);
        self.ui.deleteButton.addListener("click", function () {
        ...
        self.ui.selectAllButton.setMaxWidth(110);
        self.ui.selectAllButton.setMinWidth(110);
...
//            d.show();
        ...
        self.ui.unSelectButton.setMaxWidth(100);
        self.ui.unSelectButton.setMinWidth(100);
...
//                self.ui.booking_accepts.setVisibility("excluded");
        ...
        contextMenu: function contextMenu(veteapp) {
            ...
            m.addAction("Ampliar", "icon/16/status/dialog-warning.png", function (e) {
                self.slotAmpliar(data);
            });
            if (main.isCustomer()) {
                ...
                if (data.estado !== "LLEGADA_DESTINO") {
                    if (data.estado !== "LIQUIDADO") {
                        ...
                        if (self.permisos.editar_servicios != "false") {
                            m.addAction("Editar", "icon/16/actions/edit-paste.png", function (e) {
                                if (data.estado === "LLEGADA_DESTINO") {
                        ...
                        if (self.permisos.asignar_conductor != "false") {
                            m.addAction("Configurar conductores", "icon/16/actions/edit-paste.png", function (e) {
                                if (data.estado === "LLEGADA_DESTINO") {
                                    qxnw.utils.information("Los viajes en estado LLEGADA_DESTINO no pueden ser editados, pruebe cambiar de estado e intente nuevamente.");
                                    return false;
                                }
...
//                }
            ...
            var func = function (r) {
                ...
                for (var i = 0; i < r.records.length; i++) {
                    ...
                    var disp = "<i class='material-icons'>laptop_mac</i>"; //laptop_mac
                    if (dat == "AppPax") {
                        disp = "<i class='material-icons'>phone_iphone</i>";
                    } else
                    if (dat == "Booking") {
                        disp = "<i class='material-icons'>card_travel</i> Booking"; //card_travel local_library
                    }
                    r.records[i].creado_por_pc_html = "<span class='origendisp_html'>" + disp + "</span>";
                    ...
                    self.arrayData[dat.id] = dat;
                    var html = "";
                    html += "<div class='containBloq'>";

                    html += "<div class='containEnc'>";
                    html += "<div class='containEncTitle'>";
                    html += "<div class='rowenctitle'><span>Viaje #" + dat.id + "</span> <span>" + dat.estatus + "</span></div>";
                    html += "<div class='rowenctitle'>" + r.records[i].paradas_adicionales_iniciales_creacion_html + "</div>";
                    html += "<div class='rowenctitle'>" + r.records[i].creado_por_pc_html + "</div>";
                    if (qxnw.utils.evalueData(dat.booking_id_journey)) {
                        html += "<div class='rowenctitle'>BookingCode " + dat.booking_id_journey + "</div>";
                    }
                    if (qxnw.utils.evalueData(dat.vuelo_numero)) {
                        html += "<div class='rowenctitle'><i class='material-icons'>flight</i> " + dat.vuelo_numero + "</div>";
                    }
                    var money = "<i class='material-icons'>attach_money</i>";
                    if (dat.tipo_pago == "tarjeta_credito") {
                        money = "<i class='material-icons'>credit_card</i>";
                    }
                    if (qxnw.utils.evalueData(dat.tipo_pago)) { //monetization_on
                        html += "<div class='rowenctitle'> " + money + " " + dat.tipo_pago + "</div>";
                    }
                    if (qxnw.utils.evalueData(dat.cliente_empresa_id_text)) {
                        html += "<div class='containPuntosODEmpresaFac'>";
                        html += "<div class='containtexttitle'><i class='material-icons'>business</i> " + dat.cliente_empresa_id_text + "</div>";
                        if (qxnw.utils.evalueData(dat.cliente_sede_id_text)) {
                            html += "<div class='containtextsede'>" + dat.cliente_sede_id_text + "</div>";
                        }
                        html += "</div>";
                    }
                    html += "</div>";
                    html += "<div class='containEncButtons'><button class='ampliarBtn' data='" + dat.id + "'>Ampliar</button> <button>Opciones</button></div>";
                    html += "</div>";

                    html += "<div class='containPuntos'>";
                    html += "<div class='containPuntosBloque'>";

                    html += "<div class='containPuntosOD'>";
                    html += "<strong><i class='material-icons'>place</i> <span class='divOrigenDest divOrigenA'>A</span> " + dat.origen + "</strong>";
                    html += "<div class='containtextcity'>" + dat.ciudad_origen + "</div>";
                    html += "</div>";

                    html += "<div class='containPuntosOD'>";
                    html += "<strong><i class='material-icons'>place</i> <span class='divOrigenDest divOrigenB'>B</span> " + dat.destino + "</strong>";
                    html += "<div class='containtextcity'>" + dat.ciudad_destino + "</div>";
                    html += "</div>";
                    ...
                    html += "<div class='containPuntosBloque'>";
                    var pasajero = "";
                    if (qxnw.utils.evalueData(r.records[i].nombre_cliente)) {
                        pasajero += "<p><i class='material-icons'>person_pin</i> " + r.records[i].nombre_cliente + "</p>";
                    }
                    if (qxnw.utils.evalueData(r.records[i].email)) {
                        pasajero += "<p><i class='material-icons'>email</i> " + r.records[i].email + "</p>";
                    }
                    if (qxnw.utils.evalueData(r.records[i].celular)) {
                        pasajero += "<p><i class='material-icons'>local_phone</i> " + r.records[i].celular + "</p>";
                    }
                    if (pasajero == "") {
                        pasajero = "<i class='material-icons'>person_pin</i> Sin pasajero";
                    }
                    html += "<div class='containPasajero'>" + pasajero + "</div>";
                    html += "</div>";

                    html += "</div>";

                    html += "<div class='containOthers'>";
                    html += "<div class='containOt containDate'><i class='material-icons'>date_range</i> " + dat.hora_fecha_servicio + "</div>";
                    html += "<div class='containOt containService'>" + dat.tipo_servicio_html + "</div>";

                    var cardata = "";
                    html += "<div class='containOt containService'><i class='material-icons'>directions_car</i> ";
                    if (qxnw.utils.evalueData(r.records[i].placa)) {
                        cardata += "<span class='vehiculo_contain_html_placa'>" + r.records[i].placa + "</span> ";
                    }
                    if (qxnw.utils.evalueData(r.records[i].vehiculo_text)) {
                        cardata += "<span>" + r.records[i].vehiculo_text + "</span>  ";
                    }
                    if (qxnw.utils.evalueData(dat.conductor)) {
                        cardata += "<span><strong>" + dat.conductor + "</strong></span>";
                    }
                    if (cardata == "") {
                        cardata = "Sin conductor/vehículo";
                    }
                    html += cardata;

                    var service = "Sin servicio";
                    if (qxnw.utils.evalueData(dat.subcategoria_servicio_text)) {
                        service = dat.subcategoria_servicio_text;
                    }
                    html += "</div>";
                    html += "<div class='containOt containConductor'><i class='material-icons'>room_service</i> " + service + "</div>";

                    html += "</div>";

                    html += "</div>";
                    r.records[i].todo_html = html;

                }
                self.setModelData(r);

                setTimeout(function () {
                    var d = document.querySelectorAll(".ampliarBtn");
                    console.log("d", d);
...
//            f.ui.id.setValue(r.id.toString());
            ...
            f.ui.accept.addListener("execute", function () {
                ...
                rpc.setAsync(true);
                var func = function (r) {
                    console.log("actualizaEstadoServicio:::responseServer", r);
                    f.accept();
                    self.applyFilters();
                };
...
//            });
        ...
        slotAmpliar: function slotAmpliar(r) {
            var self = this;
            var dat = self.selectedRecord();
//            var r = self.selectedRecord();
            console.log("self.selectedRecord:::", dat);
            console.log("r", r);
