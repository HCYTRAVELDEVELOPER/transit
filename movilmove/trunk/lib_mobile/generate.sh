...
CORDOVA_ANDROID_VERSION="11.0.0"
...
if [ -z "$1" ]
    then
        ...
        echo
        echo "  translate: Busca las palabras para traducir y genera un archivo plano por idioma"
...
ANDROIDMINVERSION='<preference name="android-minSdkVersion" value="23" />'
ANDROIDMINVERSION21='<preference name="android-minSdkVersion" value="23" />'
...
function checkAndroidVersions() {
    ...
    if [ $ADD_MIN_VER == "s" ]
        ...
        echo
        echo "Escriba la versión mínima (default: 23):"
        read MIN_VER
        if [ -z $MIN_VER ]
            then
                MIN_VER=23
...
function verifyBackgroudVariables {
    V1='<uses-permission android:name="android.permission.ACCESS_BACKGROUND_LOCATION" />'
    V1EXISTS=$(grep -R "${V1}" platforms/android/app/src/main/AndroidManifest.xml)
    echo $V1EXISTS
    if [[ -z "$V1EXISTS" ]]
    then
        echo
	    echo "Agregando variables de permisos de background en AndroidManifest.xml"
            sed -i '\/manifest>/i '"$V1"'' platforms/android/app/src/main/AndroidManifest.xml
    fi
}

function verifyFirebaseNotifications {
    V2='<uses-permission android:name="android.permission.POST_NOTIFICATIONS" \/>'
    V2EXISTS=$(grep -R "${V2}" platforms/android/app/src/main/AndroidManifest.xml)
    echo $V2EXISTS
    if [ -z $V2EXISTS ]
    then
        echo
	    echo "Agregando variables de permisos de notificaciones en AndroidManifest.xml"
            sed -i '\/application>/i '"$V2"'' platforms/android/app/src/main/AndroidManifest.xml
    fi
}
...
then
    ...
    if [ -f "$FILE" ]
		...
		then
			...
			then
                ...
                if [[ "$PLUGIN" == *"cordova-plugin-background-mode"* ]]
                    then
	    		echo
	    		echo "Plugin de background encontrado, agregando variables para Android..."
	    		verifyBackgroudVariables
		fi

                if [[ "$PLUGIN" == *"cordova-plugin-firebase-messaging"* ]]
                    then
	    		echo
	    		echo "Plugin de firebase encontrado, agregando variables para Android..."
	    		verifyFirebaseNotifications
		fi
                ...
                if [ -f "$LIBRARY" ]
                    then
                        ...
                        read UPDATE
                            if [ $UPDATE = "s" ]
                                then
                                    echo "Se comenta la descarga por CURL de la librería minimizada de nwmakerlib.gruponw.com"
                        ...
                        echo
                        echo "Se omite la descarga de la librería con CURL"
    ...
    then
                        ...
                        echo
                        echo "Buscando posibles configuraciones..."
                        if [ -f "$FILESET" ]
                                then
                                    LEN=$(jq '. | length' $FILESET)
                                    arr+=($VAR)
                                    echo "Escriba el número del proyecto a configurar:" 
                                    for (( c=0; c<=$LEN-1; c++ ))
                                        do
                        ...
                        read -p "Proyecto a seleccionar: " PROY
    ...
    then
        ...
        for file in $(find ./$folder_www/js/forms -type f -name '*.js'); do
            echo $file
            FILES_COMPRESS="${FILES_COMPRESS} $file"
        done
        ...
        for file in $(find ./$folder_www/nwmaker -type f -name 'main.js'); do
            echo $file
            FILES_COMPRESS="${FILES_COMPRESS} $file"
        done

        #uglifyjs-folder ./$folder_www/js/forms/* ./$folder_www/js/lists/*.js -c -m -o "$FOLDER_NAME/code.min.js" -c -m 
        #uglifyjs ./$folder_www/js/forms/*.js ./$folder_www/js/lists/*.js -c -m -o "$FOLDER_NAME/code.min.js" -c -m 
        uglifyjs $FILES_COMPRESS -c -m -o "$FOLDER_NAME/code.min.js" -c -m 
        ...
        if [ -f "$FILE" ]
            then
                ...
                node $FILE minify $folder $folder_www
                sleep 1
                ...
                echo "Se comenta la descarga de la librería NWMAKER por CURL"
    ...
    if [ -f "$FILE" ]
        then
            ...
            echo "Se comenta la descarga por CURL de NWMAKER"
            #read -p "Para esta función debe tener la librería NWMAKER en ../nwmakerlib_mobile. Desea descargar la versión minimizada actualizada (con curl)... s/n" -t 3 DOWN
            #if [ $DOWN == "s" ]
            #    then
            #        curl https://nwmakerlibm.gruponw.com/nwmaker-2.min.js -O
                ...
                then
                    ...
                    read UPDATE
                        if [ $UPDATE = "s" ]
                            then
                                echo
                                echo "Se omite la descarga por CURL de NWMAKER"
...
then
    ...
    #grep -E '[A-Za-z]*.text \= \"[a-zA-Z]*\";' nw.filesPhotosCamera.js

    echo "Escriba la ubicación de búsqueda (i.e ./www/js )"
    read toSearch
    ...
    #toSearch=./test/
    toSearch=./nwmakerlib_mobile/class
    #toSearch=./nwmaker/class
    #toSearch=./www/js
    langToGenerate=en
    langToGenerateTmp=enTmp
    destinationFolder=./translate

    echo "Buscando en ${toSearch}"
    echo
    sleep 1

    #BUSCAR text: 
    text=$(find ${toSearch} -type f -exec grep -R -E -o '[A-Za-z]*.text \= \"[a-zA-Z]*\";' /dev/null {} +)

    #BUSCAR html
    html=$(find ${toSearch} -type f -exec grep -R -E -o '(nw\.utils|self|this|nw\.dialog).tr\(\"[A-Za-z0-9 '"'"',;:=_><\/\-]*"\)' /dev/null {} +)

    #BUSCAR nw.utils.tr
    rta=$(find ${toSearch} -type f -exec grep -R -E -o '(nw\.utils|self|this|nw\.dialog).tr\(\"[A-Za-z0-9 \'';:_,]*"\)' /dev/null {} +)

    #DIALOG nw.dialog
    dialog=$(find ${toSearch} -type f -exec grep -R -E -o '(nw\.dialog)\(\"[A-Za-z0-9 '"'"',;:=_><\/\-\+\."\!\¡]*"\)' /dev/null {} +)

    #BUSCAR label
    labels=$(find ${toSearch} -type f -exec grep -R -E -o '(\"textCancel\"|\"textAccept\"|label|text|placeholder|title): \"[A-Za-z0-9 '"'"',;:=_><\/\-]*\"' /dev/null {} +)

    #BUSCAR dialogs2
    dialog2=$(find ${toSearch} -type f -exec grep -R -E -o '(nw\.dialog)\(\"[A-Za-z0-9 '"'"',;:=_><\/\-\+\."\!\¡]*"' /dev/null {} +)

    if [ ! -d "$destinationFolder" ]; then
        mkdir $destinationFolder
    fi

    json_array="$(cat $destinationFolder/$langToGenerate.json)"

    if [ ! -f "$destinationFolder/$langToGenerateTmp.json" ]; then
        touch $destinationFolder/$langToGenerateTmp.json
    fi

    > ./$destinationFolder/$langToGenerateTmp.json
    echo "[" >> ./$destinationFolder/$langToGenerateTmp.json

    function writeInFile() {
        echo $1 >> $destinationFolder/$langToGenerate.po
    }
    ...
    function writeInJson() {
        ...
        while read repository
            do
                original=$(echo "$repository" | jq -r .original)
                reemplazo=$(echo "$repository" | jq -r .reemplazo)
        ...
        if [ $encontrado == false ]
            then
                v="${1//[\"]/}" 
                echo "{\"original\": \"${v}\", \"reemplazo\": \"\" }, " >> ./$destinationFolder/$langToGenerateTmp.json
        fi

    }

    > ./$destinationFolder/$langToGenerate.po

    writeInFile "# Andrés Flórez <direccion@netwoods.net>, 2024."
    writeInFile "#\"Project-Id-Version: 1.0\n\""
    writeInFile "#\"Report-Msgid-Bugs-To: assdres@gmail.com\n\""
    writeInFile "#\"POT-Creation-Date: 2013-04-08 12:56-0500\n\""
    writeInFile "#\"PO-Revision-Date: 2019-10-03 12:26-0500\n\""
    writeInFile "#\"Last-Translator: Flórez <direccion@netwoods.net>\n\""
    writeInFile "#\"Language-Team: Español; Castellano <>\n\""
    writeInFile "#\"MIME-Version: 1.0\n\""
    writeInFile "#\"Content-Type: text/plain; charset=UTF-8\n\""
    writeInFile "#\"Content-Transfer-Encoding: 8bit\n\""
    writeInFile "#\"Language: es\n\""
    writeInFile "#\"Plural-Forms: nplurals=2; plural=(n != 1);\n\""
    writeInFile "#\"X-Generator: NW Rocket\n\""
    writeInFile ""
    ...
    while read n; do 
      ...
      arrIN=($n)
      IFS=$oldIFS
      str=$(echo "${arrIN[1]}" | grep -E -o '"[A-Za-z0-9.,_ ]*"' )
      echo ${str}
      if [ "$str" != "\"\"" ] ;
        then
            writeInFile "#: ${arrIN[0]}"
            writeInFile "msgid ${str}"
            writeInFile "msgstr \"\""
            writeInFile ""
    ...
    while read n; do 
      ...
      arrIN=($n)
      IFS=$oldIFS
      str=$(echo "${arrIN[2]}" | grep -E -o '"[A-Za-z0-9.,_ ]*"' )
      echo ${str}
      if [ "$str" != "\"\"" ] ;
        then
            writeInFile "#: ${arrIN[0]}"
    ...
    while read n; do
      ...
      arrIN="$(cut -d ':' -f 2- <<< "$n" )"
      echo ${arrIN}
      str=$(echo "${arrIN}" | grep -E -o '\(\"[A-Za-z0-9 '"'"';:=_><\/\-]*"\)' )
      str="${str/\(/}"
      str="${str/\)/}"
      echo $str
      writeInFile "#: ${arrIN1}"
